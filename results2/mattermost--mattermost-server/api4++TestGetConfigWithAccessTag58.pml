...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/config_test.go#L58
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestGetConfigWithAccessTag580 = [1] of {int};
	run TestGetConfigWithAccessTag58(child_TestGetConfigWithAccessTag580);
	run receiver(child_TestGetConfigWithAccessTag580)
stop_process:skip
}

proctype TestGetConfigWithAccessTag58(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
