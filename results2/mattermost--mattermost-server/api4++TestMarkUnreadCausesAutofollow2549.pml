...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/post_test.go#L2549
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestMarkUnreadCausesAutofollow25490 = [1] of {int};
	run TestMarkUnreadCausesAutofollow2549(child_TestMarkUnreadCausesAutofollow25490);
	run receiver(child_TestMarkUnreadCausesAutofollow25490)
stop_process:skip
}

proctype TestMarkUnreadCausesAutofollow2549(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_TearDown2920 = [1] of {int};
	Mutexdef replyPost_propsMu;
	Mutexdef rootPost_propsMu;
	...
	run mutexMonitor(rootPost_propsMu);
	run mutexMonitor(replyPost_propsMu);
		defer1: skip;
