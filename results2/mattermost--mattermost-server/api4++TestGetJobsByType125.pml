...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/job_test.go#L125
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestGetJobsByType1250 = [1] of {int};
	run TestGetJobsByType125(child_TestGetJobsByType1250);
	run receiver(child_TestGetJobsByType1250)
stop_process:skip
}

proctype TestGetJobsByType125(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_jobsjobs = -2; // opt var_jobsjobs
