...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/command_test.go#L357
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestListAutocompleteCommands3570 = [1] of {int};
	run TestListAutocompleteCommands357(child_TestListAutocompleteCommands3570);
	run receiver(child_TestListAutocompleteCommands3570)
stop_process:skip
}

proctype TestListAutocompleteCommands357(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_listCommandslistCommands = -2; // opt var_listCommandslistCommands
