...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/upload_test.go#L139
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestGetUploadsForUser1390 = [1] of {int};
	run TestGetUploadsForUser139(child_TestGetUploadsForUser1390);
	run receiver(child_TestGetUploadsForUser1390)
stop_process:skip
}

proctype TestGetUploadsForUser139(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_uploadsuploads = -2; // opt var_uploadsuploads
