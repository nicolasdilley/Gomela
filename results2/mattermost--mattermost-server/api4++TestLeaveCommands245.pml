...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/commands_test.go#L245
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestLeaveCommands2450 = [1] of {int};
	run TestLeaveCommands245(child_TestLeaveCommands2450);
	run receiver(child_TestLeaveCommands2450)
stop_process:skip
}

proctype TestLeaveCommands245(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_cdatacdata = -2; // opt var_cdatacdata
