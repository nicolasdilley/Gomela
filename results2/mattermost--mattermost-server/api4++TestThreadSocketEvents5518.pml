...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/user_test.go#L5518
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestThreadSocketEvents55180 = [1] of {int};
	run TestThreadSocketEvents5518(child_TestThreadSocketEvents55180);
	run receiver(child_TestThreadSocketEvents55180)
stop_process:skip
}

proctype TestThreadSocketEvents5518(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	chan child_CreateWebSocketClient4311 = [1] of {int};
	Mutexdef userWSClient_Conn_writeErrMu;
	...
	int var_thread_Participantsthread_Participants = -2; // opt var_thread_Participantsthread_Participants
	...
	run mutexMonitor(userWSClient_Conn_writeErrMu);
	run CreateWebSocketClient431(th_BasicPost_propsMu,th_ConfigStore_configLock,th_Server_featureFlagSynchronizerMutex,th_Server_metricsLock,th_Server_PluginsLock,th_Server_pluginCommandsLock,th_Server_uploadLockMapMut,child_CreateWebSocketClient4311);
	child_CreateWebSocketClient4311?0;
...
proctype CreateWebSocketClient431(Mutexdef th_BasicPost_propsMu;Mutexdef th_ConfigStore_configLock;Mutexdef th_Server_featureFlagSynchronizerMutex;Mutexdef th_Server_metricsLock;Mutexdef th_Server_PluginsLock;Mutexdef th_Server_pluginCommandsLock;Mutexdef th_Server_uploadLockMapMut;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
