...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/team_test.go#L360
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestUpdateTeamSanitization3600 = [1] of {int};
	run TestUpdateTeamSanitization360(child_TestUpdateTeamSanitization3600);
	run receiver(child_TestUpdateTeamSanitization3600)
stop_process:skip
}

proctype TestUpdateTeamSanitization360(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
