...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/commands_test.go#L205
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestLoadTestChannelsCommands2050 = [1] of {int};
	run TestLoadTestChannelsCommands205(child_TestLoadTestChannelsCommands2050);
	run receiver(child_TestLoadTestChannelsCommands2050)
stop_process:skip
}

proctype TestLoadTestChannelsCommands205(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
