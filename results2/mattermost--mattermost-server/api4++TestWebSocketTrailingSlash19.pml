...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/websocket_test.go#L19
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestWebSocketTrailingSlash190 = [1] of {int};
	run TestWebSocketTrailingSlash19(child_TestWebSocketTrailingSlash190);
	run receiver(child_TestWebSocketTrailingSlash190)
stop_process:skip
}

proctype TestWebSocketTrailingSlash19(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
