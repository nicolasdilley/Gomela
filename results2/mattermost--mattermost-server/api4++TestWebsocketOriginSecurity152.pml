...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/websocket_test.go#L152
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestWebsocketOriginSecurity1520 = [1] of {int};
	run TestWebsocketOriginSecurity152(child_TestWebsocketOriginSecurity1520);
	run receiver(child_TestWebsocketOriginSecurity1520)
stop_process:skip
}

proctype TestWebsocketOriginSecurity152(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_TearDown2920 = [1] of {int};
	Mutexdef __writeErrMu;
