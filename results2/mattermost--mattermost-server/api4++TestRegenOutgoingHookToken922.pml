...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/webhook_test.go#L922
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestRegenOutgoingHookToken9220 = [1] of {int};
	run TestRegenOutgoingHookToken922(child_TestRegenOutgoingHookToken9220);
	run receiver(child_TestRegenOutgoingHookToken9220)
stop_process:skip
}

proctype TestRegenOutgoingHookToken922(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
