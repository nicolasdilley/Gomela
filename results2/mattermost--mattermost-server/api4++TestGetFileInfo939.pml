...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/file_test.go#L939
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestGetFileInfo9390 = [1] of {int};
	run TestGetFileInfo939(child_TestGetFileInfo9390);
	run receiver(child_TestGetFileInfo9390)
stop_process:skip
}

proctype TestGetFileInfo939(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
