...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/user_test.go#L3201
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestSendVerificationEmail32010 = [1] of {int};
	run TestSendVerificationEmail3201(child_TestSendVerificationEmail32010);
	run receiver(child_TestSendVerificationEmail32010)
stop_process:skip
}

proctype TestSendVerificationEmail3201(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_TearDown2920 = [1] of {int};
	chan child_GenerateTestEmail8051 = [1] of {int};
	...
	run GenerateTestEmail805(th_BasicPost_propsMu,th_ConfigStore_configLock,th_Server_featureFlagSynchronizerMutex,th_Server_metricsLock,th_Server_PluginsLock,th_Server_pluginCommandsLock,th_Server_uploadLockMapMut,child_GenerateTestEmail8051);
	child_GenerateTestEmail8051?0;
		defer1: skip;
