...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/store/storetest/reaction_store.go#L647
typedef Wgdef {
...
init { 
	...
	run receiver(child_testReactionDeadlock6470)
stop_process:skip
}
...
proctype testReactionDeadlock647(chan child) {
	...
	run receiver(child_AnonymoustestReactionDeadlock6946910);
	run AnonymoustestReactionDeadlock701691(wg,post_propsMu,child_AnonymoustestReactionDeadlock7016911);
	run receiver(child_AnonymoustestReactionDeadlock7016911);
	wg.wait?0;
	stop_process: skip;
	child!0
}
