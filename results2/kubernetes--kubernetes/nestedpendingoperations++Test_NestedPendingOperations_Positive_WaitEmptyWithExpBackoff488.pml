...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/pkg/volume/util/nestedpendingoperations/nestedpendingoperations_test.go#L488
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	...
	run receiver(child_Test_NestedPendingOperations_Positive_WaitEmptyWithExpBackoff4880)
stop_process:skip
}
...
proctype Test_NestedPendingOperations_Positive_WaitEmptyWithExpBackoff488(chan child) {
	...
	run AnonymousTest_NestedPendingOperations_Positive_WaitEmptyWithExpBackoff495494(waitDoneCh_ch,child_AnonymousTest_NestedPendingOperations_Positive_WaitEmptyWithExpBackoff4954940);
	run receiver(child_AnonymousTest_NestedPendingOperations_Positive_WaitEmptyWithExpBackoff4954940);
	run waitChannelWithTimeout870(waitDoneCh_ch,child_waitChannelWithTimeout8701);
	child_waitChannelWithTimeout8701?0;
	stop_process: skip;
	child!0
}
proctype AnonymousTest_NestedPendingOperations_Positive_WaitEmptyWithExpBackoff495494(Chandef waitDoneCh_ch;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
