...
#define var_infos343  ?? // mand var_infos343
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	...
	run perform343(var_infos343,child_perform3430);
...
proctype perform343(int infos;chan child) {
	...
	run batchPerform360(errs_ch,infos,child_batchPerform3600);
...
proctype batchPerform360(Chandef errs_ch;int infos;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
