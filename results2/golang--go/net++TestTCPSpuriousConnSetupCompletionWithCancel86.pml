...
proctype TestTCPSpuriousConnSetupCompletionWithCancel86(chan child) {
	...
	Wgdef wg;
	run wgMonitor(wg);
	wg.update!var_10000 * 2;
	

	if
	:: 5 > 0 -> 
		sem_ch.size = 5;
		run AsyncChan(sem_ch)
	:: else -> 
		run sync_monitor(sem_ch)
	fi;
		for(i : 0.. 10000-1) {
		for10: skip;
		...
		:: sem_ch.enq!0;
		:: sem_ch.sync!false -> 
			sem_ch.sending!false
		fi;
		run AnonymousTestTCPSpuriousConnSetupCompletionWithCancel9894(sem_ch,wg,child_AnonymousTestTCPSpuriousConnSetupCompletionWithCancel98940);
		run receiver(child_AnonymousTestTCPSpuriousConnSetupCompletionWithCancel98940);
		run AnonymousTestTCPSpuriousConnSetupCompletionWithCancel103103(sem_ch,wg,child_AnonymousTestTCPSpuriousConnSetupCompletionWithCancel1031031);
...
proctype AnonymousTestTCPSpuriousConnSetupCompletionWithCancel9894(Chandef sem_ch;Wgdef wg;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype AnonymousTestTCPSpuriousConnSetupCompletionWithCancel103103(Chandef sem_ch;Wgdef wg;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
