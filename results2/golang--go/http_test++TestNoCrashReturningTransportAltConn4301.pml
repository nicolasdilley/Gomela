...
// git_link=https://github.com/golang/go/blob/138d2c9b88d9e3d5adcebf9cb7c356b43d6a9782/net/http/transport_test.go#L4301
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype TestNoCrashReturningTransportAltConn4301(chan child) {
	...
	run AnonymousTestNoCrashReturningTransportAltConn43154313(testDone,wg,child_AnonymousTestNoCrashReturningTransportAltConn431543130);
	...
	:: 1 > 0 -> 
		doReturned.size = 1;
		run AsyncChan(doReturned)
	:: else -> 
		run sync_monitor(doReturned)
	fi;
	

	if
	:: 1 > 0 -> 
		madeRoundTripper.size = 1;
		run AsyncChan(madeRoundTripper)
	:: else -> 
		run sync_monitor(madeRoundTripper)
	fi;
	...
	if
	:: doReturned.enq!0;
	:: doReturned.sync!false -> 
		doReturned.sending!false
	fi;
	

	if
	:: madeRoundTripper.deq?state,num_msgs;
	:: madeRoundTripper.sync?state -> 
		madeRoundTripper.rcving!false
	fi;
	wg.wait?0;
		defer1: skip;
...
proctype AnonymousTestNoCrashReturningTransportAltConn43154313(Chandef testDone;Wgdef wg;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
