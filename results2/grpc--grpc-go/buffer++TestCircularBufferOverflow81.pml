...
// git_link=https://github.com/grpc/grpc-go/blob/ebd6aba6754d073a696e5727158cd0c917ce1019/internal/profiling/buffer/buffer_test.go#L81
#define TestCircularBufferOverflow_qs  ??
typedef Wgdef {
...
proctype TestCircularBufferOverflow81(chan child) {
	...
	int var_sizesize = -2; // opt var_sizesize
	run mutexMonitor(cb_drainMutex);
	

	if
	:: 0 != -2 && var_10 * size-1 != -3 -> 
				for(i : 0.. var_10 * size-1) {
			for11: skip;
...
proctype Drain240(Mutexdef cb_drainMutex;chan child) {
	...
	Wgdef wg;
	int var_qs = TestCircularBufferOverflow_qs; // mand var_qs
	int var_cb_qpcb_qp = -2; // opt var_cb_qpcb_qp
