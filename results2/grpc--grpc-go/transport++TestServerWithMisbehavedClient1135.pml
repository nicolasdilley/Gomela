...
proctype TestServerWithMisbehavedClient1135(chan child) {
	...
	run mutexMonitor(mu);
	run AnonymousTestServerWithMisbehavedClient11571151(success,server_mu,mu,child_AnonymousTestServerWithMisbehavedClient115711511);
...
proctype stop407(Mutexdef s_mu;chan child) {
	...
	bool state;
	int num_msgs;
	int s_conns = -2; // opt s_conns
	s_mu.Lock!false;
	s_mu.Unlock!false;
	stop_process: skip;
	child!0
}
proctype AnonymousTestServerWithMisbehavedClient11571151(Chandef success;Mutexdef server_mu;Mutexdef mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
