...
// git_link=https://github.com/fyne-io/fyne/blob/9b5a176fe1dc80beae72a3074d0d5cd2b409637c/internal/preferences_test.go#L17
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestPrefs_Bool170 = [1] of {int};
	run TestPrefs_Bool17(child_TestPrefs_Bool170);
	run receiver(child_TestPrefs_Bool170)
stop_process:skip
}

proctype TestPrefs_Bool17(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype fireChange69(Mutexdef p_lock;chan child) {
	...
	bool state;
	int num_msgs;
	int var_p_changeListenersp_changeListeners = -2; // opt var_p_changeListenersp_changeListeners
	p_lock.Lock!false;
		defer1: skip;
	p_lock.Unlock!false;
	stop_process: skip;
