...
// git_link=https://github.com/istio/istio/blob/9a5df21188426a6a7dd7d7503e01330a79db8a48/pilot/pkg/xds/ads_test.go#L803
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestEnvoyRDSProtocolError8030 = [1] of {int};
	run TestEnvoyRDSProtocolError803(child_TestEnvoyRDSProtocolError8030);
	run receiver(child_TestEnvoyRDSProtocolError8030)
stop_process:skip
}

proctype TestEnvoyRDSProtocolError803(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype startPush877(Mutexdef s_adsClientsMutex;Mutexdef s_mutex;Mutexdef s_updateMutex;chan child) {
	...
	bool state;
	int num_msgs;
	int s_AllClients__ = -2; // opt s_AllClients__
	stop_process: skip;
	child!0
}
