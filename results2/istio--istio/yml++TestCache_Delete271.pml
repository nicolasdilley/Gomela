...
// git_link=https://github.com/istio/istio/blob/9a5df21188426a6a7dd7d7503e01330a79db8a48/pkg/test/util/yml/cache_test.go#L271
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestCache_Delete2710 = [1] of {int};
	run TestCache_Delete271(child_TestCache_Delete2710);
	run receiver(child_TestCache_Delete2710)
stop_process:skip
}

proctype TestCache_Delete271(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_AllKeys1233 = [1] of {int};
	chan child_Delete972 = [1] of {int};
	...
	child_Apply591?0;
	run Delete97(c_mu,child_Delete972);
	child_Delete972?0;
