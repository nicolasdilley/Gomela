...
// git_link=https://github.com/istio/istio/blob/9a5df21188426a6a7dd7d7503e01330a79db8a48/pkg/test/util/yml/cache_test.go#L234
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestCache_Clear2340 = [1] of {int};
	run TestCache_Clear234(child_TestCache_Clear2340);
	run receiver(child_TestCache_Clear2340)
stop_process:skip
}

proctype TestCache_Clear234(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_AllKeys1233 = [1] of {int};
	chan child_Clear1372 = [1] of {int};
	chan child_Apply591 = [1] of {int};
	chan child_Apply590 = [1] of {int};
	Mutexdef c_mu;
	run mutexMonitor(c_mu);
	run Apply59(c_mu,child_Apply590);
	child_Apply590?0;
	run Apply59(c_mu,child_Apply591);
	child_Apply591?0;
	run Clear137(c_mu,child_Clear1372);
	child_Clear1372?0;
...
proctype Apply59(Mutexdef c_mu;chan child) {
	...
	int var_partsparts = -2; // opt var_partsparts
	c_mu.Lock!false;
...
proctype Clear137(Mutexdef c_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	chan child_deleteFile1683 = [1] of {int};
	int var_c_resourcesc_resources = -2; // opt var_c_resourcesc_resources
	c_mu.Lock!false;
	

	if
	:: var_c_resources-1 != -3 -> 
				for(i : 0.. var_c_resources-1) {
			for20: skip;
