...
#define var_numIterations220  -2 // opt var_numIterations220
#define var_numReaders220  ?? // mand var_numReaders220
#define ub_for232_2  ??
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	...
	run HammerRWMutex220(var_numIterations220,var_numReaders220,child_HammerRWMutex2200);
...
proctype HammerRWMutex220(int numIterations;int numReaders;chan child) {
	...
	run writer203(rwm_mu,cdone_ch,numIterations,child_writer2030);
	run receiver(child_writer2030);
		for(i : 0.. var_numReaders / 2-1) {
		for20: skip;
		run reader186(rwm_mu,cdone_ch,numIterations,child_reader1861);
		run receiver(child_reader1861);
		for20_end: skip
	};
	for20_exit: skip;
	run writer203(rwm_mu,cdone_ch,numIterations,child_writer2033);
	run receiver(child_writer2033);
		for(i : 0.. ub_for232_2) {
		for30: skip;
		run reader186(rwm_mu,cdone_ch,numIterations,child_reader1864);
...
proctype writer203(Mutexdef rwm_mu;Chandef cdone_ch;int numIterations;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype reader186(Mutexdef rwm_mu;Chandef cdone_ch;int numIterations;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
