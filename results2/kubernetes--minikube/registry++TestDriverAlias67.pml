...
// git_link=https://github.com/kubernetes/minikube/blob/ce01c06fef829245cf1b940ca59cc55f4fc9c658/pkg/minikube/registry/registry_test.go#L67
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype Register135(Mutexdef r_lock;chan child) {
	...
	bool state;
	int num_msgs;
	int var_def_Aliasdef_Alias = -2; // opt var_def_Aliasdef_Alias
	r_lock.Lock!false;
...
proctype List155(Mutexdef r_lock;chan child) {
	...
	bool state;
	int num_msgs;
	int var_r_driversr_drivers = -2; // opt var_r_driversr_drivers
	r_lock.RLock!false;
	goto defer1;
		defer1: skip;
	r_lock.RUnlock!false;
