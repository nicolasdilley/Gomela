...
// git_link=https://github.com/syncthing/syncthing/blob/87a0eecc31a0a5eeef1d0da510f4d5832e022e1f/lib/model/requests_test.go#L68
#define TestSymlinkTraversalRead_fs  ??
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestSymlinkTraversalRead680 = [1] of {int};
	run TestSymlinkTraversalRead68(child_TestSymlinkTraversalRead680);
	run receiver(child_TestSymlinkTraversalRead680)
stop_process:skip
}

proctype TestSymlinkTraversalRead68(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	Chandef done;
	Mutexdef fc_mut;
	int fs = TestSymlinkTraversalRead_fs; // mand fs
