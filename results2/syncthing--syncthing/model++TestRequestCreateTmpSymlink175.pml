...
// git_link=https://github.com/syncthing/syncthing/blob/87a0eecc31a0a5eeef1d0da510f4d5832e022e1f/lib/model/requests_test.go#L175
#define TestRequestCreateTmpSymlink_fs  ??
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype TestRequestCreateTmpSymlink175(chan child) {
	...
	Chandef goodIdx;
	Mutexdef fc_mut;
	int fs = TestRequestCreateTmpSymlink_fs; // mand fs
	run mutexMonitor(fc_mut);
	run sync_monitor(goodIdx);
	...
	do
	:: goodIdx.deq?state,num_msgs -> 
		break
	:: goodIdx.sync?state -> 
		goodIdx.rcving!false;
		break
	:: true -> 
		break
	od;
