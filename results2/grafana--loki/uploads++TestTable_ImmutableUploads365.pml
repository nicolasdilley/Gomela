...
proctype TestTable_ImmutableUploads365(chan child) {
	...
	int var_dirdir = -2; // opt var_dirdir
	int var_expectedDBsToUploadexpectedDBsToUpload = -2; // opt var_expectedDBsToUploadexpectedDBsToUpload
	int var_dbNamesdbNames = -2; // opt var_dbNamesdbNames
	...
	if
	:: var_expectedDBsToUpload-1 != -3 -> 
				for(i : 0.. var_expectedDBsToUpload-1) {
			for40: skip;
...
proctype Stop263(Wgdef lt_wait;Mutexdef lt_dbsMtx;chan child) {
	...
	bool state;
	int num_msgs;
	int var_lt_dbslt_dbs = -2; // opt var_lt_dbslt_dbs
