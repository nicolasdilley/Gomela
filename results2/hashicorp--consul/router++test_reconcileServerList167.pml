...
#define maxServers167  -2 // opt maxServers167
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype test_reconcileServerList167(int maxServers;chan child) {
	...
	Mutexdef m_listLock;
	int healthyServers = -2; // opt healthyServers
	int m_getServerList_servers = -2; // opt m_getServerList_servers
	int failedServers = -2; // opt failedServers
...
proctype reconcileServerList383(Mutexdef m_listLock;chan child) {
	...
	int mergedList = -2; // opt mergedList
	int newServerCfg_servers = -2; // opt newServerCfg_servers
	int l_servers = -2; // opt l_servers
	m_listLock.Lock!false;
