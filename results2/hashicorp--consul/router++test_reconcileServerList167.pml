...
// git_link=https://github.com/hashicorp/consul/blob/8d6cbe72813779f60ac24d119cc311c21003f4ce/agent/router/manager_internal_test.go#L167
#define var_maxServers167  -2 // opt var_maxServers167
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run test_reconcileServerList167(var_maxServers167,child_test_reconcileServerList1670);
	run receiver(child_test_reconcileServerList1670)
stop_process:skip
}
...
proctype test_reconcileServerList167(int maxServers;chan child) {
	...
	Mutexdef m_listLock;
	int var_healthyServershealthyServers = -2; // opt var_healthyServershealthyServers
	int var_m_getServerList_serversm_getServerList_servers = -2; // opt var_m_getServerList_serversm_getServerList_servers
	int var_failedServersfailedServers = -2; // opt var_failedServersfailedServers
	run mutexMonitor(m_listLock);
	

	if
	:: 0 != -2 && var_maxServers-1 != -3 -> 
				for(i : 0.. var_maxServers-1) {
			for13: skip;
...
proctype AddServer87(Mutexdef m_listLock;chan child) {
	...
	int var_l_serversl_servers = -2; // opt var_l_serversl_servers
...
proctype reconcileServerList383(Mutexdef m_listLock;chan child) {
	...
	int var_mergedListmergedList = -2; // opt var_mergedListmergedList
	int var_newServerCfg_serversnewServerCfg_servers = -2; // opt var_newServerCfg_serversnewServerCfg_servers
	int var_l_serversl_servers = -2; // opt var_l_serversl_servers
	m_listLock.Lock!false;
