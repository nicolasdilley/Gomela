...
// git_link=https://github.com/hashicorp/vault/blob/f8f289712a3d37115090591602a2b92503fb3a13/vault/plugin_catalog_test.go#L19
typedef Wgdef {
...
init { 
	...
	run receiver(child_TestPluginCatalog_CRUD190)
stop_process:skip
}
...
proctype TestPluginCatalog_CRUD19(chan child) {
	...
	run Get205(core_pluginCatalog_lock,child_Get2050);
	child_Get2050?0;
	run Set258(core_pluginCatalog_lock,child_Set2581);
	child_Set2581?0;
	run Get205(core_pluginCatalog_lock,child_Get2052);
	child_Get2052?0;
	run Delete342(core_pluginCatalog_lock,child_Delete3423);
	child_Delete3423?0;
...
proctype Get205(Mutexdef c_lock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype Delete342(Mutexdef c_lock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
