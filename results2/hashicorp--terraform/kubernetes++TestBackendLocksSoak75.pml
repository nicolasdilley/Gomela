...
#define TestBackendLocksSoak_lockers  ??
typedef Wgdef {
...
proctype TestBackendLocksSoak75(chan child) {
	...
	Wgdef wg;
	int lockers = TestBackendLocksSoak_lockers; // mand lockers
	int clientCount = -2; // opt clientCount
	run wgMonitor(wg);
		for(i : 0.. lockers-1) {
		for20: skip;
		wg.update!1;
...
proctype AnonymousTestBackendLocksSoak9999(Wgdef wg;chan child) {
	...
	bool state;
	int num_msgs;
	int lockAttempts = -2; // opt lockAttempts
		defer1: skip;
	wg.update!-1;
	stop_process: skip;
	child!0
