#define NewState_msgQueueSize  60
#define NewState_options  60

// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example770736716/consensus/state.go
typedef Chandef {
	chan sync = [0] of {int};
...
init { 
	...
	Chandef cs_done;
	int options = NewState_options;
	bool state = false;
	
	if
	:: msgQueueSize > 0 -> 
		cs_peerMsgQueue.size = msgQueueSize;
		run emptyChan(cs_peerMsgQueue)
	:: else -> 
	...
	:: msgQueueSize > 0 -> 
		...
		run emptyChan(cs_internalMsgQueue)
	:: else -> 
	...
	:: msgQueueSize > 0 -> 
		...
		run emptyChan(cs_statsMsgQueue)
	:: else -> 
