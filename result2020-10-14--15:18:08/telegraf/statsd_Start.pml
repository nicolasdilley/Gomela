#define Start_s_AllowedPendingMessages  60
#define Start_s_MaxTCPConnections  60
#define Start_parserGoRoutines  60

// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example985353586/plugins/inputs/statsd/statsd.go
typedef Chandef {
	chan sync = [0] of {int};
...
init { 
	...
	int parserGoRoutines = Start_parserGoRoutines;
	bool state = false;
	...
	:: s_MaxTCPConnections > 0 -> 
		...
		run emptyChan(s_accept)
	:: else -> 
		run sync_monitor(s_accept)
	fi;
	
	if
	:: 0 != -1 && s_MaxTCPConnections-1 != -1 -> 
...
for10_exit:	
	...
	:: true -> 
		...
		for(i : 1.. parserGoRoutines) {
for20:		run Anonymous2(s_in,s_done,s_accept)
	};
