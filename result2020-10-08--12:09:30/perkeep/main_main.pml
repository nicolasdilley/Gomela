...
// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example075292065/website/pk-web/pkweb.go
typedef Chandef {
	chan sync = [0] of {int};
...
init { 
	...
	run Anonymous2(emailErr,httpsErr);
	do
	:: emailErr.async_rcv?0 -> 
		break
	:: emailErr.sync?0 -> 
		break
	:: httpsErr.async_rcv?0 -> 
		break
	:: httpsErr.sync?0 -> 
		break
	od
...
proctype Anonymous2(Chandef emailErr;Chandef httpsErr) {
	bool closed; 
	int i;
	bool state;
