...
// git_link=https://github.com/labstack/echo/blob/643066594d00891e3151c7ed87244bfeddcd57b9/response_test.go#L11
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype TestResponse11(chan child) {
	...
	run Before40(res_echo_startupMutex,child_Before401);
	child_Before401?0;
	run After46(res_echo_startupMutex,child_After462);
	child_After462?0;
	run Write68(res_echo_startupMutex,child_Write683);
	child_Write683?0;
	stop_process: skip;
	child!0
}
...
proctype After46(Mutexdef r_echo_startupMutex;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype Write68(Mutexdef r_echo_startupMutex;chan child) {
	...
	int var_r_afterFuncsr_afterFuncs = -2; // opt var_r_afterFuncsr_afterFuncs
	

	if
	:: true -> 
		run WriteHeader54(r_echo_startupMutex,child_WriteHeader543);
...
proctype WriteHeader54(Mutexdef r_echo_startupMutex;chan child) {
	...
	bool state;
	int num_msgs;
	int var_r_beforeFuncsr_beforeFuncs = -2; // opt var_r_beforeFuncsr_beforeFuncs
