...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/app/slashcommands/command_test.go#L448
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestMentionsToTeamMembers4480 = [1] of {int};
	run TestMentionsToTeamMembers448(child_TestMentionsToTeamMembers4480);
	run receiver(child_TestMentionsToTeamMembers4480)
stop_process:skip
}

proctype TestMentionsToTeamMembers448(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype createUser195(Mutexdef th_BasicPost_propsMu;Mutexdef th_Server_featureFlagSynchronizerMutex;Mutexdef th_Server_metricsLock;Mutexdef th_Server_PluginsLock;Mutexdef th_Server_pluginCommandsLock;Mutexdef th_Server_uploadLockMapMut;chan child) {
	...
	bool state;
	int num_msgs;
	chan child_createUserOrGuest2032 = [1] of {int};
	run createUserOrGuest203(th_BasicPost_propsMu,th_Server_featureFlagSynchronizerMutex,th_Server_metricsLock,th_Server_PluginsLock,th_Server_pluginCommandsLock,th_Server_uploadLockMapMut,child_createUserOrGuest2032);
	child_createUserOrGuest2032?0;
	goto stop_process;
	stop_process: skip;
	child!0
}
