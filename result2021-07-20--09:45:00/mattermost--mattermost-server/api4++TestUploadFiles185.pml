...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/file_test.go#L185
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestUploadFiles1850 = [1] of {int};
	run TestUploadFiles185(child_TestUploadFiles1850);
	run receiver(child_TestUploadFiles1850)
stop_process:skip
}

proctype TestUploadFiles185(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_fileResp_FileInfosfileResp_FileInfos = -2; // opt var_fileResp_FileInfosfileResp_FileInfos
	int var_tc_namestc_names = -2; // opt var_tc_namestc_names
