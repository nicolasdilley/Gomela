...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/channel_test.go#L385
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestCreateDirectChannel3850 = [1] of {int};
	run TestCreateDirectChannel385(child_TestCreateDirectChannel3850);
	run receiver(child_TestCreateDirectChannel3850)
stop_process:skip
}

proctype TestCreateDirectChannel385(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
