...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/system_test.go#L389
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestGetAnalyticsOld3890 = [1] of {int};
	run TestGetAnalyticsOld389(child_TestGetAnalyticsOld3890);
	run receiver(child_TestGetAnalyticsOld3890)
stop_process:skip
}

proctype TestGetAnalyticsOld389(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	Mutexdef WebSocketClient_Conn_writeErrMu;
	...
	int var_rowsrows = -2; // opt var_rowsrows
	...
	run mutexMonitor(WebSocketClient_Conn_writeErrMu);
