...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/handlers_test.go#L67
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestAPIHandlersWithGzip670 = [1] of {int};
	run TestAPIHandlersWithGzip67(child_TestAPIHandlersWithGzip670);
	run receiver(child_TestAPIHandlersWithGzip670)
stop_process:skip
}

proctype TestAPIHandlersWithGzip67(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_TearDown2920 = [1] of {int};
	chan child_Init1421 = [1] of {int};
	...
	run Init142(th_Server_featureFlagSynchronizerMutex,th_Server_metricsLock,th_Server_PluginsLock,th_Server_pluginCommandsLock,th_Server_uploadLockMapMut,child_Init1421);
	child_Init1421?0;
		defer1: skip;
