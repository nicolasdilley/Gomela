...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/reaction_test.go#L209
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestGetReactions2090 = [1] of {int};
	run TestGetReactions209(child_TestGetReactions2090);
	run receiver(child_TestGetReactions2090)
stop_process:skip
}

proctype TestGetReactions209(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_userReactionsuserReactions = -2; // opt var_userReactionsuserReactions
