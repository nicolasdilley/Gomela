...
// git_link=https://github.com/rclone/rclone/blob/6ef7178ee4465f2361b6e2cf483cef5bbfd9c95c/fs/march/march.go#L128
#define var_checkers150  ?? // mand checkers line 150
#define var_jobs174  ?? // mand jobs line 174
#define ub_for155_4  ??
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype Run128(chan child) {
	...
	Wgdef wg;
	Mutexdef mu;
	int var_checkers = var_checkers150; // mand var_checkers
	run mutexMonitor(mu);
	run wgMonitor(wg);
	...
	run AnonymousRun203150(in_ch,traversing,wg,mu,child_AnonymousRun2031502);
...
proctype AnonymousRun153150(Chandef in_ch;Wgdef wg;Wgdef traversing;Mutexdef mu;chan child) {
	...
	int var_jobs = var_jobs174; // mand var_jobs
		for(i : 0.. ub_for155_4) {
		for11: skip;
		do
		:: true -> 
		...
		:: in_ch.deq?state,num_msgs -> 
			...
			:: true -> 
				...
				run AnonymousAnonymousRun153175150(in_ch,wg,traversing,mu,jobs,child_AnonymousAnonymousRun1531751500);
		...
		:: in_ch.sync?state -> 
			...
			:: true -> 
				traversing.update!var_jobs;
				run AnonymousAnonymousRun153175150(in_ch,traversing,wg,mu,jobs,child_AnonymousAnonymousRun1531751501);
...
proctype AnonymousAnonymousRun153175150(Chandef in_ch;Wgdef wg;Wgdef traversing;Mutexdef mu;int jobs;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype AnonymousRun203150(Chandef in_ch;Wgdef traversing;Wgdef wg;Mutexdef mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
