...
// git_link=https://github.com/hashicorp/consul/blob/8d6cbe72813779f60ac24d119cc311c21003f4ce/agent/router/manager_internal_test.go#L341
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestManagerInternal_saveServerList3410 = [1] of {int};
	run TestManagerInternal_saveServerList341(child_TestManagerInternal_saveServerList3410);
	run receiver(child_TestManagerInternal_saveServerList3410)
stop_process:skip
}

proctype TestManagerInternal_saveServerList341(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
