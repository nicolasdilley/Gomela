...
#define var_exprs  -2 // opt exprs line 796
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run RemoveDupExprs796(var_exprs,child_RemoveDupExprs7960);
...
proctype RemoveDupExprs796(int exprs;chan child) {
	...
	:: var_exprs-1 != -3 -> 
			...
			run HashCode456(sc_mu,sc_RuntimeStatsColl_mu,child_HashCode4560);
	...
	:: else -> 
		...
		:: true -> 
			...
			run HashCode456(sc_mu,sc_RuntimeStatsColl_mu,child_HashCode4561);
...
proctype HashCode456(Mutexdef __mu;Mutexdef __RuntimeStatsColl_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
