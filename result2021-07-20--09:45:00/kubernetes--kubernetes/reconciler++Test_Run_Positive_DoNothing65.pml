...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/pkg/kubelet/volumemanager/reconciler/reconciler_test.go#L65
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Test_Run_Positive_DoNothing650 = [1] of {int};
	run Test_Run_Positive_DoNothing65(child_Test_Run_Positive_DoNothing650);
	run receiver(child_Test_Run_Positive_DoNothing650)
stop_process:skip
}

proctype Test_Run_Positive_DoNothing65(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
