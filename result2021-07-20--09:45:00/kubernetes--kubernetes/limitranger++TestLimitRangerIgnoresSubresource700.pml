...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/plugin/pkg/admission/limitranger/admission_test.go#L700
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_TestLimitRangerIgnoresSubresource7000)
stop_process:skip
}
...
proctype runLimitFunc115(Mutexdef l_liveLookupCache_lock;chan child) {
	...
	int var_itemsitems = -2; // opt var_itemsitems
...
proctype GetLimitRanges155(Mutexdef l_liveLookupCache_lock;chan child) {
	...
	bool state;
	int num_msgs;
	int var_lruEntry_itemslruEntry_items = -2; // opt var_lruEntry_itemslruEntry_items
	int var_liveList_ItemsliveList_Items = -2; // opt var_liveList_ItemsliveList_Items
