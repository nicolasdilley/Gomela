...
// git_link=https://github.com/hashicorp/vault/blob/f8f289712a3d37115090591602a2b92503fb3a13/vault/activity_log_test.go#L830
typedef Wgdef {
...
init { 
	chan child_TestActivityLog_tokenCountExists8300 = [1] of {int};
	run TestActivityLog_tokenCountExists830(child_TestActivityLog_tokenCountExists8300);
	run receiver(child_TestActivityLog_tokenCountExists8300)
stop_process:skip
}

proctype TestActivityLog_tokenCountExists830(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_tokenCountExists5842 = [1] of {int};
	chan child_tokenCountExists5843 = [1] of {int};
	...
	:: else -> 
		...
		:: true -> 
			for23: skip;
			run tokenCountExists584(a_core_allLoggersLock,a_core_auditLock,a_core_authLock,a_core_clusterParamsLock,a_core_generateRootLock,a_core_leaderParamsLock,a_core_metricsMutex,a_core_mountsLock,a_core_rekeyLock,a_core_reloadFuncsLock,a_core_requestForwardingConnectionLock,a_core_unsealWithStoredKeysLock,a_fragmentLock,a_l,a_view_readOnlyErrLock,child_tokenCountExists5843);
			child_tokenCountExists5843?0;
			for23_end: skip
		:: true -> 
			break
		od;
