...
// git_link=https://github.com/syncthing/syncthing/blob/87a0eecc31a0a5eeef1d0da510f4d5832e022e1f/lib/nat/registry.go#L23
#define var_providers25  ?? // mand providers line 25
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype discoverAll23(chan child) {
	...
	Chandef c_ch;
	Wgdef wg;
	int var_providers = var_providers25; // mand var_providers
	run wgMonitor(wg);
	...
	run sync_monitor(done_ch);
		...
		for10: skip;
		run AnonymousdiscoverAll3131(c_ch,done_ch,wg,child_AnonymousdiscoverAll31310);
...
proctype AnonymousdiscoverAll3131(Chandef c_ch;Chandef done_ch;Wgdef wg;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	int var_f_ctx_renewal_timeout_f_ctx_renewal_timeout_ = -2; // opt var_f_ctx_renewal_timeout_f_ctx_renewal_timeout_
	

	if
	:: var_f_ctx_renewal_timeout_-1 != -3 -> 
				for(i : 0.. var_f_ctx_renewal_timeout_-1) {
			for11: skip;
			do
			:: c_ch.enq!0 -> 
				break
