...
// git_link=https://github.com/syncthing/syncthing/blob/87a0eecc31a0a5eeef1d0da510f4d5832e022e1f/lib/model/bytesemaphore_test.go#L22
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	...
	run receiver(child_TestByteSempahoreCapChangeUp220)
stop_process:skip
}
...
proctype TestByteSempahoreCapChangeUp22(chan child) {
	...
	child_take490?0;
	run sync_monitor(gotit_ch);
	run AnonymousTestByteSempahoreCapChangeUp3332(gotit_ch,s_mut,child_AnonymousTestByteSempahoreCapChangeUp33321);
	run receiver(child_AnonymousTestByteSempahoreCapChangeUp33321);
	run setCapacity94(s_mut,child_setCapacity942);
	child_setCapacity942?0;
	

	if
	:: gotit_ch.deq?state,num_msgs;
	:: gotit_ch.sync?state -> 
		gotit_ch.rcving!false
	fi;
	stop_process: skip;
	child!0
}
...
proctype AnonymousTestByteSempahoreCapChangeUp3332(Chandef gotit_ch;Mutexdef s_mut;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
