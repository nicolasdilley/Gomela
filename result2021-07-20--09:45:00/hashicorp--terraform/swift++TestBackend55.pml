...
// git_link=https://github.com/hashicorp/terraform/blob/c63c06d3c4d09a1bf1a1adc20216503e0cc2f881/backend/remote-state/swift/backend_test.go#L55
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestBackend550 = [1] of {int};
	run TestBackend55(child_TestBackend550);
	run receiver(child_TestBackend550)
stop_process:skip
}

proctype TestBackend55(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype cleanObjects354(Mutexdef c_mu;chan child) {
	...
	int var_objectNamesobjectNames = -2; // opt var_objectNamesobjectNames
