...
// git_link=https://github.com/hashicorp/terraform/blob/c63c06d3c4d09a1bf1a1adc20216503e0cc2f881/states/statemgr/filesystem_test.go#L373
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestFilesystem_refreshWhileLocked3730 = [1] of {int};
	run TestFilesystem_refreshWhileLocked373(child_TestFilesystem_refreshWhileLocked3730);
	run receiver(child_TestFilesystem_refreshWhileLocked3730)
stop_process:skip
}

proctype TestFilesystem_refreshWhileLocked373(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	chan child_RefreshState2312 = [1] of {int};
	chan child_Lock2960 = [1] of {int};
	Mutexdef s_mu;
	run mutexMonitor(s_mu);
	run Lock296(s_mu,child_Lock2960);
	child_Lock2960?0;
	run RefreshState231(s_mu,child_RefreshState2312);
	child_RefreshState2312?0;
