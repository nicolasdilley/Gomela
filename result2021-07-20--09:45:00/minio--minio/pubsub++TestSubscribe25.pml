...
// git_link=https://github.com/minio/minio/blob/e948e7cdf65028f7a8545d1a05c2baea75198bd0/pkg/pubsub/pubsub_test.go#L25
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestSubscribe250 = [1] of {int};
	run TestSubscribe25(child_TestSubscribe250);
	run receiver(child_TestSubscribe250)
stop_process:skip
}

proctype TestSubscribe25(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run Subscribe55(ch1_ch,doneCh_ch,child_Subscribe550);
	child_Subscribe550?0;
	run Subscribe55(ch2_ch,doneCh_ch,child_Subscribe551);
