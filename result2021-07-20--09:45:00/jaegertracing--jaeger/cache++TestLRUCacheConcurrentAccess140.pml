...
// git_link=https://github.com/jaegertracing/jaeger/blob/12bba8c9b91cf4a29d314934bc08f4a80e43c042/pkg/cache/lru_test.go#L140
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	...
	run receiver(child_TestLRUCacheConcurrentAccess1400)
stop_process:skip
}
...
proctype TestLRUCacheConcurrentAccess140(chan child) {
	...
	run wgMonitor(wg);
		...
		for20: skip;
		wg.update!1;
		run AnonymousTestLRUCacheConcurrentAccess159154(start_ch,wg,cache_mux,child_AnonymousTestLRUCacheConcurrentAccess1591542);
		run receiver(child_AnonymousTestLRUCacheConcurrentAccess1591542);
		for20_end: skip
	};
	for20_exit: skip;
...
proctype AnonymousTestLRUCacheConcurrentAccess159154(Chandef start_ch;Wgdef wg;Mutexdef cache_mux;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
