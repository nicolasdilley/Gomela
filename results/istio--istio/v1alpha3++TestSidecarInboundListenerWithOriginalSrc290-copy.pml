...
// git_link=https://github.com/istio/istio/blob//pilot/pkg/networking/core/v1alpha3/listener_builder_test.go#L290
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestSidecarInboundListenerWithOriginalSrc2900 = [1] of {int};
	run TestSidecarInboundListenerWithOriginalSrc290(child_TestSidecarInboundListenerWithOriginalSrc2900);
	run receiver(child_TestSidecarInboundListenerWithOriginalSrc2900)
stop_process:skip
}

proctype TestSidecarInboundListenerWithOriginalSrc290(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_l_ListenerFilters = -2; // opt var_l_ListenerFilters
