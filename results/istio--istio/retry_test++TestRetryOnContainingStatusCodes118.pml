...
// git_link=https://github.com/istio/istio/blob//pilot/pkg/networking/core/v1alpha3/route/retry/retry_test.go#L118
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestRetryOnContainingStatusCodes1180 = [1] of {int};
	run TestRetryOnContainingStatusCodes118(child_TestRetryOnContainingStatusCodes1180);
	run receiver(child_TestRetryOnContainingStatusCodes1180)
stop_process:skip
}

proctype TestRetryOnContainingStatusCodes118(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
