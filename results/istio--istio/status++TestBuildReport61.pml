...
// git_link=https://github.com/istio/istio/blob//pilot/pkg/status/reporter_test.go#L61
#define not_found_115  -2 // opt types line 349
#define def_var_prunes  -2 // opt completedResources line 192
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype TestBuildReport61(chan child) {
	...
	Mutexdef r_mu;
	int var_connections = -2; // opt var_connections
	...
	run removeCompletedResource192(r_controller_mu,r_mu,var_prunes,child_removeCompletedResource1928);
...
proctype buildReport147(Mutexdef r_controller_mu;Mutexdef r_mu;chan child) {
	...
	bool state;
	int num_msgs;
	int var_r_reverseStatus = -2; // opt var_r_reverseStatus
	int var_r_inProgressResources = -2; // opt var_r_inProgressResources
	r_mu.RLock!false;
	goto defer1;
		defer1: skip;
	r_mu.RUnlock!false;
...
proctype removeCompletedResource192(Mutexdef r_controller_mu;Mutexdef r_mu;int var_completedResources;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
