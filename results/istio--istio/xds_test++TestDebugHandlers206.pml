...
// git_link=https://github.com/istio/istio/blob//pilot/pkg/xds/debug_test.go#L206
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestDebugHandlers2060 = [1] of {int};
	run TestDebugHandlers206(child_TestDebugHandlers2060);
	run receiver(child_TestDebugHandlers2060)
stop_process:skip
}

proctype TestDebugHandlers206(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
