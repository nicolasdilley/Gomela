...
// git_link=https://github.com/istio/istio/blob//pilot/pkg/networking/core/v1alpha3/route/retry/retry_test.go#L183
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestRetryRemoteLocalities1830 = [1] of {int};
	run TestRetryRemoteLocalities183(child_TestRetryRemoteLocalities1830);
	run receiver(child_TestRetryRemoteLocalities1830)
stop_process:skip
}

proctype TestRetryRemoteLocalities183(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	Mutexdef expected_state_atomicMessageInfo_initMu;
	Mutexdef previousPrioritiesConfig_state_atomicMessageInfo_initMu;
	...
	run mutexMonitor(previousPrioritiesConfig_state_atomicMessageInfo_initMu);
	run mutexMonitor(expected_state_atomicMessageInfo_initMu);
	stop_process: skip;
	child!0
}
