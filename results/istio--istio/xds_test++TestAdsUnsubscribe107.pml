...
// https://github.com/istio/istio/blob/master/pilot/pkg/xds/ads_test.go#L107
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestAdsUnsubscribe1070 = [1] of {int};
	run TestAdsUnsubscribe107(child_TestAdsUnsubscribe1070)
stop_process:skip
}

proctype TestAdsUnsubscribe107(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
