...
// git_link=https://github.com/grafana/loki/blob//clients/pkg/promtail/targets/stdin/stdin_target_manager_test.go#L120
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype Test_Shutdown120(chan child) {
	...
	bool state;
	int num_msgs;
	chan child_Received472 = [1] of {int};
	chan child_Stop371 = [1] of {int};
	...
	run Stop37(recorder_wg,recorder_mtx,recorder_once_m,child_Stop371);
	child_Stop371?0;
	run Received47(recorder_wg,recorder_mtx,recorder_once_m,child_Received472);
	child_Received472?0;
	stop_process: skip;
	child!0
}
...
proctype newReaderTarget107(Wgdef client_wg;Mutexdef client_mtx;Mutexdef client_once_m;chan child) {
	...
	bool state;
	int num_msgs;
	int var_cfg_ServiceDiscoveryConfig_StaticConfigs = -2; // opt var_cfg_ServiceDiscoveryConfig_StaticConfigs
...
proctype Stop37(Wgdef c_wg;Mutexdef c_mtx;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype Received47(Wgdef c_wg;Mutexdef c_mtx;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
