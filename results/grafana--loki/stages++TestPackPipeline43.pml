...
// git_link=https://github.com/grafana/loki/blob//clients/pkg/logentry/stages/pack_test.go#L43
#define def_var_loadConfig  -2 // opt stgs line 27
#define def_var_stages  -2 // opt stgs line 27
#define def_var_cfg_Stages  -2 // opt stgs line 27
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestPackPipeline430 = [1] of {int};
	run TestPackPipeline43(child_TestPackPipeline430);
	run receiver(child_TestPackPipeline430)
stop_process:skip
}

proctype TestPackPipeline43(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_NewPipeline270 = [1] of {int};
	int var_loadConfig = def_var_loadConfig;
	Mutexdef registry_mtx;
	run mutexMonitor(registry_mtx);
	run NewPipeline27(registry_mtx,var_loadConfig,child_NewPipeline270);
...
proctype NewDocker11(Mutexdef registerer_mtx;chan child) {
	...
	int num_msgs;
	chan child_NewPipeline270 = [1] of {int};
	int var_stages = def_var_stages;
	run NewPipeline27(registerer_mtx,var_stages,child_NewPipeline270);
	child_NewPipeline270?0;
	goto stop_process;
	stop_process: skip;
	child!0
...
proctype NewCRI39(Mutexdef registerer_mtx;chan child) {
	...
	int var_stages = def_var_stages;
	run NewPipeline27(registerer_mtx,var_stages,child_NewPipeline271);
...
proctype newMatcherStage70(Mutexdef registerer_mtx;chan child) {
	...
	int var_cfg_Stages = def_var_cfg_Stages;
	...
	if
	:: true -> 
		run NewPipeline27(registerer_mtx,var_cfg_Stages,child_NewPipeline273);
