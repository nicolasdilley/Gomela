...
// git_link=https://github.com/grpc-ecosystem/grpc-gateway/blob//runtime/marshal_jsonpb_test.go#L23
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestJSONPbMarshal230 = [1] of {int};
	run TestJSONPbMarshal23(child_TestJSONPbMarshal230);
	run receiver(child_TestJSONPbMarshal230)
stop_process:skip
}

proctype TestJSONPbMarshal23(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	Mutexdef msg_Anytype_state_atomicMessageInfo_initMu;
	...
	run mutexMonitor(msg_Anytype_state_atomicMessageInfo_initMu);
	stop_process: skip;
	child!0
}
