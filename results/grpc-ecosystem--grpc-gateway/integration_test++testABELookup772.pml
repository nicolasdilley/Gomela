...
// git_link=https://github.com/grpc-ecosystem/grpc-gateway/blob//examples/internal/integration/integration_test.go#L772
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_testABELookup7720 = [1] of {int};
	run testABELookup772(child_testABELookup7720);
	run receiver(child_testABELookup7720)
stop_process:skip
}

proctype testABELookup772(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
