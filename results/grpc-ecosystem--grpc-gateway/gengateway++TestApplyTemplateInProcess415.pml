...
// git_link=https://github.com/grpc-ecosystem/grpc-gateway/blob//protoc-gen-grpc-gateway/internal/gengateway/template_test.go#L415
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestApplyTemplateInProcess4150 = [1] of {int};
	run TestApplyTemplateInProcess415(child_TestApplyTemplateInProcess4150);
	run receiver(child_TestApplyTemplateInProcess4150)
stop_process:skip
}

proctype TestApplyTemplateInProcess415(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_spec_sigWant = -2; // opt var_spec_sigWant
