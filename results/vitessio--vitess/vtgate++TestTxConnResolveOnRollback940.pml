...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtgate/tx_conn_test.go#L940
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestTxConnResolveOnRollback9400 = [1] of {int};
	run TestTxConnResolveOnRollback940(child_TestTxConnResolveOnRollback9400);
	run receiver(child_TestTxConnResolveOnRollback9400)
stop_process:skip
}

proctype TestTxConnResolveOnRollback940(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
