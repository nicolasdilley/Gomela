...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/proto/queryservice/queryservice_grpc.pb.go#L1027
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child__Query_VStreamRows_Handler10270 = [1] of {int};
	run _Query_VStreamRows_Handler1027(child__Query_VStreamRows_Handler10270);
	run receiver(child__Query_VStreamRows_Handler10270)
stop_process:skip
}

proctype _Query_VStreamRows_Handler1027(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	Mutexdef m_Lastpk_state_atomicMessageInfo_initMu;
	...
	run mutexMonitor(m_Lastpk_state_atomicMessageInfo_initMu);
