...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/orchestrator/logic/tablet_discovery.go#L242
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_LockShard2420 = [1] of {int};
	run LockShard242(child_LockShard2420);
	run receiver(child_LockShard2420)
stop_process:skip
}

proctype LockShard242(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
