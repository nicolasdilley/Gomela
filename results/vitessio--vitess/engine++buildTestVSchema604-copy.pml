...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtgate/engine/update_test.go#L604
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_buildTestVSchema6040 = [1] of {int};
	run buildTestVSchema604(child_buildTestVSchema6040);
	run receiver(child_buildTestVSchema6040)
stop_process:skip
}

proctype buildTestVSchema604(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_BuildVSchema1650 = [1] of {int};
	Mutexdef invschema_RoutingRules_state_atomicMessageInfo_initMu;
	Mutexdef invschema_state_atomicMessageInfo_initMu;
	run mutexMonitor(invschema_state_atomicMessageInfo_initMu);
	run mutexMonitor(invschema_RoutingRules_state_atomicMessageInfo_initMu);
	run BuildVSchema165(invschema_RoutingRules_state_atomicMessageInfo_initMu,invschema_state_atomicMessageInfo_initMu,child_BuildVSchema1650);
