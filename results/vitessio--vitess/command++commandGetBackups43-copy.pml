...
// git_link=https://github.com/vitessio/vitess/blob//go/cmd/vtctldclient/command/backups.go#L43
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_commandGetBackups430 = [1] of {int};
	run commandGetBackups43(child_commandGetBackups430);
	run receiver(child_commandGetBackups430)
stop_process:skip
}

proctype commandGetBackups43(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_resp_Backups = -2; // opt var_resp_Backups
