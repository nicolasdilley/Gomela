...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtgate/tx_conn_test.go#L1092
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestTxConnResolveConcludeTransactionFail10920 = [1] of {int};
	run TestTxConnResolveConcludeTransactionFail1092(child_TestTxConnResolveConcludeTransactionFail10920);
	run receiver(child_TestTxConnResolveConcludeTransactionFail10920)
stop_process:skip
}

proctype TestTxConnResolveConcludeTransactionFail1092(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
