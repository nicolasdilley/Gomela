...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtgate/executor_test.go#L1894
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestExecutorOtherRead18940 = [1] of {int};
	run TestExecutorOtherRead1894(child_TestExecutorOtherRead18940);
	run receiver(child_TestExecutorOtherRead18940)
stop_process:skip
}

proctype TestExecutorOtherRead1894(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
