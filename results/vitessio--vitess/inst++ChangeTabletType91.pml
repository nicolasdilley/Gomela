...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/orchestrator/inst/tablet_dao.go#L91
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_ChangeTabletType910 = [1] of {int};
	run ChangeTabletType91(child_ChangeTabletType910);
	run receiver(child_ChangeTabletType910)
stop_process:skip
}

proctype ChangeTabletType91(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
