...
// git_link=https://github.com/vitessio/vitess/blob//go/cache/lru_cache_test.go#L98
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestSetWithOldKeyUpdatesValue980 = [1] of {int};
	run TestSetWithOldKeyUpdatesValue98(child_TestSetWithOldKeyUpdatesValue980);
	run receiver(child_TestSetWithOldKeyUpdatesValue980)
stop_process:skip
}

proctype TestSetWithOldKeyUpdatesValue98(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Get762 = [1] of {int};
	...
	child_Set891?0;
	run Get76(cache_mu,child_Get762);
	child_Get762?0;
	stop_process: skip;
	child!0
}
...
proctype Get76(Mutexdef lru_mu;chan child) {
	...
	bool state;
	int num_msgs;
	chan child_moveToFront2112 = [1] of {int};
	lru_mu.Lock!false;
	...
	run moveToFront211(lru_mu,child_moveToFront2112);
	child_moveToFront2112?0;
	goto defer1;
		defer1: skip;
	lru_mu.Unlock!false;
	stop_process: skip;
