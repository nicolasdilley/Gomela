...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vttablet/tabletserver/tabletenv/logstats_test.go#L61
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestLogStatsFormat610 = [1] of {int};
	run TestLogStatsFormat61(child_TestLogStatsFormat610);
	run receiver(child_TestLogStatsFormat610)
stop_process:skip
}

proctype TestLogStatsFormat61(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_testFormat556 = [1] of {int};
	chan child_testFormat555 = [1] of {int};
	chan child_testFormat554 = [1] of {int};
	...
	run testFormat55(logStats_Target_state_atomicMessageInfo_initMu,child_testFormat554);
	child_testFormat554?0;
	run testFormat55(logStats_Target_state_atomicMessageInfo_initMu,child_testFormat555);
	child_testFormat555?0;
	run testFormat55(logStats_Target_state_atomicMessageInfo_initMu,child_testFormat556);
	child_testFormat556?0;
	stop_process: skip;
	child!0
}
