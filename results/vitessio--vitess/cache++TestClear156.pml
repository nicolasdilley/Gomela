...
// git_link=https://github.com/vitessio/vitess/blob//go/cache/lru_cache_test.go#L156
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestClear1560 = [1] of {int};
	run TestClear156(child_TestClear1560);
	run receiver(child_TestClear1560)
stop_process:skip
}

proctype TestClear156(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_UsedCapacity1552 = [1] of {int};
	chan child_Clear1241 = [1] of {int};
	chan child_Set890 = [1] of {int};
	Mutexdef cache_mu;
	run mutexMonitor(cache_mu);
	run Set89(cache_mu,child_Set890);
	child_Set890?0;
	run Clear124(cache_mu,child_Clear1241);
	child_Clear1241?0;
	run UsedCapacity155(cache_mu,child_UsedCapacity1552);
	child_UsedCapacity1552?0;
	stop_process: skip;
	child!0
}
...
proctype Clear124(Mutexdef lru_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
