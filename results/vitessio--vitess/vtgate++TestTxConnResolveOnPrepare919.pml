...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtgate/tx_conn_test.go#L919
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestTxConnResolveOnPrepare9190 = [1] of {int};
	run TestTxConnResolveOnPrepare919(child_TestTxConnResolveOnPrepare9190);
	run receiver(child_TestTxConnResolveOnPrepare9190)
stop_process:skip
}

proctype TestTxConnResolveOnPrepare919(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
