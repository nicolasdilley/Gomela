...
// git_link=https://github.com/vitessio/vitess/blob//go/cmd/vtctldclient/command/workflows.go#L42
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_commandGetWorkflows420 = [1] of {int};
	run commandGetWorkflows42(child_commandGetWorkflows420);
	run receiver(child_commandGetWorkflows420)
stop_process:skip
}

proctype commandGetWorkflows42(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
