...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtgate/tx_conn_test.go#L961
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestTxConnResolveOnCommit9610 = [1] of {int};
	run TestTxConnResolveOnCommit961(child_TestTxConnResolveOnCommit9610);
	run receiver(child_TestTxConnResolveOnCommit9610)
stop_process:skip
}

proctype TestTxConnResolveOnCommit961(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
