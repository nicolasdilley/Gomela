...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtgate/tx_conn_test.go#L990
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestTxConnResolveReadTransactionFail9900 = [1] of {int};
	run TestTxConnResolveReadTransactionFail990(child_TestTxConnResolveReadTransactionFail9900);
	run receiver(child_TestTxConnResolveReadTransactionFail9900)
stop_process:skip
}

proctype TestTxConnResolveReadTransactionFail990(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
