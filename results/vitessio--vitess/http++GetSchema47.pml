...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtadmin/http/schemas.go#L47
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_GetSchema470 = [1] of {int};
	run GetSchema47(child_GetSchema470);
	run receiver(child_GetSchema470)
stop_process:skip
}

proctype GetSchema47(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
