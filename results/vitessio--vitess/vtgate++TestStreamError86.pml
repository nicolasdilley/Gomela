...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtgate/executor_stream_test.go#L86
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestStreamError860 = [1] of {int};
	run TestStreamError86(child_TestStreamError860);
	run receiver(child_TestStreamError860)
stop_process:skip
}

proctype TestStreamError86(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
