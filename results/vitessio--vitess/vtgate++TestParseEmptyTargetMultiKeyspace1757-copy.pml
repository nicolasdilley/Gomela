...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtgate/executor_test.go#L1757
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestParseEmptyTargetMultiKeyspace17570 = [1] of {int};
	run TestParseEmptyTargetMultiKeyspace1757(child_TestParseEmptyTargetMultiKeyspace17570);
	run receiver(child_TestParseEmptyTargetMultiKeyspace17570)
stop_process:skip
}

proctype TestParseEmptyTargetMultiKeyspace1757(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
