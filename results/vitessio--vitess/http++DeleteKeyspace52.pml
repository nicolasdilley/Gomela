...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtadmin/http/keyspaces.go#L52
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_DeleteKeyspace520 = [1] of {int};
	run DeleteKeyspace52(child_DeleteKeyspace520);
	run receiver(child_DeleteKeyspace520)
stop_process:skip
}

proctype DeleteKeyspace52(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
