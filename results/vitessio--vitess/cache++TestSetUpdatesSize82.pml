...
// git_link=https://github.com/vitessio/vitess/blob//go/cache/lru_cache_test.go#L82
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestSetUpdatesSize820 = [1] of {int};
	run TestSetUpdatesSize82(child_TestSetUpdatesSize820);
	run receiver(child_TestSetUpdatesSize820)
stop_process:skip
}

proctype TestSetUpdatesSize82(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
