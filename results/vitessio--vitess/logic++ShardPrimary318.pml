...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/orchestrator/logic/tablet_discovery.go#L318
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_ShardPrimary3180 = [1] of {int};
	run ShardPrimary318(child_ShardPrimary3180);
	run receiver(child_ShardPrimary3180)
stop_process:skip
}

proctype ShardPrimary318(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
