...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vttablet/tabletserver/vstreamer/local_vschema_test.go#L197
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestFindTable1970 = [1] of {int};
	run TestFindTable197(child_TestFindTable1970);
	run receiver(child_TestFindTable1970)
stop_process:skip
}

proctype TestFindTable197(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
