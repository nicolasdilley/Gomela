...
// git_link=https://github.com/vitessio/vitess/blob//go/vt/vtgate/tx_conn_test.go#L1068
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestTxConnResolveCommitPreparedFail10680 = [1] of {int};
	run TestTxConnResolveCommitPreparedFail1068(child_TestTxConnResolveCommitPreparedFail10680);
	run receiver(child_TestTxConnResolveCommitPreparedFail10680)
stop_process:skip
}

proctype TestTxConnResolveCommitPreparedFail1068(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
