...
// git_link=https://github.com/google/grumpy/blob//runtime/file_test.go#L46
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestFileClosed460 = [1] of {int};
	run TestFileClosed46(child_TestFileClosed460);
	run receiver(child_TestFileClosed460)
stop_process:skip
}

proctype TestFileClosed46(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
