...
// git_link=https://github.com/google/grumpy/blob//runtime/weakref_test.go#L24
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestWeakRefCall240 = [1] of {int};
	run TestWeakRefCall24(child_TestWeakRefCall240);
	run receiver(child_TestWeakRefCall240)
stop_process:skip
}

proctype TestWeakRefCall24(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_newTestWeakRef1290 = [1] of {int};
	Mutexdef dupRef_hash_dict_mutex_mutex;
	Mutexdef dupRef_mutex;
	...
	run mutexMonitor(dupRef_mutex);
	run mutexMonitor(dupRef_hash_dict_mutex_mutex);
