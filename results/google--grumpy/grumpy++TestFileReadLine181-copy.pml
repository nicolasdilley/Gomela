...
// git_link=https://github.com/google/grumpy/blob//runtime/file_test.go#L181
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestFileReadLine1810 = [1] of {int};
	run TestFileReadLine181(child_TestFileReadLine1810);
	run receiver(child_TestFileReadLine1810)
stop_process:skip
}

proctype TestFileReadLine181(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
