...
// git_link=https://github.com/google/grumpy/blob//runtime/file_test.go#L158
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestFileRead1580 = [1] of {int};
	run TestFileRead158(child_TestFileRead1580);
	run receiver(child_TestFileRead1580)
stop_process:skip
}

proctype TestFileRead158(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
