...
// git_link=https://github.com/uber-go/zap/blob//level_test.go#L60
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype TestAtomicLevelMutation60(chan child) {
	...
	run wgMonitor(wg);
	run runConcurrently47(wg,10,child_runConcurrently470);
	child_runConcurrently470?0;
	run runConcurrently47(wg,10,child_runConcurrently471);
