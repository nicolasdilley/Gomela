...
// git_link=https://github.com/hashicorp/consul/blob//command/acl/role/update/role_update.go#L76
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Run760 = [1] of {int};
	run Run76(child_Run760);
	run receiver(child_Run760)
stop_process:skip
}

proctype Run76(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_r_NodeIdentities = -2; // opt var_r_NodeIdentities
	int var_parsedNodeIdents = -2; // opt var_parsedNodeIdents
	int var_r_ServiceIdentities = -2; // opt var_r_ServiceIdentities
	int var_parsedServiceIdents = -2; // opt var_parsedServiceIdents
	int var_r_Policies = -2; // opt var_r_Policies
