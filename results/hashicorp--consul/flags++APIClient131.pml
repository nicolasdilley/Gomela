...
// git_link=https://github.com/hashicorp/consul/blob//command/flags/http.go#L131
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_APIClient1310)
stop_process:skip
}
...
proctype APIClient131(chan child) {
	...
	run MergeOntoConfig139(c_Transport_altMu,c_Transport_connsPerHostMu,c_Transport_idleMu,c_Transport_nextProtoOnce_m,c_Transport_reqMu,c_Transport_TLSClientConfig_mutex,child_MergeOntoConfig1390);
	child_MergeOntoConfig1390?0;
	goto stop_process;
	stop_process: skip;
	child!0
}
proctype MergeOntoConfig139(Mutexdef c_Transport_altMu;Mutexdef c_Transport_connsPerHostMu;Mutexdef c_Transport_idleMu;Mutexdef c_Transport_nextProtoOnce_m;Mutexdef c_Transport_reqMu;Mutexdef c_Transport_TLSClientConfig_mutex;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
