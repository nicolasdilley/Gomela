...
// git_link=https://github.com/hashicorp/consul/blob//command/acl/policy/create/policy_create.go#L96
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Run960 = [1] of {int};
	run Run96(child_Run960);
	run receiver(child_Run960)
stop_process:skip
}

proctype Run96(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_getRules730 = [1] of {int};
	...
	run getRules73(client_config_Transport_altMu,client_config_Transport_connsPerHostMu,client_config_Transport_idleMu,client_config_Transport_nextProtoOnce_m,client_config_Transport_reqMu,client_config_Transport_TLSClientConfig_mutex,client_modifyLock,child_getRules730);
	child_getRules730?0;
