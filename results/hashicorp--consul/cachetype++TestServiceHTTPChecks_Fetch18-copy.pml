...
// git_link=https://github.com/hashicorp/consul/blob//agent/cache-types/service_checks_test.go#L18
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_TestServiceHTTPChecks_Fetch180)
stop_process:skip
}
...
proctype RemoveCheck208(Mutexdef m_state_tokens_l;Mutexdef m_state_tokens_persistenceLock;chan child) {
	...
	bool state;
	int num_msgs;
	int var_m_checks = -2; // opt var_m_checks
	goto stop_process;
	stop_process: skip;
	child!0
}
