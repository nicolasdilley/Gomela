...
// git_link=https://github.com/hashicorp/consul/blob//agent/consul/fsm/commands_oss_test.go#L829
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestFSM_ACL_CRUD8290 = [1] of {int};
	run TestFSM_ACL_CRUD829(child_TestFSM_ACL_CRUD8290);
	run receiver(child_TestFSM_ACL_CRUD8290)
stop_process:skip
}

proctype TestFSM_ACL_CRUD829(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Apply1183 = [1] of {int};
	chan child_Apply1182 = [1] of {int};
	...
	child_Apply1181?0;
	run Apply118(fsm_state_db_db_writer,fsm_state_lockDelay_lock,fsm_stateLock,child_Apply1182);
	child_Apply1182?0;
	run Apply118(fsm_state_db_db_writer,fsm_state_lockDelay_lock,fsm_stateLock,child_Apply1183);
	child_Apply1183?0;
	stop_process: skip;
	child!0
}
