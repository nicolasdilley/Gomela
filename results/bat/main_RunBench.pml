...
// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example769877968/bench.go
typedef Chandef {
	chan sync = [0] of {int};
...
proctype mainworker(Wgdef wg;Chandef ch;Chandef results;int ch;chan child) {
	bool closed; 
	int i;
	do
	:: ch.is_closed?state -> 
...
proctype Anonymous0(Chandef results;Chandef jobs;Wgdef wg) {
	...
	run mainworker(wg,jobs,results,jobs,child_mainworker0);
	child_mainworker0?0;
stop_process:
}
