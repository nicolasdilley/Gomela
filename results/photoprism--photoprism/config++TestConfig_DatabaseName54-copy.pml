...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/db_test.go#L54
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_DatabaseName540 = [1] of {int};
	run TestConfig_DatabaseName54(child_TestConfig_DatabaseName540);
	run receiver(child_TestConfig_DatabaseName540)
stop_process:skip
}

proctype TestConfig_DatabaseName54(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_DatabaseName1430 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run DatabaseName143(c_db_values_mu,c_once_m,child_DatabaseName1430);
	child_DatabaseName1430?0;
	stop_process: skip;
	child!0
}
