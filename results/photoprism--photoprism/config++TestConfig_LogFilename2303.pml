...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/fs_test.go#L303
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_LogFilename23030 = [1] of {int};
	run TestConfig_LogFilename2303(child_TestConfig_LogFilename23030);
	run receiver(child_TestConfig_LogFilename23030)
stop_process:skip
}

proctype TestConfig_LogFilename2303(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_LogFilename1851 = [1] of {int};
	chan child_LogFilename1850 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run LogFilename185(c_db_values_mu,c_once_m,child_LogFilename1850);
	child_LogFilename1850?0;
	run LogFilename185(c_db_values_mu,c_once_m,child_LogFilename1851);
	child_LogFilename1851?0;
	stop_process: skip;
	child!0
}
proctype LogFilename185(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
