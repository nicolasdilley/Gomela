...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/db_test.go#L9
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_DatabaseDriver90 = [1] of {int};
	run TestConfig_DatabaseDriver9(child_TestConfig_DatabaseDriver90);
	run receiver(child_TestConfig_DatabaseDriver90)
stop_process:skip
}

proctype TestConfig_DatabaseDriver9(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
