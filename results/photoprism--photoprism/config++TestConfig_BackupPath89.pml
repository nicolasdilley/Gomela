...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/config_test.go#L89
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_BackupPath890 = [1] of {int};
	run TestConfig_BackupPath89(child_TestConfig_BackupPath890);
	run receiver(child_TestConfig_BackupPath890)
stop_process:skip
}

proctype TestConfig_BackupPath89(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_BackupPath3140 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run BackupPath314(c_db_values_mu,c_once_m,child_BackupPath3140);
	child_BackupPath3140?0;
	stop_process: skip;
	child!0
}
