...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/db_test.go#L36
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_DatabaseHost360 = [1] of {int};
	run TestConfig_DatabaseHost36(child_TestConfig_DatabaseHost360);
	run receiver(child_TestConfig_DatabaseHost360)
stop_process:skip
}

proctype TestConfig_DatabaseHost36(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_DatabaseHost1140 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run DatabaseHost114(c_db_values_mu,c_once_m,child_DatabaseHost1140);
	child_DatabaseHost1140?0;
	stop_process: skip;
	child!0
}
