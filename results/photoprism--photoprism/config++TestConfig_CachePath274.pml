...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/fs_test.go#L74
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_CachePath2740 = [1] of {int};
	run TestConfig_CachePath274(child_TestConfig_CachePath2740);
	run receiver(child_TestConfig_CachePath2740)
stop_process:skip
}

proctype TestConfig_CachePath274(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_CachePath2631 = [1] of {int};
	...
	run CachePath263(c_db_values_mu,c_once_m,child_CachePath2631);
	child_CachePath2631?0;
	stop_process: skip;
	child!0
}
