...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/fs_test.go#L14
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_SidecarJson140 = [1] of {int};
	run TestConfig_SidecarJson14(child_TestConfig_SidecarJson140);
	run receiver(child_TestConfig_SidecarJson140)
stop_process:skip
}

proctype TestConfig_SidecarJson14(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_ExifToolJson2255 = [1] of {int};
	chan child_DisableExifTool324 = [1] of {int};
	chan child_ExifToolJson2253 = [1] of {int};
	chan child_ExifToolJson2252 = [1] of {int};
	chan child_DisableExifTool321 = [1] of {int};
	chan child_ExifToolJson2250 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run ExifToolJson225(c_db_values_mu,c_once_m,child_ExifToolJson2250);
	child_ExifToolJson2250?0;
	run DisableExifTool32(c_db_values_mu,c_once_m,child_DisableExifTool321);
	child_DisableExifTool321?0;
	run ExifToolJson225(c_db_values_mu,c_once_m,child_ExifToolJson2252);
	child_ExifToolJson2252?0;
	run ExifToolJson225(c_db_values_mu,c_once_m,child_ExifToolJson2253);
	child_ExifToolJson2253?0;
	run DisableExifTool32(c_db_values_mu,c_once_m,child_DisableExifTool324);
	child_DisableExifTool324?0;
	run ExifToolJson225(c_db_values_mu,c_once_m,child_ExifToolJson2255);
	child_ExifToolJson2255?0;
	stop_process: skip;
	child!0
}
proctype ExifToolJson225(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_DisableExifTool320 = [1] of {int};
	run DisableExifTool32(c_db_values_mu,c_once_m,child_DisableExifTool320);
	child_DisableExifTool320?0;
	goto stop_process;
	stop_process: skip;
	child!0
}
proctype DisableExifTool32(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_ExifToolBin2201 = [1] of {int};
	...
	run ExifToolBin220(c_db_values_mu,c_once_m,child_ExifToolBin2201);
	child_ExifToolBin2201?0;
...
proctype ExifToolBin220(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
