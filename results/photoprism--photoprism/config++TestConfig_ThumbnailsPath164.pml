...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/config_test.go#L164
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_ThumbnailsPath1640 = [1] of {int};
	run TestConfig_ThumbnailsPath164(child_TestConfig_ThumbnailsPath1640);
	run receiver(child_TestConfig_ThumbnailsPath1640)
stop_process:skip
}

proctype TestConfig_ThumbnailsPath164(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_ThumbPath501 = [1] of {int};
	chan child_ThumbPath500 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run ThumbPath50(c_db_values_mu,c_once_m,child_ThumbPath500);
	child_ThumbPath500?0;
	run ThumbPath50(c_db_values_mu,c_once_m,child_ThumbPath501);
	child_ThumbPath501?0;
	stop_process: skip;
	child!0
}
proctype ThumbPath50(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_CachePath2630 = [1] of {int};
	run CachePath263(c_db_values_mu,c_once_m,child_CachePath2630);
	child_CachePath2630?0;
	goto stop_process;
	stop_process: skip;
	child!0
}
proctype CachePath263(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
