...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/config_test.go#L231
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_BuildPath2310 = [1] of {int};
	run TestConfig_BuildPath231(child_TestConfig_BuildPath2310);
	run receiver(child_TestConfig_BuildPath2310)
stop_process:skip
}

proctype TestConfig_BuildPath231(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_BuildPath780 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run BuildPath78(c_db_values_mu,c_once_m,child_BuildPath780);
	child_BuildPath780?0;
	stop_process: skip;
	child!0
}
proctype BuildPath78(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
