...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/config_test.go#L27
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNewConfig270 = [1] of {int};
	run TestNewConfig27(child_TestNewConfig270);
	run receiver(child_TestNewConfig270)
stop_process:skip
}

proctype TestNewConfig27(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
