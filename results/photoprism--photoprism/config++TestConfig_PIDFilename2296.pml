...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/fs_test.go#L296
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_PIDFilename22960 = [1] of {int};
	run TestConfig_PIDFilename2296(child_TestConfig_PIDFilename22960);
	run receiver(child_TestConfig_PIDFilename22960)
stop_process:skip
}

proctype TestConfig_PIDFilename2296(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_PIDFilename1761 = [1] of {int};
	chan child_PIDFilename1760 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run PIDFilename176(c_db_values_mu,c_once_m,child_PIDFilename1760);
	child_PIDFilename1760?0;
	run PIDFilename176(c_db_values_mu,c_once_m,child_PIDFilename1761);
	child_PIDFilename1761?0;
	stop_process: skip;
	child!0
}
proctype PIDFilename176(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
