...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/fs_test.go#L88
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_TestdataPath880 = [1] of {int};
	run TestConfig_TestdataPath88(child_TestConfig_TestdataPath880);
	run receiver(child_TestConfig_TestdataPath880)
stop_process:skip
}

proctype TestConfig_TestdataPath88(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_TestdataPath3430 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run TestdataPath343(c_db_values_mu,c_once_m,child_TestdataPath3430);
	child_TestdataPath3430?0;
	stop_process: skip;
	child!0
}
proctype TestdataPath343(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
