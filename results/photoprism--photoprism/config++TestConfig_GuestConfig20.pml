...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/client_test.go#L20
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_GuestConfig200 = [1] of {int};
	run TestConfig_GuestConfig20(child_TestConfig_GuestConfig200);
	run receiver(child_TestConfig_GuestConfig200)
stop_process:skip
}

proctype TestConfig_GuestConfig20(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_GuestConfig2230 = [1] of {int};
	Mutexdef config_db_values_mu;
	Mutexdef config_once_m;
	run mutexMonitor(config_once_m);
	run mutexMonitor(config_db_values_mu);
	run GuestConfig223(config_db_values_mu,config_once_m,child_GuestConfig2230);
	child_GuestConfig2230?0;
	stop_process: skip;
	child!0
}
proctype GuestConfig223(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
