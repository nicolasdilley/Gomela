...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/fs_test.go#L48
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_SidecarPathIsAbs480 = [1] of {int};
	run TestConfig_SidecarPathIsAbs48(child_TestConfig_SidecarPathIsAbs480);
	run receiver(child_TestConfig_SidecarPathIsAbs480)
stop_process:skip
}

proctype TestConfig_SidecarPathIsAbs48(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_SidecarPathIsAbs2441 = [1] of {int};
	chan child_SidecarPathIsAbs2440 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run SidecarPathIsAbs244(c_db_values_mu,c_once_m,child_SidecarPathIsAbs2440);
	child_SidecarPathIsAbs2440?0;
