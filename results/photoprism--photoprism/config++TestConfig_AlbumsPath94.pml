...
// git_link=https://github.com/photoprism/photoprism/blob//internal/config/fs_test.go#L94
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfig_AlbumsPath940 = [1] of {int};
	run TestConfig_AlbumsPath94(child_TestConfig_AlbumsPath940);
	run receiver(child_TestConfig_AlbumsPath940)
stop_process:skip
}

proctype TestConfig_AlbumsPath94(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_AlbumsPath3630 = [1] of {int};
	Mutexdef c_db_values_mu;
	Mutexdef c_once_m;
	run mutexMonitor(c_once_m);
	run mutexMonitor(c_db_values_mu);
	run AlbumsPath363(c_db_values_mu,c_once_m,child_AlbumsPath3630);
	child_AlbumsPath3630?0;
	stop_process: skip;
	child!0
}
proctype AlbumsPath363(Mutexdef c_db_values_mu;Mutexdef c_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
