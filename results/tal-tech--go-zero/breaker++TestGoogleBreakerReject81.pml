...
// git_link=https://github.com/tal-tech/go-zero/blob//core/breaker/googlebreaker_test.go#L81
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestGoogleBreakerReject810 = [1] of {int};
	run TestGoogleBreakerReject81(child_TestGoogleBreakerReject810);
	run receiver(child_TestGoogleBreakerReject810)
stop_process:skip
}

proctype TestGoogleBreakerReject81(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_doReq632 = [1] of {int};
	...
	run doReq63(b_proba_lock,b_stat_lock,child_doReq632);
	child_doReq632?0;
	stop_process: skip;
	child!0
}
...
proctype doReq63(Mutexdef b_proba_lock;Mutexdef b_stat_lock;chan child) {
	...
	chan child_markSuccess894 = [1] of {int};
	chan child_markFailure935 = [1] of {int};
	chan child_accept372 = [1] of {int};
	run accept37(b_proba_lock,b_stat_lock,child_accept372);
	child_accept372?0;
	...
	if
	:: true -> 
		run markSuccess89(b_proba_lock,b_stat_lock,child_markSuccess894);
		child_markSuccess894?0
	:: true -> 
		run markFailure93(b_proba_lock,b_stat_lock,child_markFailure935);
		child_markFailure935?0
	fi;
	goto defer1;
		defer1: skip;
