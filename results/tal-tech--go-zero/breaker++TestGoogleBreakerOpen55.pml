...
// git_link=https://github.com/tal-tech/go-zero/blob//core/breaker/googlebreaker_test.go#L55
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestGoogleBreakerOpen550 = [1] of {int};
	run TestGoogleBreakerOpen55(child_TestGoogleBreakerOpen550);
	run receiver(child_TestGoogleBreakerOpen550)
stop_process:skip
}

proctype TestGoogleBreakerOpen55(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	child_accept370?0;
	run markFailed228(b_proba_lock,b_stat_lock,100000,child_markFailed2281);
