...
// git_link=https://github.com/tal-tech/go-zero/blob//core/hash/consistenthash_test.go#L113
...
init { 
	chan child_TestConsistentHash_Remove1130 = [1] of {int};
	run TestConsistentHash_Remove113(child_TestConsistentHash_Remove1130);
	run receiver(child_TestConsistentHash_Remove1130)
stop_process:skip
}

proctype TestConsistentHash_Remove113(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	chan child_Remove1282 = [1] of {int};
	...
	child_Add611?0;
	run Remove128(ch_lock,child_Remove1282);
	child_Remove1282?0;
