...
// git_link=https://github.com/tal-tech/go-zero/blob//core/syncx/resourcemanager_test.go#L36
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestResourceManager_GetResourceError360 = [1] of {int};
	run TestResourceManager_GetResourceError36(child_TestResourceManager_GetResourceError360);
	run receiver(child_TestResourceManager_GetResourceError360)
stop_process:skip
}

proctype TestResourceManager_GetResourceError36(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
