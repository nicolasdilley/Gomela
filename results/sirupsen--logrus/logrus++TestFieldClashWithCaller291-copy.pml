...
// git_link=https://github.com/sirupsen/logrus/blob//json_formatter_test.go#L291
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestFieldClashWithCaller2910 = [1] of {int};
	run TestFieldClashWithCaller291(child_TestFieldClashWithCaller2910);
	run receiver(child_TestFieldClashWithCaller2910)
stop_process:skip
}

proctype TestFieldClashWithCaller291(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Format630 = [1] of {int};
	Mutexdef e_Logger_mu_lock;
	run mutexMonitor(e_Logger_mu_lock);
	run Format63(e_Logger_mu_lock,child_Format630);
	child_Format630?0;
	stop_process: skip;
	child!0
}
