...
// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example877977840/samples/go/csv/write.go
typedef Chandef {
	chan sync = [0] of {int};
...
init { 
	int i;
	chan child_csvwriteValuesFromChan1 = [0] of {int};
	Chandef structChan;
	int structChan = 5;
	bool state = false;
	...
	run csvwriteValuesFromChan(structChan,structChan,child_csvwriteValuesFromChan1);
	child_csvwriteValuesFromChan1?0
stop_process:}
...
proctype csvwriteValuesFromChan(Chandef structChan;int structChan;chan child) {
	bool closed; 
	int i;
	do
