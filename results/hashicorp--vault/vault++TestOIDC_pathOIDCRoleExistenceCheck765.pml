...
// git_link=https://github.com/hashicorp/vault/blob//vault/identity_store_oidc_test.go#L765
typedef Wgdef {
...
proctype TestOIDC_pathOIDCRoleExistenceCheck765(chan child) {
	...
	run pathOIDCRoleExistenceCheck862(c_identityStore_db_writer,c_identityStore_groupLock,c_identityStore_lock,c_identityStore_oidcLock,child_pathOIDCRoleExistenceCheck8620);
	child_pathOIDCRoleExistenceCheck8620?0;
	run pathOIDCRoleExistenceCheck862(c_identityStore_db_writer,c_identityStore_groupLock,c_identityStore_lock,c_identityStore_oidcLock,child_pathOIDCRoleExistenceCheck8621);
	child_pathOIDCRoleExistenceCheck8621?0;
	stop_process: skip;
	child!0
}
proctype pathOIDCRoleExistenceCheck862(Mutexdef i_db_writer;Mutexdef i_groupLock;Mutexdef i_lock;Mutexdef i_oidcLock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_getOIDCRole9930 = [1] of {int};
	run getOIDCRole993(i_db_writer,i_groupLock,i_lock,i_oidcLock,child_getOIDCRole9930);
...
proctype getOIDCRole993(Mutexdef i_db_writer;Mutexdef i_groupLock;Mutexdef i_lock;Mutexdef i_oidcLock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
