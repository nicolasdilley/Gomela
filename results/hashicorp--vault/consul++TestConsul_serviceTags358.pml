...
// git_link=https://github.com/hashicorp/vault/blob//serviceregistration/consul/consul_service_registration_test.go#L358
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype TestConsul_serviceTags358(chan child) {
	...
	:: var_tests-1 != -3 -> 
				for(i : 0.. var_tests-1) {
			for10: skip;
			run fetchServiceTags521(c_Client_config_Transport_altMu,c_Client_config_Transport_connsPerHostMu,c_Client_config_Transport_idleMu,c_Client_config_Transport_reqMu,c_serviceLock,child_fetchServiceTags5210);
	...
	:: else -> 
		...
		:: true -> 
			for11: skip;
			run fetchServiceTags521(c_Client_config_Transport_altMu,c_Client_config_Transport_connsPerHostMu,c_Client_config_Transport_idleMu,c_Client_config_Transport_reqMu,c_serviceLock,child_fetchServiceTags5211);
...
proctype fetchServiceTags521(Mutexdef c_Client_config_Transport_altMu;Mutexdef c_Client_config_Transport_connsPerHostMu;Mutexdef c_Client_config_Transport_idleMu;Mutexdef c_Client_config_Transport_reqMu;Mutexdef c_serviceLock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
