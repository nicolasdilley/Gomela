...
// git_link=https://github.com/hashicorp/vault/blob//command/format_test.go#L54
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestYamlFormatter540 = [1] of {int};
	run TestYamlFormatter54(child_TestYamlFormatter540);
	run receiver(child_TestYamlFormatter540)
stop_process:skip
}

proctype TestYamlFormatter54(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
