...
// https://github.com/hashicorp/vault/blob/master/builtin/credential/aws/path_role_test.go#L260
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestBackend_pathIam2600 = [1] of {int};
	run TestBackend_pathIam260(child_TestBackend_pathIam2600)
stop_process:skip
}

proctype TestBackend_pathIam260(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	Mutexdef secondRole_Auth_Alias_state_atomicMessageInfo_initMu;
	...
	run mutexMonitor(secondRole_Auth_Alias_state_atomicMessageInfo_initMu);
	stop_process: skip;
	child!0
}
