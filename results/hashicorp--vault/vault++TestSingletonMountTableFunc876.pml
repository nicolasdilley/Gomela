...
// git_link=https://github.com/hashicorp/vault/blob//vault/mount_test.go#L876
typedef Wgdef {
...
init { 
	chan child_TestSingletonMountTableFunc8760 = [1] of {int};
	run TestSingletonMountTableFunc876(child_TestSingletonMountTableFunc8760);
	run receiver(child_TestSingletonMountTableFunc8760)
stop_process:skip
}

proctype TestSingletonMountTableFunc876(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_singletonMountTables15320 = [1] of {int};
	...
	int var_mounts_Entries = -2; // opt var_mounts_Entries
