...
// git_link=https://github.com/grafana/k6/blob//lib/executor/execution_test.go#L121
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestExecutionStateGettingVUs1210 = [1] of {int};
	run TestExecutionStateGettingVUs121(child_TestExecutionStateGettingVUs1210);
	run receiver(child_TestExecutionStateGettingVUs1210)
stop_process:skip
}

proctype TestExecutionStateGettingVUs121(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run mutexMonitor(es_vuIDSegIndexMx);
