...
// git_link=https://github.com/grafana/k6/blob//js/modules/k6/html/util.go#L86
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_nodeToElement860)
stop_process:skip
}
...
proctype nodeToElement86(chan child) {
	...
	run selToElement7(emptySel_rt_vm_interruptLock,child_selToElement70);
	child_selToElement70?0;
	goto stop_process;
	stop_process: skip;
	child!0
}
proctype selToElement7(Mutexdef sel_rt_vm_interruptLock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
