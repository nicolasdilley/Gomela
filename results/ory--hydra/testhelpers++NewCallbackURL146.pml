...
// git_link=https://github.com/ory/hydra/blob//internal/testhelpers/oauth2.go#L146
typedef Wgdef {
...
init { 
	chan child_NewCallbackURL1460 = [1] of {int};
	run NewCallbackURL146(child_NewCallbackURL1460);
	run receiver(child_NewCallbackURL1460)
stop_process:skip
}

proctype NewCallbackURL146(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
