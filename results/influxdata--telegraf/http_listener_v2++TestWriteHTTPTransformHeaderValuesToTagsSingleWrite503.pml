...
// git_link=https://github.com/influxdata/telegraf/blob//plugins/inputs/http_listener_v2/http_listener_v2_test.go#L503
typedef Wgdef {
...
init { 
	chan child_TestWriteHTTPTransformHeaderValuesToTagsSingleWrite5030 = [1] of {int};
	run TestWriteHTTPTransformHeaderValuesToTagsSingleWrite503(child_TestWriteHTTPTransformHeaderValuesToTagsSingleWrite5030);
	run receiver(child_TestWriteHTTPTransformHeaderValuesToTagsSingleWrite5030)
stop_process:skip
}

proctype TestWriteHTTPTransformHeaderValuesToTagsSingleWrite503(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
