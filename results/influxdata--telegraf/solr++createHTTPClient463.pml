...
// git_link=https://github.com/influxdata/telegraf/blob//plugins/inputs/solr/solr.go#L463
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_createHTTPClient4630 = [1] of {int};
	run createHTTPClient463(child_createHTTPClient4630);
	run receiver(child_createHTTPClient4630)
stop_process:skip
}

proctype createHTTPClient463(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
