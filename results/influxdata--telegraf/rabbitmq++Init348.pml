...
// git_link=https://github.com/influxdata/telegraf/blob//plugins/inputs/rabbitmq/rabbitmq.go#L348
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Init3480 = [1] of {int};
	run Init348(child_Init3480);
	run receiver(child_Init3480)
stop_process:skip
}

proctype Init348(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
