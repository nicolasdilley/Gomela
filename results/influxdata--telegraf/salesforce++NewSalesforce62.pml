...
// git_link=https://github.com/influxdata/telegraf/blob//plugins/inputs/salesforce/salesforce.go#L62
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_NewSalesforce620 = [1] of {int};
	run NewSalesforce62(child_NewSalesforce620);
	run receiver(child_NewSalesforce620)
stop_process:skip
}

proctype NewSalesforce62(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
