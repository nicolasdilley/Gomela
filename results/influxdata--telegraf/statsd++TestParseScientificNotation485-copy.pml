...
// git_link=https://github.com/influxdata/telegraf/blob//plugins/inputs/statsd/statsd_test.go#L485
typedef Wgdef {
...
init { 
	chan child_TestParseScientificNotation4850 = [1] of {int};
	run TestParseScientificNotation485(child_TestParseScientificNotation4850);
	run receiver(child_TestParseScientificNotation4850)
stop_process:skip
}

proctype TestParseScientificNotation485(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	Wgdef s_wg;
	Mutexdef s_cleanup;
	int var_sciNotationLines = -2; // opt var_sciNotationLines
	run mutexMonitor(s_cleanup);
	run wgMonitor(s_wg);
	

	if
	:: var_sciNotationLines-1 != -3 -> 
				for(i : 0.. var_sciNotationLines-1) {
			for10: skip;
