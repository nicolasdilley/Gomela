...
// git_link=https://github.com/influxdata/telegraf/blob//plugins/inputs/http_listener_v2/http_listener_v2_test.go#L185
typedef Wgdef {
...
init { 
	chan child_TestWriteHTTP1850 = [1] of {int};
	run TestWriteHTTP185(child_TestWriteHTTP1850);
	run receiver(child_TestWriteHTTP1850)
stop_process:skip
}

proctype TestWriteHTTP185(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Stop1961 = [1] of {int};
	chan child_createURL974 = [1] of {int};
	...
	run createURL97(listener_wg,child_createURL974);
	child_createURL974?0;
		defer1: skip;
