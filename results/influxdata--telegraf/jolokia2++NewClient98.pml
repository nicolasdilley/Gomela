...
// git_link=https://github.com/influxdata/telegraf/blob//plugins/inputs/jolokia2/client.go#L98
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_NewClient980 = [1] of {int};
	run NewClient98(child_NewClient980);
	run receiver(child_NewClient980)
stop_process:skip
}

proctype NewClient98(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
