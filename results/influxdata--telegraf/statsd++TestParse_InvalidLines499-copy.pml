...
// git_link=https://github.com/influxdata/telegraf/blob//plugins/inputs/statsd/statsd_test.go#L499
typedef Wgdef {
...
init { 
	chan child_TestParse_InvalidLines4990 = [1] of {int};
	run TestParse_InvalidLines499(child_TestParse_InvalidLines4990);
	run receiver(child_TestParse_InvalidLines4990)
stop_process:skip
}

proctype TestParse_InvalidLines499(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	Wgdef s_wg;
	Mutexdef s_cleanup;
	int var_invalidLines = -2; // opt var_invalidLines
	run mutexMonitor(s_cleanup);
	run wgMonitor(s_wg);
	

	if
	:: var_invalidLines-1 != -3 -> 
				for(i : 0.. var_invalidLines-1) {
			for10: skip;
