...
// git_link=https://github.com/influxdata/telegraf/blob//plugins/inputs/http_listener_v2/http_listener_v2_test.go#L541
typedef Wgdef {
...
init { 
	chan child_TestWriteHTTPTransformHeaderValuesToTagsBulkWrite5410 = [1] of {int};
	run TestWriteHTTPTransformHeaderValuesToTagsBulkWrite541(child_TestWriteHTTPTransformHeaderValuesToTagsBulkWrite5410);
	run receiver(child_TestWriteHTTPTransformHeaderValuesToTagsBulkWrite5410)
stop_process:skip
}

proctype TestWriteHTTPTransformHeaderValuesToTagsBulkWrite541(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	Wgdef listener_wg;
	int var_hostTags = -2; // opt var_hostTags
