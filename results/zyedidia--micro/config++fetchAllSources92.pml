...
// git_link=https://github.com/zyedidia/micro/blob//internal/config/plugin_installer.go#L92
#define def_var_count  ?? // mand count line 92
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	...
	run fetchAllSources92(def_var_count,child_fetchAllSources920);
	run receiver(child_fetchAllSources920)
stop_process:skip
}

proctype fetchAllSources92(int var_count;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	wgDone.update!1;
		for(i : 0.. var_count-1) {
		for10: skip;
		run AnonymousfetchAllSources102102(results_ch,wgDone,wgQuery,child_AnonymousfetchAllSources1021020);
...
proctype AnonymousfetchAllSources102102(Chandef results_ch;Wgdef wgDone;Wgdef wgQuery;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
