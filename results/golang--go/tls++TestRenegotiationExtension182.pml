...
// git_link=https://github.com/golang/go/blob//crypto/tls/handshake_server_test.go#L182
...
proctype TestRenegotiationExtension182(chan child) {
	...
	:: 1 > 0 -> 
		bufChan_ch.size = 1;
		run AsyncChan(bufChan_ch)
	:: else -> 
		run sync_monitor(bufChan_ch)
	fi;
	run AnonymousTestRenegotiationExtension194191(bufChan_ch,child_AnonymousTestRenegotiationExtension1941910);
	run receiver(child_AnonymousTestRenegotiationExtension1941910);
	

	if
	:: bufChan_ch.deq?state,num_msgs;
	:: bufChan_ch.sync?state -> 
		bufChan_ch.rcving!false
	fi;
	stop_process: skip;
	child!0
}
proctype AnonymousTestRenegotiationExtension194191(Chandef bufChan_ch;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	if
	:: bufChan_ch.enq!0;
	:: bufChan_ch.sync!false -> 
		bufChan_ch.sending!false
	fi;
	stop_process: skip;
	child!0
}
