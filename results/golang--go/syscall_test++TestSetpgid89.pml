...
// git_link=https://github.com/golang/go/blob//syscall/exec_unix_test.go#L89
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestSetpgid890 = [1] of {int};
	run TestSetpgid89(child_TestSetpgid890);
	run receiver(child_TestSetpgid890)
stop_process:skip
}

proctype TestSetpgid89(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
