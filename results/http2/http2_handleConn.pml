...
// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example200647836/server.go
typedef Chandef {
	chan sync = [0] of {int};
...
init { 
	...
	if
	:: 1 > 0 -> 
		sc_readFrameErrCh.size = 1;
		run emptyChan(sc_readFrameErrCh)
	:: else -> 
		run sync_monitor(sc_readFrameErrCh)
	fi;
	
	if
	:: 8 > 0 -> 
		sc_wantWriteFrameCh.size = 8;
		run emptyChan(sc_wantWriteFrameCh)
	:: else -> 
