...
// https://github.com/helm/helm/blob/master/pkg/action/list_test.go#L131
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestList_LimitOffset1310 = [1] of {int};
	run TestList_LimitOffset131(child_TestList_LimitOffset1310)
stop_process:skip
}

proctype TestList_LimitOffset131(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
