...
// https://github.com/helm/helm/blob/master/pkg/action/list_test.go#L116
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestList_BigLimit1160 = [1] of {int};
	run TestList_BigLimit116(child_TestList_BigLimit1160)
stop_process:skip
}

proctype TestList_BigLimit116(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
