...
// https://github.com/helm/helm/blob/master/pkg/action/list_test.go#L103
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestList_Limit1030 = [1] of {int};
	run TestList_Limit103(child_TestList_Limit1030)
stop_process:skip
}

proctype TestList_Limit103(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
