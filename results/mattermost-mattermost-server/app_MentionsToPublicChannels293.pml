#define MentionsToPublicChannels_channelMentions  3

// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example778389005/app/command.go
typedef Chandef {
	chan sync = [0] of {int};
...
init { 
	...
	bool state = false;
	int i;
	int channelMentions = MentionsToPublicChannels_channelMentions;
	

	if
	:: channelMentions > 0 -> 
		mentionChan.size = channelMentions;
		run AsyncChan(mentionChan)
	:: else -> 
		run sync_monitor(mentionChan)
	fi;
	run wgMonitor(wg);
		for(i : 0.. channelMentions-1) {
		for10: skip;
		wg.Add!1;
