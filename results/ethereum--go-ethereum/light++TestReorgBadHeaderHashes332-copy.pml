...
// git_link=https://github.com/ethereum/go-ethereum/blob//light/lightchain_test.go#L332
...
proctype TestReorgBadHeaderHashes332(chan child) {
	...
	Mutexdef ncm_chainHeadFeed_once_m;
	Mutexdef ncm_chainSideFeed_mu;
	Mutexdef ncm_chainSideFeed_once_m;
	Mutexdef ncm_chainFeed_mu;
	Mutexdef ncm_chainFeed_once_m;
	Mutexdef ncm_hc_numberCache_lock;
	Mutexdef ncm_hc_tdCache_lock;
	Mutexdef ncm_hc_headerCache_lock;
	...
	run mutexMonitor(ncm_hc_headerCache_lock);
	run mutexMonitor(ncm_hc_tdCache_lock);
	run mutexMonitor(ncm_hc_numberCache_lock);
	run mutexMonitor(ncm_chainFeed_once_m);
	run mutexMonitor(ncm_chainFeed_mu);
	run mutexMonitor(ncm_chainSideFeed_once_m);
	run mutexMonitor(ncm_chainSideFeed_mu);
	run mutexMonitor(ncm_chainHeadFeed_once_m);
