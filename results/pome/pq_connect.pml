#define 2  5
#define lb_for640_1  -1
#define ub_for640_2  -1

// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example406015738/Godeps/_workspace/src/github.com/lib/pq/notify.go
typedef Chandef {
	chan sync = [0] of {int};
...
init { 
	...
	run lresync(notificationChan,notificationChan,child_lresync1);
	child_lresync1?0;
...
proctype lresync(Chandef notificationChan;int notificationChan;chan child) {
	bool closed; 
	int i;
	Chandef doneChan;
	run sync_monitor(doneChan);
	run Anonymous2(doneChan,notificationChan);
	do
	:: true -> 
for20:		do
		od
...
proctype Anonymous2(Chandef doneChan;Chandef notificationChan) {
	bool closed; 
	int i;
	do
	:: true -> 
