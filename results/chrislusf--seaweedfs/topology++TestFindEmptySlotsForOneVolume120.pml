...
// git_link=https://github.com/chrislusf/seaweedfs/blob//weed/topology/volume_growth_test.go#L120
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestFindEmptySlotsForOneVolume1200 = [1] of {int};
	run TestFindEmptySlotsForOneVolume120(child_TestFindEmptySlotsForOneVolume1200);
	run receiver(child_TestFindEmptySlotsForOneVolume1200)
stop_process:skip
}

proctype TestFindEmptySlotsForOneVolume120(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
