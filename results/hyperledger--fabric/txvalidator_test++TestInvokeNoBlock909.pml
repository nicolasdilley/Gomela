...
// git_link=https://github.com/hyperledger/fabric/blob//core/committer/txvalidator/v20/validator_test.go#L909
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestInvokeNoBlock9090 = [1] of {int};
	run TestInvokeNoBlock909(child_TestInvokeNoBlock9090);
	run receiver(child_TestInvokeNoBlock9090)
stop_process:skip
}

proctype TestInvokeNoBlock909(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
