...
// git_link=https://github.com/hyperledger/fabric/blob//orderer/common/channelparticipation/restapi_test.go#L411
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestHTTPHandler_ServeHTTP_Remove4110 = [1] of {int};
	run TestHTTPHandler_ServeHTTP_Remove411(child_TestHTTPHandler_ServeHTTP_Remove4110);
	run receiver(child_TestHTTPHandler_ServeHTTP_Remove4110)
stop_process:skip
}

proctype TestHTTPHandler_ServeHTTP_Remove411(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
