...
// git_link=https://github.com/hyperledger/fabric/blob//common/ledger/blockledger/fileledger/impl_test.go#L197
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestRetrieval1970 = [1] of {int};
	run TestRetrieval197(child_TestRetrieval1970);
	run receiver(child_TestRetrieval1970)
stop_process:skip
}

proctype TestRetrieval197(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
