...
// git_link=https://github.com/hyperledger/fabric/blob//orderer/common/msgprocessor/sigfilter_test.go#L79
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestEmptyPayload790 = [1] of {int};
	run TestEmptyPayload79(child_TestEmptyPayload790);
	run receiver(child_TestEmptyPayload790)
stop_process:skip
}

proctype TestEmptyPayload79(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
