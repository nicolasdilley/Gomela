...
// git_link=https://github.com/hyperledger/fabric/blob//core/ledger/kvledger/txmgmt/statedb/stateleveldb/stateleveldb_test.go#L37
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestIterator370 = [1] of {int};
	run TestIterator37(child_TestIterator370);
	run receiver(child_TestIterator370)
stop_process:skip
}

proctype TestIterator37(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Cleanup370 = [1] of {int};
	chan child_TestIterator2211 = [1] of {int};
	...
	run TestIterator221(env_DBProvider_dbProvider_db_mutex,env_DBProvider_dbProvider_mux,child_TestIterator2211);
	child_TestIterator2211?0;
		defer1: skip;
