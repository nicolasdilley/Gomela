...
// git_link=https://github.com/hyperledger/fabric/blob//core/ledger/kvledger/txmgmt/statedb/stateleveldb/stateleveldb_test.go#L119
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestPaginatedRangeQuery1190 = [1] of {int};
	run TestPaginatedRangeQuery119(child_TestPaginatedRangeQuery1190);
	run receiver(child_TestPaginatedRangeQuery1190)
stop_process:skip
}

proctype TestPaginatedRangeQuery119(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Cleanup370 = [1] of {int};
	chan child_TestPaginatedRangeQuery7921 = [1] of {int};
	...
	run TestPaginatedRangeQuery792(env_DBProvider_dbProvider_db_mutex,env_DBProvider_dbProvider_mux,child_TestPaginatedRangeQuery7921);
	child_TestPaginatedRangeQuery7921?0;
		defer1: skip;
