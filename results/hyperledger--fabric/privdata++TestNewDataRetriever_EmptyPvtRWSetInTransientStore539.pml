...
// git_link=https://github.com/hyperledger/fabric/blob//gossip/privdata/dataretriever_test.go#L539
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNewDataRetriever_EmptyPvtRWSetInTransientStore5390 = [1] of {int};
	run TestNewDataRetriever_EmptyPvtRWSetInTransientStore539(child_TestNewDataRetriever_EmptyPvtRWSetInTransientStore5390);
	run receiver(child_TestNewDataRetriever_EmptyPvtRWSetInTransientStore5390)
stop_process:skip
}

proctype TestNewDataRetriever_EmptyPvtRWSetInTransientStore539(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
