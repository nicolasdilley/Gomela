...
// git_link=https://github.com/hyperledger/fabric/blob//common/ledger/blockledger/fileledger/impl_test.go#L130
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestInitialization1300 = [1] of {int};
	run TestInitialization130(child_TestInitialization1300);
	run receiver(child_TestInitialization1300)
stop_process:skip
}

proctype TestInitialization130(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
