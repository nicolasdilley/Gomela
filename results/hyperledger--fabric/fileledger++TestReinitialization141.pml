...
// git_link=https://github.com/hyperledger/fabric/blob//common/ledger/blockledger/fileledger/impl_test.go#L141
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestReinitialization1410 = [1] of {int};
	run TestReinitialization141(child_TestReinitialization1410);
	run receiver(child_TestReinitialization1410)
stop_process:skip
}

proctype TestReinitialization141(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_tearDown550 = [1] of {int};
	chan child_shutDown631 = [1] of {int};
	...
	run shutDown63(tev_t_context_match_mu,tev_t_context_mu,child_shutDown631);
	child_shutDown631?0;
		defer1: skip;
