...
// git_link=https://github.com/hyperledger/fabric/blob//core/ledger/kvledger/txmgmt/statedb/stateleveldb/stateleveldb_test.go#L87
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestGetStateMultipleKeys870 = [1] of {int};
	run TestGetStateMultipleKeys87(child_TestGetStateMultipleKeys870);
	run receiver(child_TestGetStateMultipleKeys870)
stop_process:skip
}

proctype TestGetStateMultipleKeys87(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Cleanup370 = [1] of {int};
	chan child_TestGetStateMultipleKeys231 = [1] of {int};
	...
	run TestGetStateMultipleKeys23(env_DBProvider_dbProvider_db_mutex,env_DBProvider_dbProvider_mux,child_TestGetStateMultipleKeys231);
	child_TestGetStateMultipleKeys231?0;
		defer1: skip;
