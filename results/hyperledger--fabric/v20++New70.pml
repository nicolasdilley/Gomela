...
// git_link=https://github.com/hyperledger/fabric/blob//core/handlers/validation/builtin/v20/validation_logic.go#L70
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run New70(child_New700);
	run receiver(child_New700)
stop_process:skip
}

proctype New70(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run NewV20Evaluator22(vpmgr_validationCtx_mutex,child_NewV20Evaluator220);
...
proctype NewV20Evaluator22(Mutexdef vpmgr_validationCtx_mutex;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
