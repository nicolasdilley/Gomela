...
// git_link=https://github.com/hyperledger/fabric/blob//internal/peer/common/peerclient.go#L99
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Endorser990 = [1] of {int};
	run Endorser99(child_Endorser990);
	run receiver(child_Endorser990)
stop_process:skip
}

proctype Endorser99(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
