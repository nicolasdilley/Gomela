...
// git_link=https://github.com/hyperledger/fabric/blob//core/ledger/confighistory/mgr_test.go#L59
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestWithEmptyCollectionConfig590 = [1] of {int};
	run TestWithEmptyCollectionConfig59(child_TestWithEmptyCollectionConfig590);
	run receiver(child_TestWithEmptyCollectionConfig590)
stop_process:skip
}

proctype TestWithEmptyCollectionConfig59(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
