...
// git_link=https://github.com/hyperledger/fabric/blob//gossip/privdata/dataretriever_test.go#L103
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNewDataRetriever_GetDataFromLedger1030 = [1] of {int};
	run TestNewDataRetriever_GetDataFromLedger103(child_TestNewDataRetriever_GetDataFromLedger1030);
	run receiver(child_TestNewDataRetriever_GetDataFromLedger1030)
stop_process:skip
}

proctype TestNewDataRetriever_GetDataFromLedger103(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
