...
// git_link=https://github.com/hyperledger/fabric/blob//core/chaincode/accesscontrol/access_test.go#L105
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_newClient1050 = [1] of {int};
	run newClient105(child_newClient1050);
	run receiver(child_newClient1050)
stop_process:skip
}

proctype newClient105(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
