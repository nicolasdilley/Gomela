...
// git_link=https://github.com/hyperledger/fabric/blob//gossip/privdata/dataretriever_test.go#L363
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNewDataRetriever_EmptyWriteSet3630 = [1] of {int};
	run TestNewDataRetriever_EmptyWriteSet363(child_TestNewDataRetriever_EmptyWriteSet3630);
	run receiver(child_TestNewDataRetriever_EmptyWriteSet3630)
stop_process:skip
}

proctype TestNewDataRetriever_EmptyWriteSet363(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
