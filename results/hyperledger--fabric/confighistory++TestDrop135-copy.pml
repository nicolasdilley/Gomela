...
// git_link=https://github.com/hyperledger/fabric/blob//core/ledger/confighistory/mgr_test.go#L135
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestDrop1350 = [1] of {int};
	run TestDrop135(child_TestDrop1350);
	run receiver(child_TestDrop1350)
stop_process:skip
}

proctype TestDrop135(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
