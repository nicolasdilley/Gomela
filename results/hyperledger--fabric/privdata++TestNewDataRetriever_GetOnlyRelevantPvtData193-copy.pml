...
// git_link=https://github.com/hyperledger/fabric/blob//gossip/privdata/dataretriever_test.go#L193
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNewDataRetriever_GetOnlyRelevantPvtData1930 = [1] of {int};
	run TestNewDataRetriever_GetOnlyRelevantPvtData193(child_TestNewDataRetriever_GetOnlyRelevantPvtData1930);
	run receiver(child_TestNewDataRetriever_GetOnlyRelevantPvtData1930)
stop_process:skip
}

proctype TestNewDataRetriever_GetOnlyRelevantPvtData193(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_pvtRWSet_RWSet = -2; // opt var_pvtRWSet_RWSet
