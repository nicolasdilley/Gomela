...
// git_link=https://github.com/hyperledger/fabric/blob//gossip/privdata/dataretriever_test.go#L254
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNewDataRetriever_GetMultipleDigests2540 = [1] of {int};
	run TestNewDataRetriever_GetMultipleDigests254(child_TestNewDataRetriever_GetMultipleDigests2540);
	run receiver(child_TestNewDataRetriever_GetMultipleDigests2540)
stop_process:skip
}

proctype TestNewDataRetriever_GetMultipleDigests254(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
