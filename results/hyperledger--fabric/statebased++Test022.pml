...
// git_link=https://github.com/hyperledger/fabric/blob//core/common/validation/statebased/v20_test.go#L22
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Test0220 = [1] of {int};
	run Test022(child_Test0220);
	run receiver(child_Test0220)
stop_process:skip
}

proctype Test022(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
