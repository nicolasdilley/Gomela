...
// git_link=https://github.com/hyperledger/fabric/blob//core/ledger/kvledger/txmgmt/statedb/stateleveldb/stateleveldb_test.go#L137
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestDataExportImport1370 = [1] of {int};
	run TestDataExportImport137(child_TestDataExportImport1370);
	run receiver(child_TestDataExportImport1370)
stop_process:skip
}

proctype TestDataExportImport137(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Cleanup370 = [1] of {int};
	chan child_TestDataExportImport10241 = [1] of {int};
	...
	run TestDataExportImport1024(env_DBProvider_dbProvider_db_mutex,env_DBProvider_dbProvider_mux,child_TestDataExportImport10241);
	child_TestDataExportImport10241?0;
		defer1: skip;
