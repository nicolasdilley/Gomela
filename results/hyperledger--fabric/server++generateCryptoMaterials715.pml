...
// git_link=https://github.com/hyperledger/fabric/blob//orderer/common/server/main_test.go#L715
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_generateCryptoMaterials7150 = [1] of {int};
	run generateCryptoMaterials715(child_generateCryptoMaterials7150);
	run receiver(child_generateCryptoMaterials7150)
stop_process:skip
}

proctype generateCryptoMaterials715(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
