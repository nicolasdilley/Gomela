...
// git_link=https://github.com/hyperledger/fabric/blob//core/ledger/kvledger/txmgmt/statedb/stateleveldb/stateleveldb_test.go#L25
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestMultiDBBasicRW250 = [1] of {int};
	run TestMultiDBBasicRW25(child_TestMultiDBBasicRW250);
	run receiver(child_TestMultiDBBasicRW250)
stop_process:skip
}

proctype TestMultiDBBasicRW25(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Cleanup370 = [1] of {int};
	chan child_TestMultiDBBasicRW1471 = [1] of {int};
	...
	run TestMultiDBBasicRW147(env_DBProvider_dbProvider_db_mutex,env_DBProvider_dbProvider_mux,child_TestMultiDBBasicRW1471);
	child_TestMultiDBBasicRW1471?0;
		defer1: skip;
