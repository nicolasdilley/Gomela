...
// git_link=https://github.com/hyperledger/fabric/blob//internal/pkg/gateway/endpoint.go#L69
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_newOrderer690 = [1] of {int};
	run newOrderer69(child_newOrderer690);
	run receiver(child_newOrderer690)
stop_process:skip
}

proctype newOrderer69(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
