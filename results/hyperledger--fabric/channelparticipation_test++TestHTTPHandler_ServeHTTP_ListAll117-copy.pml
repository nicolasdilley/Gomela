...
// git_link=https://github.com/hyperledger/fabric/blob//orderer/common/channelparticipation/restapi_test.go#L117
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestHTTPHandler_ServeHTTP_ListAll1170 = [1] of {int};
	run TestHTTPHandler_ServeHTTP_ListAll117(child_TestHTTPHandler_ServeHTTP_ListAll1170);
	run receiver(child_TestHTTPHandler_ServeHTTP_ListAll1170)
stop_process:skip
}

proctype TestHTTPHandler_ServeHTTP_ListAll117(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_listAll_Channels = -2; // opt var_listAll_Channels
