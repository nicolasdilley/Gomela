...
// git_link=https://github.com/hyperledger/fabric/blob//orderer/common/server/etcdraft_test.go#L259
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_testEtcdRaftOSNNoTLSDualListener2590 = [1] of {int};
	run testEtcdRaftOSNNoTLSDualListener259(child_testEtcdRaftOSNNoTLSDualListener2590);
	run receiver(child_testEtcdRaftOSNNoTLSDualListener2590)
stop_process:skip
}

proctype testEtcdRaftOSNNoTLSDualListener259(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
