...
// git_link=https://github.com/hyperledger/fabric/blob//internal/peer/common/peerclient.go#L186
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_SnapshotClient1860 = [1] of {int};
	run SnapshotClient186(child_SnapshotClient1860);
	run receiver(child_SnapshotClient1860)
stop_process:skip
}

proctype SnapshotClient186(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
