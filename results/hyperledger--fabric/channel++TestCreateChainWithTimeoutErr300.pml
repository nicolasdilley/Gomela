...
// git_link=https://github.com/hyperledger/fabric/blob//internal/peer/channel/create_test.go#L300
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestCreateChainWithTimeoutErr3000 = [1] of {int};
	run TestCreateChainWithTimeoutErr300(child_TestCreateChainWithTimeoutErr3000);
	run receiver(child_TestCreateChainWithTimeoutErr3000)
stop_process:skip
}

proctype TestCreateChainWithTimeoutErr300(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
