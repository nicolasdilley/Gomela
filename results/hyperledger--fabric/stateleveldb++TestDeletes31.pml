...
// git_link=https://github.com/hyperledger/fabric/blob//core/ledger/kvledger/txmgmt/statedb/stateleveldb/stateleveldb_test.go#L31
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestDeletes310 = [1] of {int};
	run TestDeletes31(child_TestDeletes310);
	run receiver(child_TestDeletes310)
stop_process:skip
}

proctype TestDeletes31(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Cleanup370 = [1] of {int};
	chan child_TestDeletes1861 = [1] of {int};
	...
	run TestDeletes186(env_DBProvider_dbProvider_db_mutex,env_DBProvider_dbProvider_mux,child_TestDeletes1861);
	child_TestDeletes1861?0;
		defer1: skip;
