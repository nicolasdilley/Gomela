...
// git_link=https://github.com/hyperledger/fabric/blob//core/common/validation/statebased/v20_test.go#L50
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Test1500 = [1] of {int};
	run Test150(child_Test1500);
	run receiver(child_Test1500)
stop_process:skip
}

proctype Test150(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
