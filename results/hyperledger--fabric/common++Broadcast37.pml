...
// git_link=https://github.com/hyperledger/fabric/blob//internal/peer/common/ordererclient.go#L37
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Broadcast370 = [1] of {int};
	run Broadcast37(child_Broadcast370);
	run receiver(child_Broadcast370)
stop_process:skip
}

proctype Broadcast37(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
