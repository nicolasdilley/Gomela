...
// git_link=https://github.com/hyperledger/fabric/blob//gossip/privdata/dataretriever_test.go#L514
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNewDataRetriever_FailedGetLedgerHeight5140 = [1] of {int};
	run TestNewDataRetriever_FailedGetLedgerHeight514(child_TestNewDataRetriever_FailedGetLedgerHeight5140);
	run receiver(child_TestNewDataRetriever_FailedGetLedgerHeight5140)
stop_process:skip
}

proctype TestNewDataRetriever_FailedGetLedgerHeight514(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
