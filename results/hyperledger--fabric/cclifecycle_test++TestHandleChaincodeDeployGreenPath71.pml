...
// git_link=https://github.com/hyperledger/fabric/blob//core/cclifecycle/lifecycle_test.go#L71
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestHandleChaincodeDeployGreenPath710 = [1] of {int};
	run TestHandleChaincodeDeployGreenPath71(child_TestHandleChaincodeDeployGreenPath710);
	run receiver(child_TestHandleChaincodeDeployGreenPath710)
stop_process:skip
}

proctype TestHandleChaincodeDeployGreenPath71(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
