...
// git_link=https://github.com/hyperledger/fabric/blob//gossip/privdata/dataretriever_test.go#L449
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNewDataRetriever_NoCollectionConfig4490 = [1] of {int};
	run TestNewDataRetriever_NoCollectionConfig449(child_TestNewDataRetriever_NoCollectionConfig4490);
	run receiver(child_TestNewDataRetriever_NoCollectionConfig4490)
stop_process:skip
}

proctype TestNewDataRetriever_NoCollectionConfig449(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
