...
// git_link=https://github.com/hyperledger/fabric/blob//orderer/common/msgprocessor/sigfilter_test.go#L67
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestAccept670 = [1] of {int};
	run TestAccept67(child_TestAccept670);
	run receiver(child_TestAccept670)
stop_process:skip
}

proctype TestAccept67(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
