...
// git_link=https://github.com/hyperledger/fabric/blob//orderer/common/msgprocessor/sigfilter_test.go#L86
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestErrorOnPolicy860 = [1] of {int};
	run TestErrorOnPolicy86(child_TestErrorOnPolicy860);
	run receiver(child_TestErrorOnPolicy860)
stop_process:skip
}

proctype TestErrorOnPolicy86(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
