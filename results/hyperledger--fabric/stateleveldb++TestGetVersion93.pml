...
// git_link=https://github.com/hyperledger/fabric/blob//core/ledger/kvledger/txmgmt/statedb/stateleveldb/stateleveldb_test.go#L93
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestGetVersion930 = [1] of {int};
	run TestGetVersion93(child_TestGetVersion930);
	run receiver(child_TestGetVersion930)
stop_process:skip
}

proctype TestGetVersion93(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Cleanup370 = [1] of {int};
	chan child_TestGetVersion5291 = [1] of {int};
	...
	run TestGetVersion529(env_DBProvider_dbProvider_db_mutex,env_DBProvider_dbProvider_mux,child_TestGetVersion5291);
	child_TestGetVersion5291?0;
		defer1: skip;
