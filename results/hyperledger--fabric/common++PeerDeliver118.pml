...
// git_link=https://github.com/hyperledger/fabric/blob//internal/peer/common/peerclient.go#L118
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_PeerDeliver1180 = [1] of {int};
	run PeerDeliver118(child_PeerDeliver1180);
	run receiver(child_PeerDeliver1180)
stop_process:skip
}

proctype PeerDeliver118(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
