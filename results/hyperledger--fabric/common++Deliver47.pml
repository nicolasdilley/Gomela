...
// git_link=https://github.com/hyperledger/fabric/blob//internal/peer/common/ordererclient.go#L47
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Deliver470 = [1] of {int};
	run Deliver47(child_Deliver470);
	run receiver(child_Deliver470)
stop_process:skip
}

proctype Deliver47(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
