...
// git_link=https://github.com/hyperledger/fabric/blob//core/ledger/kvledger/txmgmt/statedb/stateleveldb/stateleveldb_test.go#L125
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestRangeQuerySpecialCharacters1250 = [1] of {int};
	run TestRangeQuerySpecialCharacters125(child_TestRangeQuerySpecialCharacters1250);
	run receiver(child_TestRangeQuerySpecialCharacters1250)
stop_process:skip
}

proctype TestRangeQuerySpecialCharacters125(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Cleanup370 = [1] of {int};
	chan child_TestRangeQuerySpecialCharacters9191 = [1] of {int};
	...
	run TestRangeQuerySpecialCharacters919(env_DBProvider_dbProvider_db_mutex,env_DBProvider_dbProvider_mux,child_TestRangeQuerySpecialCharacters9191);
	child_TestRangeQuerySpecialCharacters9191?0;
		defer1: skip;
