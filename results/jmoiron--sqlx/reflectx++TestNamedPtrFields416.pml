...
// git_link=https://github.com/jmoiron/sqlx/blob//reflectx/reflect_test.go#L416
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNamedPtrFields4160 = [1] of {int};
	run TestNamedPtrFields416(child_TestNamedPtrFields4160);
	run receiver(child_TestNamedPtrFields4160)
stop_process:skip
}

proctype TestNamedPtrFields416(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_FieldByName1335 = [1] of {int};
	chan child_FieldByName1334 = [1] of {int};
	...
	run FieldByName133(m_mutex,child_FieldByName1334);
	child_FieldByName1334?0;
	run FieldByName133(m_mutex,child_FieldByName1335);
	child_FieldByName1335?0;
	stop_process: skip;
	child!0
}
