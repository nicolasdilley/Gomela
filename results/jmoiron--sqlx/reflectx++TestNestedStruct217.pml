...
// git_link=https://github.com/jmoiron/sqlx/blob//reflectx/reflect_test.go#L217
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNestedStruct2170 = [1] of {int};
	run TestNestedStruct217(child_TestNestedStruct2170);
	run receiver(child_TestNestedStruct2170)
stop_process:skip
}

proctype TestNestedStruct217(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	chan child_FieldByName1330 = [1] of {int};
	Mutexdef m_mutex;
	run mutexMonitor(m_mutex);
	run FieldByName133(m_mutex,child_FieldByName1330);
	child_FieldByName1330?0;
