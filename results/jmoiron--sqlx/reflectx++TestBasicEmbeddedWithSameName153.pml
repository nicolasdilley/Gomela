...
// git_link=https://github.com/jmoiron/sqlx/blob//reflectx/reflect_test.go#L153
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestBasicEmbeddedWithSameName1530 = [1] of {int};
	run TestBasicEmbeddedWithSameName153(child_TestBasicEmbeddedWithSameName1530);
	run receiver(child_TestBasicEmbeddedWithSameName1530)
stop_process:skip
}

proctype TestBasicEmbeddedWithSameName153(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
