...
// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example589333928/cmd/diff.go
typedef Chandef {
	chan sync = [0] of {int};
...
init { 
	chan child_cmdreadErrorsFromChannel2 = [0] of {int};
	Wgdef wg;
	bool state = false;
	Chandef errChan;
	int i;
	...
	run cmdreadErrorsFromChannel(errChan,child_cmdreadErrorsFromChannel2);
	child_cmdreadErrorsFromChannel2?0;
...
proctype Anonymous2(Chandef errChan;Wgdef wg) {
	bool closed; 
	int i;
	chan child_cmdprocessImage1 = [0] of {int};
	run cmdprocessImage(errChan,child_cmdprocessImage1);
	child_cmdprocessImage1?0;
	wg.Add!-1;
stop_process:
}
