...
// git_link=https://github.com/cli/cli/blob//pkg/cmd/pr/checkout/checkout_test.go#L145
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestPRCheckout_urlArg_differentBase1450 = [1] of {int};
	run TestPRCheckout_urlArg_differentBase145(child_TestPRCheckout_urlArg_differentBase1450);
	run receiver(child_TestPRCheckout_urlArg_differentBase1450)
stop_process:skip
}

proctype TestPRCheckout_urlArg_differentBase145(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Verify270 = [1] of {int};
	chan child_runCommand243 = [1] of {int};
	chan child_StubRepoInfoResponse262 = [1] of {int};
	chan child_Register151 = [1] of {int};
	Mutexdef http_mu;
	run mutexMonitor(http_mu);
	run Register15(http_mu,child_Register151);
	child_Register151?0;
	run StubRepoInfoResponse26(http_mu,child_StubRepoInfoResponse262);
	child_StubRepoInfoResponse262?0;
	run runCommand24(http_mu,child_runCommand243);
	child_runCommand243?0;
		defer1: skip;
...
proctype StubRepoInfoResponse26(Mutexdef r_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Register152 = [1] of {int};
	run Register15(r_mu,child_Register152);
	child_Register152?0;
	stop_process: skip;
	child!0
}
proctype runCommand24(Mutexdef rt_mu;chan child) {
	...
	bool state;
	int num_msgs;
	chan child_NewCmdCheckout373 = [1] of {int};
	...
	run NewCmdCheckout37(factory_IOStreams_pagerProcess_sigMu,factory_IOStreams_progressIndicator_mu,child_NewCmdCheckout373);
	child_NewCmdCheckout373?0;
