...
// https://github.com/kubernetes/kubernetes/blob/master/pkg/volume/util/nestedpendingoperations/nestedpendingoperations_test.go#L469
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype Test_NestedPendingOperations_Positive_WaitEmpty469(chan child) {
	...
	run AnonymousTest_NestedPendingOperations_Positive_WaitEmpty476475(waitDoneCh,child_AnonymousTest_NestedPendingOperations_Positive_WaitEmpty4764750);
	run waitChannelWithTimeout870(waitDoneCh,child_waitChannelWithTimeout8701);
	child_waitChannelWithTimeout8701?0;
	stop_process: skip;
	child!0
}
proctype AnonymousTest_NestedPendingOperations_Positive_WaitEmpty476475(Chandef waitDoneCh;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
