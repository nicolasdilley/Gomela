...
// git_link=https://github.com/kubernetes/kubernetes/blob//pkg/kubelet/kuberuntime/kuberuntime_manager_test.go#L719
#define not_found_73226  -2
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype TestComputePodActions719(chan child) {
	...
	int var_status_ContainerStatuses = -2; // opt var_status_ContainerStatuses
	run mutexMonitor(m_logReduction_errorMapLock);
	

	if
	:: not_found_73226-1 != -3 -> 
				for(i : 0.. not_found_73226-1) {
			for10: skip;
