...
// git_link=https://github.com/kubernetes/kubernetes/blob//pkg/proxy/userspace/roundrobin_test.go#L499
#define def_var_newEndpoints  -2 // opt newEndpoints line 205
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype OnEndpointsUpdate248(Mutexdef lb_lock;chan child) {
	...
	:: var_portsToEndpoints-1 != -3 -> 
			...
			if
			:: true -> 
				run removeStaleAffinity205(lb_lock,var_newEndpoints,child_removeStaleAffinity20510);
	...
	:: else -> 
		...
		:: true -> 
			...
			if
			:: true -> 
				run removeStaleAffinity205(lb_lock,var_newEndpoints,child_removeStaleAffinity20512);
