...
// git_link=https://github.com/kubernetes/kubernetes/blob//pkg/kubelet/kuberuntime/kuberuntime_manager_test.go#L530
#define def_var_templates  -2 // opt templates line 184
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype TestPruneInitContainers530(chan child) {
	...
	int var_templates = def_var_templates;
	Mutexdef m_logReduction_errorMapLock;
	run mutexMonitor(m_logReduction_errorMapLock);
	run makeFakeContainers184(m_logReduction_errorMapLock,var_templates,child_makeFakeContainers1840);
...
proctype GetPodStatus967(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	int var_podSandboxIDs = -2; // opt var_podSandboxIDs
...
proctype getSandboxIDByPodUID264(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	bool state;
	int num_msgs;
	int var_sandboxes = -2; // opt var_sandboxes
...
proctype determinePodSandboxIPs231(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	bool state;
	int num_msgs;
	int var_podSandbox_Network_AdditionalIps = -2; // opt var_podSandbox_Network_AdditionalIps
...
proctype pruneInitContainersBeforeStart710(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	int var_podStatus_ContainerStatuses = -2; // opt var_podStatus_ContainerStatuses
	int var_initContainerNames = -2; // opt var_initContainerNames
	int var_pod_Spec_InitContainers = -2; // opt var_pod_Spec_InitContainers
