...
// git_link=https://github.com/kubernetes/kubernetes/blob//test/e2e/framework/providers/aws/aws.go#L62
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_GroupSize620 = [1] of {int};
	run GroupSize62(child_GroupSize620);
	run receiver(child_GroupSize620)
stop_process:skip
}

proctype GroupSize62(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
