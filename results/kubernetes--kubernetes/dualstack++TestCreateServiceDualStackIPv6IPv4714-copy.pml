...
// git_link=https://github.com/kubernetes/kubernetes/blob//test/integration/dualstack/dualstack_test.go#L714
typedef Wgdef {
...
init { 
	chan child_TestCreateServiceDualStackIPv6IPv47140 = [1] of {int};
	run TestCreateServiceDualStackIPv6IPv4714(child_TestCreateServiceDualStackIPv6IPv47140);
	run receiver(child_TestCreateServiceDualStackIPv6IPv47140)
stop_process:skip
}

proctype TestCreateServiceDualStackIPv6IPv4714(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	Mutexdef cfg_GenericConfig_HandlerChainWaitGroup_mu;
	Wgdef cfg_GenericConfig_HandlerChainWaitGroup_wg;
	int var_testcases = -2; // opt var_testcases
	run wgMonitor(cfg_GenericConfig_HandlerChainWaitGroup_wg);
	run mutexMonitor(cfg_GenericConfig_HandlerChainWaitGroup_mu);
