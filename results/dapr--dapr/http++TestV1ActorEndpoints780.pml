...
// https://github.com/dapr/dapr/blob/master/pkg/http/api_test.go#L780
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestV1ActorEndpoints7800 = [1] of {int};
	run TestV1ActorEndpoints780(child_TestV1ActorEndpoints7800)
stop_process:skip
}

proctype TestV1ActorEndpoints780(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
