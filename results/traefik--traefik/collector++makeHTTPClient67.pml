...
// git_link=https://github.com/traefik/traefik/blob//pkg/collector/collector.go#L67
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_makeHTTPClient670 = [1] of {int};
	run makeHTTPClient67(child_makeHTTPClient670);
	run receiver(child_makeHTTPClient670)
stop_process:skip
}

proctype makeHTTPClient67(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
