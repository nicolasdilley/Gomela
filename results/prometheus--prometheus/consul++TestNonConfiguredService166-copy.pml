...
// git_link=https://github.com/prometheus/prometheus/blob//discovery/consul/consul_test.go#L166
...
init { 
	chan child_TestNonConfiguredService1660 = [1] of {int};
	run TestNonConfiguredService166(child_TestNonConfiguredService1660);
	run receiver(child_TestNonConfiguredService1660)
stop_process:skip
}

proctype TestNonConfiguredService166(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
