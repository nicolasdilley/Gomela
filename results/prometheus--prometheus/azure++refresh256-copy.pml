...
// git_link=https://github.com/prometheus/prometheus/blob//discovery/azure/azure.go#L256
#define def_var_machines293  3 // mand machines line 293
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype refresh256(chan child) {
	...
	Chandef ch_ch;
	Wgdef wg;
	int var_machines = def_var_machines293; // mand var_machines
	int var_scaleSets = -2; // opt var_scaleSets
...
proctype Anonymousrefresh296296(Chandef ch_ch;Wgdef wg;chan child) {
	...
	bool state;
	int num_msgs;
	int var_networkInterface_IPConfigurations = -2; // opt var_networkInterface_IPConfigurations
	int var_vm_NetworkInterfaces = -2; // opt var_vm_NetworkInterfaces
	int var_vm_Tags = -2; // opt var_vm_Tags
