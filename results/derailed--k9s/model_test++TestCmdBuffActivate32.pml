...
// git_link=https://github.com/derailed/k9s/blob//internal/model/cmd_buff_test.go#L32
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestCmdBuffActivate320 = [1] of {int};
	run TestCmdBuffActivate32(child_TestCmdBuffActivate320);
	run receiver(child_TestCmdBuffActivate320)
stop_process:skip
}

proctype TestCmdBuffActivate32(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
