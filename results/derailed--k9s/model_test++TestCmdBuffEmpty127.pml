...
// git_link=https://github.com/derailed/k9s/blob//internal/model/cmd_buff_test.go#L127
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestCmdBuffEmpty1270 = [1] of {int};
	run TestCmdBuffEmpty127(child_TestCmdBuffEmpty1270);
	run receiver(child_TestCmdBuffEmpty1270)
stop_process:skip
}

proctype TestCmdBuffEmpty127(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	:: var_uu-1 != -3 -> 
			...
			run Empty153(b_mx,child_Empty1532);
			child_Empty1532?0;
	...
	:: else -> 
		...
		:: true -> 
			...
			run Empty153(b_mx,child_Empty1536);
			child_Empty1536?0;
