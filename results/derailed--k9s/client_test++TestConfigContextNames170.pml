...
// git_link=https://github.com/derailed/k9s/blob//internal/client/config_test.go#L170
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestConfigContextNames1700 = [1] of {int};
	run TestConfigContextNames170(child_TestConfigContextNames1700);
	run receiver(child_TestConfigContextNames1700)
stop_process:skip
}

proctype TestConfigContextNames170(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_ContextNames1260 = [1] of {int};
	Mutexdef cfg_mutex;
	...
	run ContextNames126(cfg_flags_lock,cfg_mutex,child_ContextNames1260);
	child_ContextNames1260?0;
	stop_process: skip;
	child!0
}
