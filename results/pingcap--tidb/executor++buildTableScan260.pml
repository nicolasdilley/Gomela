...
// git_link=https://github.com/pingcap/tidb/blob//executor/admin.go#L260
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_buildTableScan2600)
stop_process:skip
}
...
proctype buildTableScan260(chan child) {
	...
	run Select61(kvReq_MemTracker_actionMu,kvReq_MemTracker_mu,kvReq_MemTracker_parMu,child_Select610);
	child_Select610?0;
...
proctype Select61(Mutexdef kvReq_MemTracker_actionMu;Mutexdef kvReq_MemTracker_mu;Mutexdef kvReq_MemTracker_parMu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
