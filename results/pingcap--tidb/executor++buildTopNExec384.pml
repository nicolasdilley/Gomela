...
// git_link=https://github.com/pingcap/tidb/blob//executor/executor_required_rows_test.go#L384
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_buildTopNExec3840 = [1] of {int};
	run buildTopNExec384(child_buildTopNExec3840);
	run receiver(child_buildTopNExec3840)
stop_process:skip
}

proctype buildTopNExec384(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
