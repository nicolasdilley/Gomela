...
// git_link=https://github.com/pingcap/tidb/blob//store/mockstore/mockcopr/rpc_copr.go#L35
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype NewCoprRPCHandler35(chan child) {
	...
	run CheckStreamTimeoutLoop1011(ch_ch,done_ch,child_CheckStreamTimeoutLoop10110);
	run receiver(child_CheckStreamTimeoutLoop10110);
	goto stop_process;
	stop_process: skip;
	child!0
}
proctype CheckStreamTimeoutLoop1011(Chandef ch_ch;Chandef done_ch;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
