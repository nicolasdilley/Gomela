...
// git_link=https://github.com/pingcap/tidb/blob//planner/core/task.go#L220
typedef Wgdef {
...
init { 
	chan child_GetCost2200 = [1] of {int};
	run GetCost220(child_GetCost2200);
	run receiver(child_GetCost2200)
stop_process:skip
}

proctype GetCost220(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
