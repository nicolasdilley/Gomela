...
// git_link=https://github.com/pingcap/tidb/blob//planner/core/plan_to_pb.go#L92
#define def_var_p_Conditions  -2 // opt exprs line 35
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_ToPB920 = [1] of {int};
	run ToPB92(child_ToPB920);
	run receiver(child_ToPB920)
stop_process:skip
}

proctype ToPB92(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_ExpressionsToPBList350 = [1] of {int};
	int var_p_Conditions = def_var_p_Conditions;
	...
	run ExpressionsToPBList35(sc_DiskTracker_actionMu,sc_DiskTracker_mu,sc_DiskTracker_parMu,sc_digestMemo_m,sc_MemTracker_actionMu,sc_MemTracker_mu,sc_MemTracker_parMu,sc_mu,sc_RuntimeStatsColl_mu,var_p_Conditions,child_ExpressionsToPBList350);
