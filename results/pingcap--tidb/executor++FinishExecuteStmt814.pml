...
// https://github.com/pingcap/tidb/blob/master/executor/adapter.go#L814
typedef Wgdef {
...
init { 
	chan child_FinishExecuteStmt8140 = [1] of {int};
	run FinishExecuteStmt814(child_FinishExecuteStmt8140)
stop_process:skip
}

proctype FinishExecuteStmt814(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	Mutexdef statsWithCommit_LockKeys_Mu;
	Mutexdef statsWithCommit_Commit_Mu;
	Mutexdef execDetail_LockKeysDetail_Mu;
	Mutexdef execDetail_CommitDetail_Mu;
	...
	run mutexMonitor(execDetail_CommitDetail_Mu);
	run mutexMonitor(execDetail_LockKeysDetail_Mu);
	

	if
	:: true -> 
		run mutexMonitor(statsWithCommit_Commit_Mu);
		run mutexMonitor(statsWithCommit_LockKeys_Mu)
	:: true;
	fi;
	stop_process: skip;
	child!0
