...
// git_link=https://github.com/v2fly/v2ray-core/blob//common/task/periodic_test.go#L12
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestPeriodicTaskStop120 = [1] of {int};
	run TestPeriodicTaskStop12(child_TestPeriodicTaskStop120);
	run receiver(child_TestPeriodicTaskStop120)
stop_process:skip
}

proctype TestPeriodicTaskStop12(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
