...
// git_link=https://github.com/GoogleContainerTools/skaffold/blob//pkg/skaffold/util/gsutil.go#L32
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Copy320 = [1] of {int};
	run Copy32(child_Copy320);
	run receiver(child_Copy320)
stop_process:skip
}

proctype Copy32(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
