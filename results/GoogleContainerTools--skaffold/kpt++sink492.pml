...
// git_link=https://github.com/GoogleContainerTools/skaffold/blob//pkg/skaffold/deploy/kpt/kpt.go#L492
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_sink4920 = [1] of {int};
	run sink492(child_sink4920);
	run receiver(child_sink4920)
stop_process:skip
}

proctype sink492(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
