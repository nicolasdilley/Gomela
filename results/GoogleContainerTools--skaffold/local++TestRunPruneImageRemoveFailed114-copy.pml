...
// git_link=https://github.com/GoogleContainerTools/skaffold/blob//pkg/skaffold/build/local/prune_test.go#L114
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestRunPruneImageRemoveFailed1140 = [1] of {int};
	run TestRunPruneImageRemoveFailed114(child_TestRunPruneImageRemoveFailed1140);
	run receiver(child_TestRunPruneImageRemoveFailed1140)
stop_process:skip
}

proctype TestRunPruneImageRemoveFailed114(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
