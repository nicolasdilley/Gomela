...
// git_link=https://github.com/GoogleContainerTools/skaffold/blob//pkg/skaffold/build/local/prune_test.go#L139
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestIsPrunedFail1390 = [1] of {int};
	run TestIsPrunedFail139(child_TestIsPrunedFail1390);
	run receiver(child_TestIsPrunedFail1390)
stop_process:skip
}

proctype TestIsPrunedFail139(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
