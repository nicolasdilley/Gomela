...
// git_link=https://github.com/GoogleContainerTools/skaffold/blob//pkg/skaffold/event/event.go#L549
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_LogMetaEvent5490 = [1] of {int};
	run LogMetaEvent549(child_LogMetaEvent5490);
	run receiver(child_LogMetaEvent5490)
stop_process:skip
}

proctype LogMetaEvent549(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
