...
// git_link=https://github.com/GoogleContainerTools/skaffold/blob//pkg/skaffold/deploy/helm/util.go#L80
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_binVer800 = [1] of {int};
	run binVer80(child_binVer800);
	run receiver(child_binVer800)
stop_process:skip
}

proctype binVer80(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
