...
// git_link=https://github.com/GoogleContainerTools/skaffold/blob//pkg/skaffold/build/jib/init.go#L87
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_validate870 = [1] of {int};
	run validate87(child_validate870);
	run receiver(child_validate870)
stop_process:skip
}

proctype validate87(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
