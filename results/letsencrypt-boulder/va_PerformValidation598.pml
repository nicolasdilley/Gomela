...
// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example221834695/va/va.go
typedef Chandef {
	chan sync = [0] of {int};
...
proctype processRemoteResults(Chandef remoteResultsChan;chan child) {
	...
	bool state;
	do
	:: remoteResultsChan.is_closed?state -> 
		if
		:: state -> 
			break
		:: else -> 
			...
			if
			:: remoteResultsChan.async_rcv?0;
			:: remoteResultsChan.sync?0;
			fi;
			for20: skip;
