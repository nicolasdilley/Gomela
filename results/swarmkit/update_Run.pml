...
// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example453434091/manager/orchestrator/update/updater.go
typedef Chandef {
	chan sync = [0] of {int};
...
proctype uworker(Chandef queue;int queue;chan child) {
	bool closed; 
	int i;
	do
...
proctype Anonymous0(Chandef slotQueue) {
	...
	run uworker(slotQueue,slotQueue,child_uworker0);
	child_uworker0?0;
stop_process:
}
