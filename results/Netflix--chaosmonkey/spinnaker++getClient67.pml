...
// git_link=https://github.com/Netflix/chaosmonkey/blob//spinnaker/spinnaker.go#L67
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_getClient670 = [1] of {int};
	run getClient67(child_getClient670);
	run receiver(child_getClient670)
stop_process:skip
}

proctype getClient67(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
