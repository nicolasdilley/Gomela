...
// git_link=https://github.com/Netflix/chaosmonkey/blob//config/monkey_test.go#L71
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestDefaultCronForStartHourBeforeClockStart710 = [1] of {int};
	run TestDefaultCronForStartHourBeforeClockStart71(child_TestDefaultCronForStartHourBeforeClockStart710);
	run receiver(child_TestDefaultCronForStartHourBeforeClockStart710)
stop_process:skip
}

proctype TestDefaultCronForStartHourBeforeClockStart71(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
