...
// git_link=https://github.com/Netflix/chaosmonkey/blob//config/monkey_test.go#L23
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestDefaultCron230 = [1] of {int};
	run TestDefaultCron23(child_TestDefaultCron230);
	run receiver(child_TestDefaultCron230)
stop_process:skip
}

proctype TestDefaultCron23(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
