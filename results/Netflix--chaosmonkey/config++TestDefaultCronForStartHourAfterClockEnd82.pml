...
// git_link=https://github.com/Netflix/chaosmonkey/blob//config/monkey_test.go#L82
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestDefaultCronForStartHourAfterClockEnd820 = [1] of {int};
	run TestDefaultCronForStartHourAfterClockEnd82(child_TestDefaultCronForStartHourAfterClockEnd820);
	run receiver(child_TestDefaultCronForStartHourAfterClockEnd820)
stop_process:skip
}

proctype TestDefaultCronForStartHourAfterClockEnd82(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
