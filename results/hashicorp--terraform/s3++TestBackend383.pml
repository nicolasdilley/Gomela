...
// git_link=https://github.com/hashicorp/terraform/blob//backend/remote-state/s3/backend_test.go#L383
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestBackend3830 = [1] of {int};
	run TestBackend383(child_TestBackend3830);
	run receiver(child_TestBackend3830)
stop_process:skip
}

proctype TestBackend383(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
