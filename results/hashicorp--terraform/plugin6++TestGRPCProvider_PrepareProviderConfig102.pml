...
// git_link=https://github.com/hashicorp/terraform/blob//plugin6/grpc_provider_test.go#L102
typedef Wgdef {
...
init { 
	chan child_TestGRPCProvider_PrepareProviderConfig1020 = [1] of {int};
	run TestGRPCProvider_PrepareProviderConfig102(child_TestGRPCProvider_PrepareProviderConfig1020);
	run receiver(child_TestGRPCProvider_PrepareProviderConfig1020)
stop_process:skip
}

proctype TestGRPCProvider_PrepareProviderConfig102(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_ValidateProviderConfig1760 = [1] of {int};
	Mutexdef p_mu;
