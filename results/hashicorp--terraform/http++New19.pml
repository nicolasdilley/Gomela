...
// git_link=https://github.com/hashicorp/terraform/blob//backend/remote-state/http/backend.go#L19
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_New190 = [1] of {int};
	run New19(child_New190);
	run receiver(child_New190)
stop_process:skip
}

proctype New19(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
