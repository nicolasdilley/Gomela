...
// git_link=https://github.com/hashicorp/terraform/blob//backend/remote-state/etcdv2/backend.go#L16
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_New160 = [1] of {int};
	run New16(child_New160);
	run receiver(child_New160)
stop_process:skip
}

proctype New16(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
