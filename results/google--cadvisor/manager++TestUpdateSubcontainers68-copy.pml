...
// git_link=https://github.com/google/cadvisor/blob//manager/container_test.go#L68
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestUpdateSubcontainers680 = [1] of {int};
	run TestUpdateSubcontainers68(child_TestUpdateSubcontainers680);
	run receiver(child_TestUpdateSubcontainers680)
stop_process:skip
}

proctype TestUpdateSubcontainers68(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_cd_info_Subcontainers = -2; // opt var_cd_info_Subcontainers
