...
// git_link=https://github.com/google/cadvisor/blob//manager/container_test.go#L117
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestUpdateSubcontainersWithErrorOnDeadContainer1170 = [1] of {int};
	run TestUpdateSubcontainersWithErrorOnDeadContainer117(child_TestUpdateSubcontainersWithErrorOnDeadContainer1170);
	run receiver(child_TestUpdateSubcontainersWithErrorOnDeadContainer1170)
stop_process:skip
}

proctype TestUpdateSubcontainersWithErrorOnDeadContainer117(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
