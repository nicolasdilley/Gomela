...
// https://github.com/minio/minio/blob/master/cmd/config/identity/openid/jwt_test.go#L108
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestJWT1080 = [1] of {int};
	run TestJWT108(child_TestJWT1080)
stop_process:skip
}

proctype TestJWT108(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
