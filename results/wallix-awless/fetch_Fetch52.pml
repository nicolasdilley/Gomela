...
// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example924182127/fetch/fetchers.go
typedef Chandef {
...
proctype go_Anonymous0(Chandef results;Wgdef wg) {
	...
	bool state;
	int num_msgs;
	chan child_fetchResource0 = [0] of {int};
	run fetchResource(results,child_fetchResource0);
	child_fetchResource0?0;
	wg.Add!-1;
	stop_process: skip
}
proctype fetchResource(Chandef results;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype go_Anonymous2(Chandef results;Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
