...
// https://github.com/moby/moby/blob/master/pkg/authorization/authz_unix_test.go#L93
typedef Wgdef {
...
init { 
	chan child_TestAuthZResponsePlugin930 = [1] of {int};
	run TestAuthZResponsePlugin93(child_TestAuthZResponsePlugin930)
stop_process:skip
}

proctype TestAuthZResponsePlugin93(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_stop3181 = [1] of {int};
	chan child_AuthZResponse833 = [1] of {int};
	...
	run AuthZResponse83(authZPlugin_once_m,child_AuthZResponse833);
	child_AuthZResponse833?0;
	stop_process: skip;
...
proctype AuthZResponse83(Mutexdef a_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
