...
// git_link=https://github.com/moby/moby/blob//daemon/logger/splunk/splunk.go#L155
#define def_var_streamChannelSize279  3 // mand streamChannelSize line 279
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype New155(chan child) {
	...
	int var_attrs = -2; // opt var_attrs
	int var_streamChannelSize = def_var_streamChannelSize279; // mand var_streamChannelSize
	...
	if
	:: var_streamChannelSize > 0 -> 
		logger_stream_ch.size = var_streamChannelSize;
		run AsyncChan(logger_stream_ch)
	:: else -> 
		run sync_monitor(logger_stream_ch)
	fi;
	...
	if
	:: true -> 
		run verifySplunkConnection623(logger_stream_ch,logger_lock,logger_transport_altMu,logger_transport_connsPerHostMu,logger_transport_idleMu,logger_transport_nextProtoOnce_m,logger_transport_reqMu,logger_transport_TLSClientConfig_mutex,child_verifySplunkConnection6230);
...
proctype verifySplunkConnection623(Chandef l_stream_ch;Mutexdef l_lock;Mutexdef l_transport_altMu;Mutexdef l_transport_connsPerHostMu;Mutexdef l_transport_idleMu;Mutexdef l_transport_nextProtoOnce_m;Mutexdef l_transport_reqMu;Mutexdef l_transport_TLSClientConfig_mutex;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
