...
// git_link=https://github.com/grpc/grpc-go/blob//xds/internal/balancer/cdsbalancer/cdsbalancer_security_test.go#L568
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestSecurityConfigUpdate_GoodToBad5680 = [1] of {int};
	run TestSecurityConfigUpdate_GoodToBad568(child_TestSecurityConfigUpdate_GoodToBad5680);
	run receiver(child_TestSecurityConfigUpdate_GoodToBad5680)
stop_process:skip
}

proctype TestSecurityConfigUpdate_GoodToBad568(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Close4940 = [1] of {int};
	chan child_WaitForCancelClusterWatch1514 = [1] of {int};
	chan child_InvokeWatchClusterCallback1453 = [1] of {int};
	chan child_invokeWatchCbAndWait672 = [1] of {int};
	...
	run invokeWatchCbAndWait67(xdsC_loadStore_mu,child_invokeWatchCbAndWait672);
	child_invokeWatchCbAndWait672?0;
	...
	run WaitForCancelClusterWatch151(xdsC_loadStore_mu,child_WaitForCancelClusterWatch1514);
	child_WaitForCancelClusterWatch1514?0;
		defer1: skip;
	run Close494(cdsB_ccw_xdsHI_mu,cdsB_closed_o_m,cdsB_updateCh_mu,cdsB_xdsHI_mu,child_Close4940);
	child_Close4940?0;
	stop_process: skip;
	child!0
}
