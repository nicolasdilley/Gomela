...
// git_link=https://github.com/grpc/grpc-go/blob//xds/internal/balancer/cdsbalancer/cdsbalancer_security_test.go#L518
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestSecurityConfigUpdate_GoodToFallback5180 = [1] of {int};
	run TestSecurityConfigUpdate_GoodToFallback518(child_TestSecurityConfigUpdate_GoodToFallback5180);
	run receiver(child_TestSecurityConfigUpdate_GoodToFallback5180)
stop_process:skip
}

proctype TestSecurityConfigUpdate_GoodToFallback518(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
