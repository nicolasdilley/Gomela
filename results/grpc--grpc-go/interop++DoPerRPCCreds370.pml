...
// git_link=https://github.com/grpc/grpc-go/blob//interop/test_utils.go#L370
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_DoPerRPCCreds3700 = [1] of {int};
	run DoPerRPCCreds370(child_DoPerRPCCreds3700);
	run receiver(child_DoPerRPCCreds3700)
stop_process:skip
}

proctype DoPerRPCCreds370(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
