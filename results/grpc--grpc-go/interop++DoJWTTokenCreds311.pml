...
// git_link=https://github.com/grpc/grpc-go/blob//interop/test_utils.go#L311
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_DoJWTTokenCreds3110 = [1] of {int};
	run DoJWTTokenCreds311(child_DoJWTTokenCreds3110);
	run receiver(child_DoJWTTokenCreds3110)
stop_process:skip
}

proctype DoJWTTokenCreds311(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
