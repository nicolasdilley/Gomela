...
// git_link=https://github.com/grpc/grpc-go/blob//interop/test_utils.go#L286
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_DoServiceAccountCreds2860 = [1] of {int};
	run DoServiceAccountCreds286(child_DoServiceAccountCreds2860);
	run receiver(child_DoServiceAccountCreds2860)
stop_process:skip
}

proctype DoServiceAccountCreds286(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
