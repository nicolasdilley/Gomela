...
// git_link=https://github.com/grpc/grpc-go/blob//xds/internal/resolver/xds_resolver_test.go#L784
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestXDSResolverDelayedOnCommitted7840 = [1] of {int};
	run TestXDSResolverDelayedOnCommitted784(child_TestXDSResolverDelayedOnCommitted7840);
	run receiver(child_TestXDSResolverDelayedOnCommitted7840)
stop_process:skip
}

proctype TestXDSResolverDelayedOnCommitted784(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Close2620 = [1] of {int};
	chan child_InvokeWatchRouteConfigCallback1117 = [1] of {int};
	chan child_InvokeWatchRouteConfigCallback1116 = [1] of {int};
	...
	run InvokeWatchRouteConfigCallback111(xdsC_loadStore_mu,child_InvokeWatchRouteConfigCallback1116);
	child_InvokeWatchRouteConfigCallback1116?0;
	run InvokeWatchRouteConfigCallback111(xdsC_loadStore_mu,child_InvokeWatchRouteConfigCallback1117);
	child_InvokeWatchRouteConfigCallback1117?0;
		defer1: skip;
