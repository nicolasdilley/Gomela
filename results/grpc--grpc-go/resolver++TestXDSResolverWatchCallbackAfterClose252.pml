...
// git_link=https://github.com/grpc/grpc-go/blob//xds/internal/resolver/xds_resolver_test.go#L252
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestXDSResolverWatchCallbackAfterClose2520 = [1] of {int};
	run TestXDSResolverWatchCallbackAfterClose252(child_TestXDSResolverWatchCallbackAfterClose2520);
	run receiver(child_TestXDSResolverWatchCallbackAfterClose2520)
stop_process:skip
}

proctype TestXDSResolverWatchCallbackAfterClose252(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_InvokeWatchRouteConfigCallback1114 = [1] of {int};
	chan child_Close2623 = [1] of {int};
	...
	run Close262(xdsR_closed_o_m,child_Close2623);
	child_Close2623?0;
