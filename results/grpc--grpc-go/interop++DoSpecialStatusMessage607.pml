...
// git_link=https://github.com/grpc/grpc-go/blob//interop/test_utils.go#L607
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_DoSpecialStatusMessage6070 = [1] of {int};
	run DoSpecialStatusMessage607(child_DoSpecialStatusMessage6070);
	run receiver(child_DoSpecialStatusMessage6070)
stop_process:skip
}

proctype DoSpecialStatusMessage607(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
