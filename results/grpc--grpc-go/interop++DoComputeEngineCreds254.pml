...
// git_link=https://github.com/grpc/grpc-go/blob//interop/test_utils.go#L254
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_DoComputeEngineCreds2540 = [1] of {int};
	run DoComputeEngineCreds254(child_DoComputeEngineCreds2540);
	run receiver(child_DoComputeEngineCreds2540)
stop_process:skip
}

proctype DoComputeEngineCreds254(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
