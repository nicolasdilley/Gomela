...
// git_link=https://github.com/containers/podman/blob//pkg/hooks/hooks_test.go#L114
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestBrokenMatch1140 = [1] of {int};
	run TestBrokenMatch114(child_TestBrokenMatch1140);
	run receiver(child_TestBrokenMatch1140)
stop_process:skip
}

proctype TestBrokenMatch114(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
