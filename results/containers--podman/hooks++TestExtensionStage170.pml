...
// git_link=https://github.com/containers/podman/blob//pkg/hooks/hooks_test.go#L170
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestExtensionStage1700 = [1] of {int};
	run TestExtensionStage170(child_TestExtensionStage1700);
	run receiver(child_TestExtensionStage1700)
stop_process:skip
}

proctype TestExtensionStage170(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
