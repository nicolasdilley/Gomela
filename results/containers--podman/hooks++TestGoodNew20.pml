...
// git_link=https://github.com/containers/podman/blob//pkg/hooks/hooks_test.go#L20
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestGoodNew200 = [1] of {int};
	run TestGoodNew20(child_TestGoodNew200);
	run receiver(child_TestGoodNew200)
stop_process:skip
}

proctype TestGoodNew20(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
