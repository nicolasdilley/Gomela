...
// git_link=https://github.com/elastic/beats/blob//libbeat/cfgfile/list_test.go#L109
#define not_found_113  -2 // opt configs line 54
#define not_found_122  -2 // opt configs line 54
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestReloadSameConfigs1090 = [1] of {int};
	run TestReloadSameConfigs109(child_TestReloadSameConfigs1090);
	run receiver(child_TestReloadSameConfigs1090)
stop_process:skip
}

proctype TestReloadSameConfigs109(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run Reload54(list_mutex,not_found_113,child_Reload540);
	child_Reload540?0;
	run copyRunnerList166(list_mutex,child_copyRunnerList1661);
	child_copyRunnerList1661?0;
	run Reload54(list_mutex,not_found_122,child_Reload542);
	child_Reload542?0;
