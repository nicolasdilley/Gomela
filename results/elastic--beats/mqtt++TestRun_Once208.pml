...
// git_link=https://github.com/elastic/beats/blob//filebeat/input/mqtt/input_test.go#L208
typedef Wgdef {
...
init { 
	chan child_TestRun_Once2080 = [1] of {int};
	run TestRun_Once208(child_TestRun_Once2080);
	run receiver(child_TestRun_Once2080)
stop_process:skip
}

proctype TestRun_Once208(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
