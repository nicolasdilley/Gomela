...
// git_link=https://github.com/elastic/beats/blob//packetbeat/protos/memcache/text_test.go#L139
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_Test_TextParseSetCont1390 = [1] of {int};
	run Test_TextParseSetCont139(child_Test_TextParseSetCont1390);
	run receiver(child_Test_TextParseSetCont1390)
stop_process:skip
}

proctype Test_TextParseSetCont139(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_textNoFail1142 = [1] of {int};
	...
	run textNoFail114(p_testing_context_match_mu,p_testing_context_mu,child_textNoFail1142);
	child_textNoFail1142?0;
	stop_process: skip;
	child!0
}
