...
// git_link=https://github.com/elastic/beats/blob//libbeat/cfgfile/list_test.go#L96
#define not_found_100  -2 // opt configs line 54
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestNewConfigs960 = [1] of {int};
	run TestNewConfigs96(child_TestNewConfigs960);
	run receiver(child_TestNewConfigs960)
stop_process:skip
}

proctype TestNewConfigs96(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run Reload54(list_mutex,not_found_100,child_Reload540);
	child_Reload540?0;
