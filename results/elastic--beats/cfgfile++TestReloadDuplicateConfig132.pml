...
// git_link=https://github.com/elastic/beats/blob//libbeat/cfgfile/list_test.go#L132
#define not_found_136  -2 // opt configs line 54
#define not_found_145  -2 // opt configs line 54
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestReloadDuplicateConfig1320 = [1] of {int};
	run TestReloadDuplicateConfig132(child_TestReloadDuplicateConfig1320);
	run receiver(child_TestReloadDuplicateConfig1320)
stop_process:skip
}

proctype TestReloadDuplicateConfig132(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run Reload54(list_mutex,not_found_136,child_Reload540);
	child_Reload540?0;
	run copyRunnerList166(list_mutex,child_copyRunnerList1661);
	child_copyRunnerList1661?0;
	run Reload54(list_mutex,not_found_145,child_Reload542);
	child_Reload542?0;
