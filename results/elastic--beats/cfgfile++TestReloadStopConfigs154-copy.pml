...
// git_link=https://github.com/elastic/beats/blob//libbeat/cfgfile/list_test.go#L154
#define not_found_158  -2 // opt configs line 54
#define not_found_166  -2 // opt configs line 54
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestReloadStopConfigs1540 = [1] of {int};
	run TestReloadStopConfigs154(child_TestReloadStopConfigs1540);
	run receiver(child_TestReloadStopConfigs1540)
stop_process:skip
}

proctype TestReloadStopConfigs154(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run Reload54(list_mutex,not_found_158,child_Reload540);
	child_Reload540?0;
	run copyRunnerList166(list_mutex,child_copyRunnerList1661);
	child_copyRunnerList1661?0;
	run Reload54(list_mutex,not_found_166,child_Reload542);
	child_Reload542?0;
