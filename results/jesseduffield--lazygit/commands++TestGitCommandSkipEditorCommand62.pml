...
// git_link=https://github.com/jesseduffield/lazygit/blob//pkg/commands/rebasing_test.go#L62
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_TestGitCommandSkipEditorCommand620)
stop_process:skip
}
...
proctype TestGitCommandSkipEditorCommand62(chan child) {
	...
	run runSkipEditorCommand278(cmd_Log_Logger_mu_lock,child_runSkipEditorCommand2780);
	child_runSkipEditorCommand2780?0;
	stop_process: skip;
	child!0
}
proctype runSkipEditorCommand278(Mutexdef c_Log_Logger_mu_lock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
