...
// https://github.com/valyala/fasthttp/blob/master/fasthttputil/pipeconns_test.go#L147
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype testPipeConnsCloseWhileReadWrite147(chan child) {
	...
	od;
	do
	:: writeCh.deq?state,num_msgs -> 
		break
	:: writeCh.sync?state -> 
		writeCh.rcving!false;
		break
	:: true -> 
		break
	od;
...
proctype AnonymoustestPipeConnsCloseWhileReadWrite153152(Chandef readCh;Mutexdef pc_c1_readDeadlineChLock;Mutexdef pc_c2_readDeadlineChLock;Mutexdef pc_stopChLock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
