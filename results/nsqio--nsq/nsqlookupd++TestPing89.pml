...
// git_link=https://github.com/nsqio/nsq/blob//nsqlookupd/http_test.go#L89
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestPing890 = [1] of {int};
	run TestPing89(child_TestPing890);
	run receiver(child_TestPing890)
stop_process:skip
}

proctype TestPing89(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
