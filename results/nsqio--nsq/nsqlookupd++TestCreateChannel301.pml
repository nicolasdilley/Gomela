...
// git_link=https://github.com/nsqio/nsq/blob//nsqlookupd/http_test.go#L301
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestCreateChannel3010 = [1] of {int};
	run TestCreateChannel301(child_TestCreateChannel3010);
	run receiver(child_TestCreateChannel3010)
stop_process:skip
}

proctype TestCreateChannel301(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
