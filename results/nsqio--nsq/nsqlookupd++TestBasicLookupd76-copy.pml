...
// git_link=https://github.com/nsqio/nsq/blob//nsqlookupd/nsqlookupd_test.go#L76
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestBasicLookupd760 = [1] of {int};
	run TestBasicLookupd76(child_TestBasicLookupd760);
	run receiver(child_TestBasicLookupd760)
stop_process:skip
}

proctype TestBasicLookupd76(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_lr_Producers = -2; // opt var_lr_Producers
