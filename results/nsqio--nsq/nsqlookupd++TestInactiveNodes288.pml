...
// git_link=https://github.com/nsqio/nsq/blob//nsqlookupd/nsqlookupd_test.go#L288
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestInactiveNodes2880 = [1] of {int};
	run TestInactiveNodes288(child_TestInactiveNodes2880);
	run receiver(child_TestInactiveNodes2880)
stop_process:skip
}

proctype TestInactiveNodes288(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
