...
// git_link=https://github.com/nsqio/nsq/blob//nsqlookupd/nsqlookupd_test.go#L150
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestChannelUnregister1500 = [1] of {int};
	run TestChannelUnregister150(child_TestChannelUnregister1500);
	run receiver(child_TestChannelUnregister1500)
stop_process:skip
}

proctype TestChannelUnregister150(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
