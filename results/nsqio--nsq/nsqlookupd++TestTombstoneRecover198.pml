...
// git_link=https://github.com/nsqio/nsq/blob//nsqlookupd/nsqlookupd_test.go#L198
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestTombstoneRecover1980 = [1] of {int};
	run TestTombstoneRecover198(child_TestTombstoneRecover1980);
	run receiver(child_TestTombstoneRecover1980)
stop_process:skip
}

proctype TestTombstoneRecover198(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
