...
// git_link=https://github.com/nsqio/nsq/blob//nsqlookupd/http_test.go#L237
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestGetChannels2370 = [1] of {int};
	run TestGetChannels237(child_TestGetChannels2370);
	run receiver(child_TestGetChannels2370)
stop_process:skip
}

proctype TestGetChannels237(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
