...
// git_link=https://github.com/nsqio/nsq/blob//nsqlookupd/http_test.go#L129
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestCreateTopic1290 = [1] of {int};
	run TestCreateTopic129(child_TestCreateTopic1290);
	run receiver(child_TestCreateTopic1290)
stop_process:skip
}

proctype TestCreateTopic129(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
