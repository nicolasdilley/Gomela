...
// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example360880768/queue/queue.go
typedef Chandef {
	chan sync = [0] of {int};
...
init { 
	...
	run sync_monitor(complete);
		for(i : 0.. q_Threads-1) {
for10:		run go_queueindependentRunner(requestc,complete,requestc)
	};
...
proctype go_queueindependentRunner(Chandef requestc;Chandef complete;int requestc) {
	bool closed; 
	int i;
	do
