...
// git_link=https://github.com/ipfs/go-ipfs/blob//thirdparty/notifier/notifier_test.go#L102
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestNotifies1020 = [1] of {int};
	run TestNotifies102(child_TestNotifies1020);
	run receiver(child_TestNotifies1020)
stop_process:skip
}

proctype TestNotifies102(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
