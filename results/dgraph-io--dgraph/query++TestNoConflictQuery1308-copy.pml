...
// git_link=https://github.com/dgraph-io/dgraph/blob//query/query4_test.go#L308
#define def_var_in341  1 // mand in line 341
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	...
	run receiver(child_TestNoConflictQuery13080)
stop_process:skip
}
...
proctype TestNoConflictQuery1308(chan child) {
	...
	int var_in = def_var_in341; // mand var_in
	run sync_monitor(errChan_ch);
		for(i : 0.. var_in-1) {
		for20: skip;
		run AnonymousTestNoConflictQuery1342342(errChan_ch,child_AnonymousTestNoConflictQuery13423420);
...
proctype AnonymousTestNoConflictQuery1342342(Chandef errChan_ch;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
