...
// git_link=https://github.com/gravitational/teleport/blob//lib/auth/accountrecovery_test.go#L109
typedef Wgdef {
...
init { 
	chan child_TestRecoveryCodeEventsEmitted1090 = [1] of {int};
	run TestRecoveryCodeEventsEmitted109(child_TestRecoveryCodeEventsEmitted1090);
	run receiver(child_TestRecoveryCodeEventsEmitted1090)
stop_process:skip
}

proctype TestRecoveryCodeEventsEmitted109(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	Mutexdef mockEmitter_mtx;
	...
	run mutexMonitor(mockEmitter_mtx);
	stop_process: skip;
	child!0
}
