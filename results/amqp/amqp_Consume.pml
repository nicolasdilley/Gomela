...
// /var/folders/28/gltwgskn4998yb1_d73qtg8h0000gn/T/clone-example327388299/channel.go
typedef Chandef {
	chan sync = [0] of {int};
...
proctype ch_consumersadd(Chandef consumer;chan child) {
	...
	Chandef in;
	run sync_monitor(in);
	run go_subsbuffer(in,consumer,in);
	child!0;
stop_process:
}
proctype go_subsbuffer(Chandef in;Chandef out;int in) {
	bool closed; 
	int i;
	do
	:: in.is_closed?state -> 
