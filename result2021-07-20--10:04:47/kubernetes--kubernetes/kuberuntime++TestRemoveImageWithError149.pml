...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/pkg/kubelet/kuberuntime/kuberuntime_image_test.go#L149
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestRemoveImageWithError1490 = [1] of {int};
	run TestRemoveImageWithError149(child_TestRemoveImageWithError1490);
	run receiver(child_TestRemoveImageWithError1490)
stop_process:skip
}

proctype TestRemoveImageWithError149(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype PullImage31(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	bool state;
	int num_msgs;
	int var_credscreds = -2; // opt var_credscreds
