...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/test/integration/auth/auth_test.go#L585
typedef Wgdef {
...
init { 
	chan child_TestAliceNotForbiddenOrUnauthorized5850 = [1] of {int};
	run TestAliceNotForbiddenOrUnauthorized585(child_TestAliceNotForbiddenOrUnauthorized5850);
	run receiver(child_TestAliceNotForbiddenOrUnauthorized5850)
stop_process:skip
}

proctype TestAliceNotForbiddenOrUnauthorized585(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_getTestRequests_ns_Name_getTestRequests_ns_Name_ = -2; // opt var_getTestRequests_ns_Name_getTestRequests_ns_Name_
