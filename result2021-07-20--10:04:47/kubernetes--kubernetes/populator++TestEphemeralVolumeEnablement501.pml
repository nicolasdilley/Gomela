...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/pkg/kubelet/volumemanager/populator/desired_state_of_world_populator_test.go#L501
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestEphemeralVolumeEnablement5010 = [1] of {int};
	run TestEphemeralVolumeEnablement501(child_TestEphemeralVolumeEnablement5010);
	run receiver(child_TestEphemeralVolumeEnablement5010)
stop_process:skip
}

proctype TestEphemeralVolumeEnablement501(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_findAndRemoveDeletedPods2163 = [1] of {int};
	...
	run findAndRemoveDeletedPods216(dswp_hasAddedPodsLock,dswp_volumePluginMgr_mutex,child_findAndRemoveDeletedPods2163);
	child_findAndRemoveDeletedPods2163?0;
	stop_process: skip;
	child!0
}
...
proctype findAndRemoveDeletedPods216(Mutexdef dswp_hasAddedPodsLock;Mutexdef dswp_volumePluginMgr_mutex;chan child) {
	...
	bool state;
	int num_msgs;
	chan child_deleteProcessedPod4823 = [1] of {int};
	chan child_isPodTerminated1802 = [1] of {int};
	chan child_deleteProcessedPod4825 = [1] of {int};
	chan child_isPodTerminated1804 = [1] of {int};
	int var_podsWithErrorpodsWithError = -2; // opt var_podsWithErrorpodsWithError
	int var_runningPodsrunningPods = -2; // opt var_runningPodsrunningPods
	int var_dswp_desiredStateOfWorld_GetVolumesToMount__dswp_desiredStateOfWorld_GetVolumesToMount__ = -2; // opt var_dswp_desiredStateOfWorld_GetVolumesToMount__dswp_desiredStateOfWorld_GetVolumesToMount__
	

	if
	:: var_dswp_desiredStateOfWorld_GetVolumesToMount__-1 != -3 -> 
				for(i : 0.. var_dswp_desiredStateOfWorld_GetVolumesToMount__-1) {
			for30: skip;
			...
			run deleteProcessedPod482(dswp_hasAddedPodsLock,dswp_volumePluginMgr_mutex,child_deleteProcessedPod4823);
			child_deleteProcessedPod4823?0;
			for30_end: skip
		};
		for30_exit: skip
	:: else -> 
		...
		:: true -> 
			...
			:: true -> 
				...
				run isPodTerminated180(dswp_hasAddedPodsLock,dswp_volumePluginMgr_mutex,child_isPodTerminated1804);
				child_isPodTerminated1804?0;
			...
			run deleteProcessedPod482(dswp_hasAddedPodsLock,dswp_volumePluginMgr_mutex,child_deleteProcessedPod4825);
			child_deleteProcessedPod4825?0;
			for33_end: skip
		:: true -> 
			break
		od;
