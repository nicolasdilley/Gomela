...
#define var_vSphereInstances  -2 // opt vSphereInstances line 62
#define var_nodeList_Items108  ?? // mand nodeList.Items line 108
typedef Wgdef {
...
init { 
	...
	run GenerateNodeMap62(var_vSphereInstances,child_GenerateNodeMap620);
...
proctype GenerateNodeMap62(int vSphereInstances;chan child) {
	...
	Wgdef wg;
	int var_nodeList_Items = var_nodeList_Items108; // mand var_nodeList_Items
	int var_datacentersdatacenters = -2; // opt var_datacentersdatacenters
	int var_dcNamedcName = -2; // opt var_dcNamedcName
	run wgMonitor(wg);
...
proctype AnonymousGenerateNodeMap11068(Wgdef wg;chan child) {
	...
	bool state;
	int num_msgs;
	int var_queueChannelqueueChannel = -2; // opt var_queueChannelqueueChannel
	wg.update!-1;
	stop_process: skip;
	child!0
}
