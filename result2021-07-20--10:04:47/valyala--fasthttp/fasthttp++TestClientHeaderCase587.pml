...
// git_link=https://github.com/valyala/fasthttp/blob/19fcd408632d6dae8425ae95c6c62a25c823ff81/client_test.go#L587
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_TestClientHeaderCase5870)
stop_process:skip
}
...
proctype TestClientHeaderCase587(chan child) {
	...
	run receiver(child_AnonymousTestClientHeaderCase5935901);
	run mutexMonitor(c_TLSConfig_mutex);
	run mutexMonitor(c_mLock);
	run Get310(c_mLock,c_TLSConfig_mutex,child_Get3102);
	child_Get3102?0;
		defer1: skip;
...
proctype AnonymousTestClientHeaderCase593590(Mutexdef ln_lock;chan child) {
	...
	bool state;
	int num_msgs;
	chan child_Accept14801 = [1] of {int};
	run Accept1480(ln_lock,child_Accept14801);
	child_Accept14801?0;
	stop_process: skip;
	child!0
}
proctype Accept1480(Mutexdef ln_lock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype Get310(Mutexdef c_mLock;Mutexdef c_TLSConfig_mutex;chan child) {
	...
	run clientGetURL841(c_mLock,c_TLSConfig_mutex,child_clientGetURL8412);
...
proctype clientGetURL841(Mutexdef c_mLock;Mutexdef c_TLSConfig_mutex;chan child) {
	...
	run doRequestFollowRedirectsBuffer966(c_mLock,c_TLSConfig_mutex,child_doRequestFollowRedirectsBuffer9662);
...
proctype doRequestFollowRedirectsBuffer966(Mutexdef c_mLock;Mutexdef c_TLSConfig_mutex;chan child) {
	...
	run doRequestFollowRedirects983(c_mLock,c_TLSConfig_mutex,child_doRequestFollowRedirects9832);
