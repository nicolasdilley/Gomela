...
// git_link=https://github.com/beego/beego/blob/511714d616c8da2a9562c2d0130096f4c7c6b169/server/web/tree_test.go#L156
#define not_found_292  -2 // opt pattern line 295
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestStaticPath1560 = [1] of {int};
	run TestStaticPath156(child_TestStaticPath1560);
	run receiver(child_TestStaticPath1560)
stop_process:skip
}

proctype TestStaticPath156(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Match2871 = [1] of {int};
	...
	child_Match2870?0;
	run Match287(ctx_Input_dataLock,child_Match2871);
	child_Match2871?0;
	stop_process: skip;
	child!0
}
...
proctype match295(Mutexdef ctx_Input_dataLock;int pattern;chan child) {
	...
	int var_allowSuffixExtallowSuffixExt = -2; // opt var_allowSuffixExtallowSuffixExt
	int var_t_fixrouterst_fixrouters = -2; // opt var_t_fixrouterst_fixrouters
	int var_t_wildcard_leavest_wildcard_leaves = -2; // opt var_t_wildcard_leavest_wildcard_leaves
	int var_t_leavest_leaves = -2; // opt var_t_leavest_leaves
	...
	:: true -> 
		...
		:: true -> 
			...
			if
			:: var_t_wildcard_leaves-1 != -3 -> 
								for(i : 0.. var_t_wildcard_leaves-1) {
					for30: skip;
	...
	if
	:: var_t_fixrouters-1 != -3 -> 
				for(i : 0.. var_t_fixrouters-1) {
			for50: skip;
	...
	:: true -> 
		...
		if
		:: var_allowSuffixExt-1 != -3 -> 
						for(i : 0.. var_allowSuffixExt-1) {
				for60: skip;
...
proctype match396(Mutexdef ctx_Input_dataLock;chan child) {
	...
	bool state;
	int num_msgs;
	int var_leaf_wildcardsleaf_wildcards = -2; // opt var_leaf_wildcardsleaf_wildcards
