...
// git_link=https://github.com/go-gitea/gitea/blob/ed393779004335a0afcca0dfac48937db41fabd6/modules/queue/queue_disk.go#L67
typedef Wgdef {
...
init { 
	chan child_NewLevelQueueByteFIFO670 = [1] of {int};
	run NewLevelQueueByteFIFO67(child_NewLevelQueueByteFIFO670);
	run receiver(child_NewLevelQueueByteFIFO670)
stop_process:skip
}

proctype NewLevelQueueByteFIFO67(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	Mutexdef internal_lowLock;
	Mutexdef internal_highLock;
	...
	run mutexMonitor(internal_highLock);
	run mutexMonitor(internal_lowLock);
