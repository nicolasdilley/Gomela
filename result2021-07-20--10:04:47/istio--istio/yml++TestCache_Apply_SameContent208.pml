...
// git_link=https://github.com/istio/istio/blob/9a5df21188426a6a7dd7d7503e01330a79db8a48/pkg/test/util/yml/cache_test.go#L208
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestCache_Apply_SameContent2080 = [1] of {int};
	run TestCache_Apply_SameContent208(child_TestCache_Apply_SameContent2080);
	run receiver(child_TestCache_Apply_SameContent2080)
stop_process:skip
}

proctype TestCache_Apply_SameContent208(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_AllKeys1232 = [1] of {int};
	...
	child_Apply591?0;
	run AllKeys123(c_mu,child_AllKeys1232);
	child_AllKeys1232?0;
	stop_process: skip;
	child!0
}
