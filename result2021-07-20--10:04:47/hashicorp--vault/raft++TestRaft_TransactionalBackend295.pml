...
// git_link=https://github.com/hashicorp/vault/blob/f8f289712a3d37115090591602a2b92503fb3a13/physical/raft/raft_test.go#L295
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestRaft_TransactionalBackend2950 = [1] of {int};
	run TestRaft_TransactionalBackend295(child_TestRaft_TransactionalBackend2950);
	run receiver(child_TestRaft_TransactionalBackend2950)
stop_process:skip
}

proctype TestRaft_TransactionalBackend295(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
