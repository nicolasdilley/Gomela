...
proctype TestConsul_serviceTags358(chan child) {
	...
	bool state;
	int num_msgs;
	chan child_fetchServiceTags5400 = [1] of {int};
	chan child_fetchServiceTags5401 = [1] of {int};
	...
	:: var_tests-1 != -3 -> 
				for(i : 0.. var_tests-1) {
			for10: skip;
			run fetchServiceTags540(c_serviceLock,child_fetchServiceTags5400);
			child_fetchServiceTags5400?0;
			for10_end: skip
		};
		for10_exit: skip
	:: else -> 
		...
		:: true -> 
			for11: skip;
			run fetchServiceTags540(c_serviceLock,child_fetchServiceTags5401);
			child_fetchServiceTags5401?0;
			for11_end: skip
		:: true -> 
			break
		od;
...
proctype fetchServiceTags540(Mutexdef c_serviceLock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
