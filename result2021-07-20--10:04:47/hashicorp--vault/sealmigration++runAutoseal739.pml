...
proctype runAutoseal739(chan child) {
	...
	chan child_DeriveActiveCore2391 = [1] of {int};
	...
	chan child_WaitForNCoresUnsealed2730 = [1] of {int};
	Mutexdef cluster_base_ReloadFuncsLock;
	Mutexdef conf_ReloadFuncsLock;
	int var_cluster_Corescluster_Cores = -2; // opt var_cluster_Corescluster_Cores
	run mutexMonitor(conf_ReloadFuncsLock);
	run mutexMonitor(cluster_base_ReloadFuncsLock);
	run WaitForNCoresUnsealed273(cluster_base_ReloadFuncsLock,child_WaitForNCoresUnsealed2730);
	child_WaitForNCoresUnsealed2730?0;
	...
	run DeriveActiveCore239(cluster_base_ReloadFuncsLock,child_DeriveActiveCore2391);
	child_DeriveActiveCore2391?0;
	run mutexMonitor(client_modifyLock);
	run mutexMonitor(client_config_modifyLock);
	stop_process: skip;
	child!0
}
proctype WaitForNCoresUnsealed273(Mutexdef cluster_base_ReloadFuncsLock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype DeriveActiveCore239(Mutexdef cluster_base_ReloadFuncsLock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
