// num_comm_params=0
// num_mand_comm_params=0
// num_opt_comm_params=0

// git_link=https://github.com/gin-gonic/gin/blob/5452a1d3ef0982ffea95fb9e88e5425b0928c431/context_test.go#L1399
typedef Mutexdef {
	chan Lock = [0] of {bool};
	chan Unlock = [0] of {bool};
	chan RLock = [0] of {bool};
	chan RUnlock = [0] of {bool};
	int Counter = 0;}



init { 
	chan child_TestContextClientIP13990 = [1] of {int};
	run TestContextClientIP1399(child_TestContextClientIP13990);
	run receiver(child_TestContextClientIP13990)
stop_process:skip
}

proctype TestContextClientIP1399(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_ClientIP73331 = [1] of {int};
	chan child_ClientIP73330 = [1] of {int};
	chan child_ClientIP73329 = [1] of {int};
	chan child_ClientIP73328 = [1] of {int};
	chan child_resetTrustedCIDRs139527 = [1] of {int};
	chan child_ClientIP73326 = [1] of {int};
	chan child_resetTrustedCIDRs139525 = [1] of {int};
	chan child_ClientIP73324 = [1] of {int};
	chan child_resetTrustedCIDRs139523 = [1] of {int};
	chan child_ClientIP73322 = [1] of {int};
	chan child_resetTrustedCIDRs139521 = [1] of {int};
	chan child_ClientIP73320 = [1] of {int};
	chan child_resetTrustedCIDRs139519 = [1] of {int};
	chan child_ClientIP73318 = [1] of {int};
	chan child_resetTrustedCIDRs139517 = [1] of {int};
	chan child_ClientIP73316 = [1] of {int};
	chan child_resetTrustedCIDRs139515 = [1] of {int};
	chan child_ClientIP73314 = [1] of {int};
	chan child_resetTrustedCIDRs139513 = [1] of {int};
	chan child_ClientIP73312 = [1] of {int};
	chan child_resetTrustedCIDRs139511 = [1] of {int};
	chan child_ClientIP73310 = [1] of {int};
	chan child_resetTrustedCIDRs13959 = [1] of {int};
	chan child_resetContextForClientIPTests14988 = [1] of {int};
	chan child_ClientIP7337 = [1] of {int};
	chan child_ClientIP7336 = [1] of {int};
	chan child_ClientIP7335 = [1] of {int};
	chan child_ClientIP7334 = [1] of {int};
	chan child_ClientIP7333 = [1] of {int};
	chan child_ClientIP7332 = [1] of {int};
	chan child_resetContextForClientIPTests14981 = [1] of {int};
	chan child_resetTrustedCIDRs13950 = [1] of {int};
	Mutexdef c_mu;
	run mutexMonitor(c_mu);
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs13950);
	child_resetTrustedCIDRs13950?0;
	run resetContextForClientIPTests1498(c_mu,child_resetContextForClientIPTests14981);
	child_resetContextForClientIPTests14981?0;
	run ClientIP733(c_mu,child_ClientIP7332);
	child_ClientIP7332?0;
	run ClientIP733(c_mu,child_ClientIP7333);
	child_ClientIP7333?0;
	run ClientIP733(c_mu,child_ClientIP7334);
	child_ClientIP7334?0;
	run ClientIP733(c_mu,child_ClientIP7335);
	child_ClientIP7335?0;
	run ClientIP733(c_mu,child_ClientIP7336);
	child_ClientIP7336?0;
	run ClientIP733(c_mu,child_ClientIP7337);
	child_ClientIP7337?0;
	run resetContextForClientIPTests1498(c_mu,child_resetContextForClientIPTests14988);
	child_resetContextForClientIPTests14988?0;
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs13959);
	child_resetTrustedCIDRs13959?0;
	run ClientIP733(c_mu,child_ClientIP73310);
	child_ClientIP73310?0;
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs139511);
	child_resetTrustedCIDRs139511?0;
	run ClientIP733(c_mu,child_ClientIP73312);
	child_ClientIP73312?0;
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs139513);
	child_resetTrustedCIDRs139513?0;
	run ClientIP733(c_mu,child_ClientIP73314);
	child_ClientIP73314?0;
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs139515);
	child_resetTrustedCIDRs139515?0;
	run ClientIP733(c_mu,child_ClientIP73316);
	child_ClientIP73316?0;
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs139517);
	child_resetTrustedCIDRs139517?0;
	run ClientIP733(c_mu,child_ClientIP73318);
	child_ClientIP73318?0;
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs139519);
	child_resetTrustedCIDRs139519?0;
	run ClientIP733(c_mu,child_ClientIP73320);
	child_ClientIP73320?0;
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs139521);
	child_resetTrustedCIDRs139521?0;
	run ClientIP733(c_mu,child_ClientIP73322);
	child_ClientIP73322?0;
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs139523);
	child_resetTrustedCIDRs139523?0;
	run ClientIP733(c_mu,child_ClientIP73324);
	child_ClientIP73324?0;
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs139525);
	child_resetTrustedCIDRs139525?0;
	run ClientIP733(c_mu,child_ClientIP73326);
	child_ClientIP73326?0;
	run resetTrustedCIDRs1395(c_mu,child_resetTrustedCIDRs139527);
	child_resetTrustedCIDRs139527?0;
	run ClientIP733(c_mu,child_ClientIP73328);
	child_ClientIP73328?0;
	run ClientIP733(c_mu,child_ClientIP73329);
	child_ClientIP73329?0;
	run ClientIP733(c_mu,child_ClientIP73330);
	child_ClientIP73330?0;
	run ClientIP733(c_mu,child_ClientIP73331);
	child_ClientIP73331?0;
	stop_process: skip;
	child!0
}
proctype resetTrustedCIDRs1395(Mutexdef c_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	stop_process: skip;
	child!0
}
proctype resetContextForClientIPTests1498(Mutexdef c_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	stop_process: skip;
	child!0
}
proctype ClientIP733(Mutexdef c_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_requestHeader8194 = [1] of {int};
	chan child_requestHeader8195 = [1] of {int};
	chan child_RemoteIP7603 = [1] of {int};
	chan child_requestHeader8192 = [1] of {int};
	int var_c_engine_RemoteIPHeadersc_engine_RemoteIPHeaders = -2; // opt var_c_engine_RemoteIPHeadersc_engine_RemoteIPHeaders
	

	if
	:: true -> 
		run requestHeader819(c_mu,child_requestHeader8192);
		child_requestHeader8192?0;
		

		if
		:: true -> 
			goto stop_process
		:: true;
		fi
	:: true;
	fi;
	run RemoteIP760(c_mu,child_RemoteIP7603);
	child_RemoteIP7603?0;
	

	if
	:: true -> 
		goto stop_process
	:: true;
	fi;
	

	if
	:: true -> 
		

		if
		:: var_c_engine_RemoteIPHeaders-1 != -3 -> 
						for(i : 0.. var_c_engine_RemoteIPHeaders-1) {
				for20: skip;
				run requestHeader819(c_mu,child_requestHeader8194);
				child_requestHeader8194?0;
				

				if
				:: true -> 
					goto stop_process
				:: true;
				fi;
				for20_end: skip
			};
			for20_exit: skip
		:: else -> 
			do
			:: true -> 
				for21: skip;
				run requestHeader819(c_mu,child_requestHeader8195);
				child_requestHeader8195?0;
				

				if
				:: true -> 
					goto stop_process
				:: true;
				fi;
				for21_end: skip
			:: true -> 
				break
			od;
			for21_exit: skip
		fi
	:: true;
	fi;
	goto stop_process;
	stop_process: skip;
	child!0
}
proctype requestHeader819(Mutexdef c_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	goto stop_process;
	stop_process: skip;
	child!0
}
proctype RemoteIP760(Mutexdef c_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	int var_c_engine_trustedCIDRsc_engine_trustedCIDRs = -2; // opt var_c_engine_trustedCIDRsc_engine_trustedCIDRs
	

	if
	:: true -> 
		goto stop_process
	:: true;
	fi;
	

	if
	:: true -> 
		goto stop_process
	:: true;
	fi;
	goto stop_process;
	stop_process: skip;
	child!0
}

 /* ================================================================================== */
 /* ================================================================================== */
 /* ================================================================================== */ 
proctype mutexMonitor(Mutexdef m) {
bool locked = false;
do
:: true ->
	if
	:: m.Counter > 0 ->
		if 
		:: m.RUnlock?false -> 
			m.Counter = m.Counter - 1;
		:: m.RLock?false -> 
			m.Counter = m.Counter + 1;
		fi;
	:: locked ->
		m.Unlock?false;
		locked = false;
	:: else ->	 end:	if
		:: m.Unlock?false ->
			assert(0==32);		:: m.Lock?false ->
			locked =true;
		:: m.RUnlock?false ->
			assert(0==32);		:: m.RLock?false ->
			m.Counter = m.Counter + 1;
		fi;
	fi;
od
}

proctype receiver(chan c) {
c?0
}

