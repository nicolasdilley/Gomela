...
// git_link=https://github.com/jaegertracing/jaeger/blob/12bba8c9b91cf4a29d314934bc08f4a80e43c042/pkg/config/tlscfg/cert_watcher_test.go#L237
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_TestReload_err_watch2370)
stop_process:skip
}
...
proctype TestReload_err_watch237(chan child) {
	...
	run newCertWatcher43(opts_certWatcher_mu,child_newCertWatcher430);
	child_newCertWatcher430?0;
	stop_process: skip;
	child!0
}
proctype newCertWatcher43(Mutexdef opts_certWatcher_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run addCertsToWatch132(watcher_mu,opts_certWatcher_mu,child_addCertsToWatch1320);
...
proctype addCertsToWatch132(Mutexdef watcher_mu;Mutexdef opts_certWatcher_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
