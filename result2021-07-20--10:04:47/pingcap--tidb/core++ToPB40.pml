...
// git_link=https://github.com/pingcap/tidb/blob/207ce344cbb044ffb1b2681f1ba320a154979f6d/planner/core/plan_to_pb.go#L40
#define not_found_43  -2 // opt exprs line 35
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_ToPB400 = [1] of {int};
	run ToPB40(child_ToPB400);
	run receiver(child_ToPB400)
stop_process:skip
}

proctype ToPB40(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	Mutexdef sc_mu;
	int var_p_AggFuncsp_AggFuncs = -2; // opt var_p_AggFuncsp_AggFuncs
	run mutexMonitor(sc_mu);
	run mutexMonitor(sc_RuntimeStatsColl_mu);
	run ExpressionsToPBList35(sc_mu,sc_RuntimeStatsColl_mu,not_found_43,child_ExpressionsToPBList350);
...
proctype AggFuncToPBExpr28(Mutexdef sc_mu;Mutexdef sc_RuntimeStatsColl_mu;chan child) {
	...
	int var_aggFunc_ArgsaggFunc_Args = -2; // opt var_aggFunc_ArgsaggFunc_Args
