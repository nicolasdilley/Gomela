...
#define not_found_176  -2 // opt labels line 1902
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype filterStoreCandidate173(chan child) {
	...
	bool state;
	int num_msgs;
	chan child_IsLabelsMatch19020 = [1] of {int};
	Mutexdef s_resolveMutex;
	run mutexMonitor(s_resolveMutex);
	run IsLabelsMatch1902(s_resolveMutex,not_found_176,child_IsLabelsMatch19020);
	child_IsLabelsMatch19020?0;
	goto stop_process;
	stop_process: skip;
	child!0
}
proctype IsLabelsMatch1902(Mutexdef s_resolveMutex;int labels;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
