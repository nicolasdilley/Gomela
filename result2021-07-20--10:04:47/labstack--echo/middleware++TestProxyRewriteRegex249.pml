...
// git_link=https://github.com/labstack/echo/blob/643066594d00891e3151c7ed87244bfeddcd57b9/middleware/proxy_test.go#L249
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	...
	run receiver(child_TestProxyRewriteRegex2490)
stop_process:skip
}
...
proctype TestProxyRewriteRegex249(chan child) {
	...
	Chandef receivedRequestURI_ch;
	int var_testCasestestCases = -2; // opt var_testCasestestCases
	

	if
	:: 1 > 0 -> 
		receivedRequestURI_ch.size = 1;
		run AsyncChan(receivedRequestURI_ch)
	:: else -> 
		run sync_monitor(receivedRequestURI_ch)
	fi;
	...
	run mutexMonitor(e_AutoTLSManager_challengeMu);
		defer1: skip;
	run Close101(upstream_wg,upstream_Config_mu,upstream_mu,upstream_TLS_mutex,child_Close1010);
	child_Close1010?0;
	stop_process: skip;
	child!0
}
proctype Close101(Wgdef r_wg;Mutexdef r_Config_mu;Mutexdef r_mu;Mutexdef r_TLS_mutex;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
