...
// git_link=https://github.com/labstack/echo/blob/643066594d00891e3151c7ed87244bfeddcd57b9/context_test.go#L403
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype TestContext_JSON_DoesntCommitResponseCodePrematurely403(chan child) {
	...
	run JSON489(c_echo_startupMutex,c_lock,child_JSON4891);
	child_JSON4891?0;
	stop_process: skip;
	child!0
}
...
proctype JSON489(Mutexdef c_echo_startupMutex;Mutexdef c_lock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_json4791 = [1] of {int};
	run json479(c_echo_startupMutex,c_lock,child_json4791);
	child_json4791?0;
	goto stop_process;
	stop_process: skip;
	child!0
}
proctype json479(Mutexdef c_echo_startupMutex;Mutexdef c_lock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_writeContentType2201 = [1] of {int};
	run writeContentType220(c_echo_startupMutex,c_lock,child_writeContentType2201);
	child_writeContentType2201?0;
	goto stop_process;
	stop_process: skip;
	child!0
}
proctype writeContentType220(Mutexdef c_echo_startupMutex;Mutexdef c_lock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
