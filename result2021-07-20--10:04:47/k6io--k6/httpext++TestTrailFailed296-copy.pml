...
#define var_testCases325  3 // mand testCases line 325
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype TestTrailFailed296(chan child) {
	...
	int var_sample_GetSamples__sample_GetSamples__ = -2; // opt var_sample_GetSamples__sample_GetSamples__
	int var_testCases = var_testCases325; // mand var_testCases
	...
	:: 10 > 0 -> 
		samples_ch.size = 10;
		run AsyncChan(samples_ch)
	:: else -> 
		run sync_monitor(samples_ch)
	fi;
