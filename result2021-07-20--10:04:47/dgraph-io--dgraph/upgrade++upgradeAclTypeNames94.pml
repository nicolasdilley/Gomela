...
// git_link=https://github.com/dgraph-io/dgraph/blob/6b188f254202f9c38b16c4769b672dd0e9d596ac/upgrade/change_v20.07.0.go#L94
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_upgradeAclTypeNames940)
stop_process:skip
}
...
proctype upgradeAclTypeNames94(chan child) {
	...
	int var_aclTypeNameInfoaclTypeNameInfo = -2; // opt var_aclTypeNameInfoaclTypeNameInfo
	run mutexMonitor(dg_jwtMutex);
	

	if
	:: var_aclTypeNameInfo-1 != -3 -> 
				for(i : 0.. var_aclTypeNameInfo-1) {
			for10: skip;
...
proctype updateTypeName152(Mutexdef dg_jwtMutex;chan child) {
	...
	int var_oldQueryRes_NodesoldQueryRes_Nodes = -2; // opt var_oldQueryRes_NodesoldQueryRes_Nodes
