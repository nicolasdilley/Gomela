...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/user_test.go#L658
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestGetUserWithAcceptedTermsOfServiceForOtherUser6580 = [1] of {int};
	run TestGetUserWithAcceptedTermsOfServiceForOtherUser658(child_TestGetUserWithAcceptedTermsOfServiceForOtherUser6580);
	run receiver(child_TestGetUserWithAcceptedTermsOfServiceForOtherUser6580)
stop_process:skip
}

proctype TestGetUserWithAcceptedTermsOfServiceForOtherUser658(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
