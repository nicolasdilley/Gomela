...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/permissions_test.go#L14
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestGetAncillaryPermissions140 = [1] of {int};
	run TestGetAncillaryPermissions14(child_TestGetAncillaryPermissions140);
	run receiver(child_TestGetAncillaryPermissions140)
stop_process:skip
}

proctype TestGetAncillaryPermissions14(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
