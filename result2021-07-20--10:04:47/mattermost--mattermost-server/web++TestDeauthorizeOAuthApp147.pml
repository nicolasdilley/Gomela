...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/web/oauth_test.go#L147
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestDeauthorizeOAuthApp1470 = [1] of {int};
	run TestDeauthorizeOAuthApp147(child_TestDeauthorizeOAuthApp1470);
	run receiver(child_TestDeauthorizeOAuthApp1470)
stop_process:skip
}

proctype TestDeauthorizeOAuthApp147(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_TearDown1611 = [1] of {int};
	chan child_Logout6642 = [1] of {int};
	...
	child_Login6530?0;
	run Logout664(th_Server_featureFlagSynchronizerMutex,th_Server_metricsLock,th_Server_PluginsLock,th_Server_pluginCommandsLock,th_Server_uploadLockMapMut,child_Logout6642);
	child_Logout6642?0;
		defer1: skip;
...
proctype Logout664(Mutexdef th_Server_featureFlagSynchronizerMutex;Mutexdef th_Server_metricsLock;Mutexdef th_Server_PluginsLock;Mutexdef th_Server_pluginCommandsLock;Mutexdef th_Server_uploadLockMapMut;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
