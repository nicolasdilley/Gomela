...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/data_retention_test.go#L10
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestDataRetentionGetPolicy100 = [1] of {int};
	run TestDataRetentionGetPolicy10(child_TestDataRetentionGetPolicy100);
	run receiver(child_TestDataRetentionGetPolicy100)
stop_process:skip
}

proctype TestDataRetentionGetPolicy10(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
