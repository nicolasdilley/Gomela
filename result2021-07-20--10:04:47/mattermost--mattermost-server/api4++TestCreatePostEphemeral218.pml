...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/post_test.go#L218
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestCreatePostEphemeral2180 = [1] of {int};
	run TestCreatePostEphemeral218(child_TestCreatePostEphemeral2180);
	run receiver(child_TestCreatePostEphemeral2180)
stop_process:skip
}

proctype TestCreatePostEphemeral218(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	Mutexdef ephemeralPost_Post_propsMu;
	...
	run mutexMonitor(ephemeralPost_Post_propsMu);
