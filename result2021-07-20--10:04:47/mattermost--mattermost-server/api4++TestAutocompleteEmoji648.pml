...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/emoji_test.go#L648
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestAutocompleteEmoji6480 = [1] of {int};
	run TestAutocompleteEmoji648(child_TestAutocompleteEmoji6480);
	run receiver(child_TestAutocompleteEmoji6480)
stop_process:skip
}

proctype TestAutocompleteEmoji648(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_remojisremojis = -2; // opt var_remojisremojis
	int var_emojisemojis = -2; // opt var_emojisemojis
