...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/user_test.go#L5967
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestMarkThreadUnreadMentionCount59670 = [1] of {int};
	run TestMarkThreadUnreadMentionCount5967(child_TestMarkThreadUnreadMentionCount59670);
	run receiver(child_TestMarkThreadUnreadMentionCount59670)
stop_process:skip
}

proctype TestMarkThreadUnreadMentionCount5967(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_TearDown2920 = [1] of {int};
	Mutexdef reply_propsMu;
	...
	run mutexMonitor(reply_propsMu);
		defer1: skip;
