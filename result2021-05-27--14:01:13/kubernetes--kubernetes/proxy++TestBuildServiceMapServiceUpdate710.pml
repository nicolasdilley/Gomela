...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/pkg/proxy/service_test.go#L710
#define not_found_736  -2 // opt not_found_736
#define not_found_750  -2 // opt not_found_750
#define not_found_764  -2 // opt not_found_764
#define not_found_777  -2 // opt not_found_777
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype TestBuildServiceMapServiceUpdate710(chan child) {
	...
	child_Update3231?0;
	run updateService543(fp_endpointsChanges_lock,fp_serviceChanges_lock,child_updateService5432);
	child_updateService5432?0;
	run Update323(fp_serviceChanges_lock,not_found_750,child_Update3233);
	child_Update3233?0;
	run updateService543(fp_endpointsChanges_lock,fp_serviceChanges_lock,child_updateService5434);
	child_updateService5434?0;
	run Update323(fp_serviceChanges_lock,not_found_764,child_Update3235);
	child_Update3235?0;
	run updateService543(fp_endpointsChanges_lock,fp_serviceChanges_lock,child_updateService5436);
...
proctype serviceToServiceMap345(Mutexdef sct_lock;chan child) {
	...
	int service_Spec_Ports = -2; // opt service_Spec_Ports
...
proctype newBaseServiceInfo147(Mutexdef sct_lock;chan child) {
	...
	bool state;
	int num_msgs;
	int service_Status_LoadBalancer_Ingress = -2; // opt service_Status_LoadBalancer_Ingress
	int service_Spec_LoadBalancerSourceRanges = -2; // opt service_Spec_LoadBalancerSourceRanges
	goto stop_process;
	stop_process: skip;
	child!0
}
...
proctype apply377(Mutexdef changes_lock;chan child) {
	...
	bool state;
	int num_msgs;
	int changes_items = -2; // opt changes_items
