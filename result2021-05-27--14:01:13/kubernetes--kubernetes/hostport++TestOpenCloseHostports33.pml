...
#define not_found_222  -2 // opt not_found_222
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype TestOpenCloseHostports33(chan child) {
	...
	Mutexdef manager_mu;
	int closePortCases = -2; // opt closePortCases
	int tc_podPortMapping_PortMappings = -2; // opt tc_podPortMapping_PortMappings
	int mapping = -2; // opt mapping
	int openPortCases = -2; // opt openPortCases
...
proctype openHostports282(Mutexdef hm_mu;chan child) {
	...
	int ports = -2; // opt ports
	int podPortMapping_PortMappings = -2; // opt podPortMapping_PortMappings
