...
#define not_found_555  -2 // opt not_found_555
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype generatePodSandboxConfig76(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	int containerPortMappings = -2; // opt containerPortMappings
...
proctype generatePodSandboxLinuxConfig148(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	bool state;
	int num_msgs;
	int sc_SupplementalGroups = -2; // opt sc_SupplementalGroups
	int pod_Spec_SecurityContext_Sysctls = -2; // opt pod_Spec_SecurityContext_Sysctls
	goto stop_process;
	stop_process: skip;
	child!0
}
...
proctype GetPodStatus967(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	int podSandboxIDs = -2; // opt podSandboxIDs
...
proctype determinePodSandboxIPs231(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	bool state;
	int num_msgs;
	int podSandbox_Network_AdditionalIps = -2; // opt podSandbox_Network_AdditionalIps
...
proctype getPodContainerStatuses440(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	int containers = -2; // opt containers
...
proctype pruneInitContainersBeforeStart710(Mutexdef m_logReduction_errorMapLock;chan child) {
	...
	int podStatus_ContainerStatuses = -2; // opt podStatus_ContainerStatuses
	int initContainerNames = -2; // opt initContainerNames
	int pod_Spec_InitContainers = -2; // opt pod_Spec_InitContainers
