...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/pkg/proxy/winuserspace/roundrobin_test.go#L499
#define not_found_227  -2 // opt not_found_227
#define not_found_261  -2 // opt not_found_261
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_TestStickyLoadBalanceWorksWithMultipleEndpointsAndUpdates4990)
stop_process:skip
}
...
proctype OnEndpointsAdd214(Mutexdef lb_lock;chan child) {
	...
	int portsToEndpoints = -2; // opt portsToEndpoints
	lb_lock.Lock!false;
...
proctype updateAffinityMap194(Mutexdef lb_lock;int newEndpoints;chan child) {
	...
	bool state;
	int num_msgs;
	int allEndpoints = -2; // opt allEndpoints
	int state_endpoints = -2; // opt state_endpoints
...
proctype OnEndpointsUpdate241(Mutexdef lb_lock;chan child) {
	...
	int oldPortsToEndpoints = -2; // opt oldPortsToEndpoints
	int portsToEndpoints = -2; // opt portsToEndpoints
	lb_lock.Lock!false;
