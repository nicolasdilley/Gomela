...
// git_link=https://github.com/valyala/fasthttp/blob/19fcd408632d6dae8425ae95c6c62a25c823ff81/http_test.go#L270
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype TestResponseSwapBodyConcurrent270(chan child) {
	...
	run sync_monitor(ch);
		for(i : 0.. 10-1) {
		for10: skip;
		run AnonymousTestResponseSwapBodyConcurrent275273(ch,child_AnonymousTestResponseSwapBodyConcurrent2752730);
...
proctype AnonymousTestResponseSwapBodyConcurrent275273(Chandef ch;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
