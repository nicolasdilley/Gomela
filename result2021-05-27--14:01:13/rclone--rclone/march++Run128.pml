...
proctype Run128(chan child) {
	...
	Wgdef wg;
	Mutexdef mu;
	int checkers = Run_checkers; // mand checkers
	run mutexMonitor(mu);
	run wgMonitor(wg);
	...
	run AnonymousRun203150(in,wg,traversing,mu,child_AnonymousRun2031502);
	...
	int jobs = Run_jobs; // mand jobs
		...
		:: in.deq?state,num_msgs -> 
			...
			:: true -> 
				...
				run AnonymousAnonymousRun153175150(in,wg,traversing,mu,jobs,child_AnonymousAnonymousRun1531751500);
...
proctype AnonymousAnonymousRun153175150(Chandef in;Wgdef wg;Wgdef traversing;Mutexdef mu;int jobs;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype AnonymousRun203150(Chandef in;Wgdef wg;Wgdef traversing;Mutexdef mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
