...
proctype years251(Mutexdef f_albums_mu;chan child) {
	...
	int f_startYear__ = -2; // opt f_startYear__
	int currentYear = -2; // opt currentYear
	run startYear223(f_albums_mu,child_startYear2230);
	...
	:: 0 != -2 && currentYear != -2 -> 
				for(i : 0.. currentYear) {
			for11: skip;
			run dirTime218(f_albums_mu,child_dirTime2182);
			child_dirTime2182?0;
			for11_end: skip
		};
		for11_exit: skip
...
proctype startYear223(Mutexdef f_albums_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
