...
// git_link=https://github.com/hashicorp/vault/blob/f8f289712a3d37115090591602a2b92503fb3a13/vault/activity_log_test.go#L397
#define not_found_{activityEntityBasePathactivityTokenBasePath}38627  -2
typedef Wgdef {
...
init { 
	chan child_TestActivityLog_availableLogs3970 = [1] of {int};
	run TestActivityLog_availableLogs397(child_TestActivityLog_availableLogs3970);
	run receiver(child_TestActivityLog_availableLogs3970)
stop_process:skip
}

proctype TestActivityLog_availableLogs397(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_availableLogs3842 = [1] of {int};
...
proctype availableLogs384(Mutexdef a_core_allLoggersLock;Mutexdef a_core_auditLock;Mutexdef a_core_authLock;Mutexdef a_core_clusterParamsLock;Mutexdef a_core_generateRootLock;Mutexdef a_core_leaderParamsLock;Mutexdef a_core_metricsMutex;Mutexdef a_core_mountsLock;Mutexdef a_core_rekeyLock;Mutexdef a_core_reloadFuncsLock;Mutexdef a_core_requestForwardingConnectionLock;Mutexdef a_core_unsealWithStoredKeysLock;Mutexdef a_fragmentLock;Mutexdef a_l;Mutexdef a_view_readOnlyErrLock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_List532 = [1] of {int};
	chan child_List533 = [1] of {int};
	int paths = -2; // opt paths
	

	if
	:: not_found_{activityEntityBasePathactivityTokenBasePath}38627-1 != -3 -> 
				for(i : 0.. not_found_{activityEntityBasePathactivityTokenBasePath}38627-1) {
			for20: skip;
	...
	:: else -> 
		...
		:: true -> 
			for21: skip;
			run List53(a_view_readOnlyErrLock,child_List533);
			child_List533?0;
