...
proctype Test_Shutdown120(chan child) {
	...
	Chandef appMock_called;
	

	if
	:: 1 > 0 -> 
		appMock_called.size = 1;
		run AsyncChan(appMock_called)
	:: else -> 
		run sync_monitor(appMock_called)
	fi;
	run mutexMonitor(recorder_once_m);
	run mutexMonitor(recorder_mtx);
	run wgMonitor(recorder_wg);
	run NewStdinTargetManager61(appMock_called,recorder_wg,recorder_mtx,recorder_once_m,child_NewStdinTargetManager610);
	child_NewStdinTargetManager610?0;
	

	if
	:: appMock_called.deq?state,num_msgs;
	:: appMock_called.sync?state -> 
		appMock_called.rcving!false
	fi;
...
proctype NewStdinTargetManager61(Chandef app_called;Wgdef client_wg;Mutexdef client_mtx;Mutexdef client_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run AnonymousNewStdinTargetManager710(app_called,client_wg,client_mtx,client_once_m,child_AnonymousNewStdinTargetManager7101);
...
proctype newReaderTarget107(Wgdef client_wg;Mutexdef client_mtx;Mutexdef client_once_m;chan child) {
	...
	bool state;
	int num_msgs;
	int cfg_ServiceDiscoveryConfig_StaticConfigs = -2; // opt cfg_ServiceDiscoveryConfig_StaticConfigs
...
proctype AnonymousNewStdinTargetManager710(Chandef app_called;Wgdef client_wg;Mutexdef client_mtx;Mutexdef client_once_m;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Shutdown1031 = [1] of {int};
	run Shutdown103(app_called,child_Shutdown1031);
	child_Shutdown1031?0;
	stop_process: skip;
	child!0
}
proctype Shutdown103(Chandef m_called;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
