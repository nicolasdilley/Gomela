...
#define not_found_292  -2 // opt not_found_292
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype match295(Mutexdef ctx_Input_dataLock;int pattern;chan child) {
	...
	int allowSuffixExt = -2; // opt allowSuffixExt
	int t_fixrouters = -2; // opt t_fixrouters
	int t_wildcard_leaves = -2; // opt t_wildcard_leaves
	int t_leaves = -2; // opt t_leaves
...
proctype match396(Mutexdef ctx_Input_dataLock;chan child) {
	...
	bool state;
	int num_msgs;
	int leaf_wildcards = -2; // opt leaf_wildcards
