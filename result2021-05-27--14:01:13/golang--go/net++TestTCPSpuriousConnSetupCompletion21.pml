...
proctype TestTCPSpuriousConnSetupCompletion21(chan child) {
	...
	Wgdef wg;
	int runtime_GOMAXPROCS_1_ = TestTCPSpuriousConnSetupCompletion_runtime_GOMAXPROCS_1_; // mand runtime_GOMAXPROCS_1_
	int attempts = TestTCPSpuriousConnSetupCompletion_attempts; // mand attempts
	run wgMonitor(wg);
	wg.update!1;
	...
	if
	:: runtime_GOMAXPROCS_1_ * 2 > 0 -> 
		throttle.size = runtime_GOMAXPROCS_1_ * 2;
		run AsyncChan(throttle)
	:: else -> 
		...
		:: throttle.enq!0;
		:: throttle.sync!false -> 
			throttle.sending!false
		fi;
		run AnonymousTestTCPSpuriousConnSetupCompletion5454(throttle,wg,child_AnonymousTestTCPSpuriousConnSetupCompletion54541);
...
proctype AnonymousTestTCPSpuriousConnSetupCompletion5454(Chandef throttle;Wgdef wg;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	if
	:: throttle.deq?state,num_msgs;
	:: throttle.sync?state -> 
		throttle.rcving!false
	fi;
	wg.update!-1;
	stop_process: skip;
	child!0
