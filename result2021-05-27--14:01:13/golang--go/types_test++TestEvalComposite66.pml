...
// git_link=https://github.com/golang/go/blob/138d2c9b88d9e3d5adcebf9cb7c356b43d6a9782/go/types/eval_test.go#L66
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype TestEvalComposite66(chan child) {
	...
	Mutexdef fset_mutex;
	int independentTestTypes = -2; // opt independentTestTypes
	run mutexMonitor(fset_mutex);
	run mutexMonitor(fset_last_mutex);
	

	if
	:: independentTestTypes-1 != -3 -> 
				for(i : 0.. independentTestTypes-1) {
			for10: skip;
