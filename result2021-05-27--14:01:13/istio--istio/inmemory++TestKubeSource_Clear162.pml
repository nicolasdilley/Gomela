...
// git_link=https://github.com/istio/istio/blob/9a5df21188426a6a7dd7d7503e01330a79db8a48/galley/pkg/config/source/kube/inmemory/kubesource_test.go#L162
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestKubeSource_Clear1620 = [1] of {int};
	run TestKubeSource_Clear162(child_TestKubeSource_Clear1620);
	run receiver(child_TestKubeSource_Clear1620)
stop_process:skip
}

proctype TestKubeSource_Clear162(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Stop1071 = [1] of {int};
	chan child_Clear1123 = [1] of {int};
	...
	run Clear112(s_mu,s_source_mu,child_Clear1123);
	child_Clear1123?0;
		defer1: skip;
...
proctype ApplyContent146(Mutexdef s_mu;Mutexdef s_source_mu;chan child) {
	...
	int oldKeys = -2; // opt oldKeys
	int resources = -2; // opt resources
	s_mu.Lock!false;
...
proctype Clear112(Mutexdef s_mu;Mutexdef s_source_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Clear1123 = [1] of {int};
	run Clear112(s_source_mu,child_Clear1123);
	child_Clear1123?0;
	stop_process: skip;
	child!0
}
