...
proctype New155(chan child) {
	...
	int attrs = -2; // opt attrs
	int streamChannelSize = New_streamChannelSize; // mand streamChannelSize
	...
	if
	:: true -> 
		run verifySplunkConnection623(logger_stream,logger_lock,logger_transport_altMu,logger_transport_connsPerHostMu,logger_transport_idleMu,logger_transport_reqMu,child_verifySplunkConnection6230);
...
proctype verifySplunkConnection623(Chandef l_stream;Mutexdef l_lock;Mutexdef l_transport_altMu;Mutexdef l_transport_connsPerHostMu;Mutexdef l_transport_idleMu;Mutexdef l_transport_reqMu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
