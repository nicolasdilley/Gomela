#define doWriteUniqueKeys_parallelWriters 3
...
init { 
	...
	int parallelWriters = doWriteUniqueKeys_parallelWriters;
	bool state = false;
...
for10_exit:	run dbclientwaitWriters(doneCh,parallelWriters,child_dbclientwaitWriters0);
	child_dbclientwaitWriters0?0;
		for(i : 0.. parallelWriters-1) {
for30:		run go_dbclientwriteUniqueKeys(doneCh)
	};
...
proctype go_dbclientwriteUniqueKeys(Chandef doneCh) {
	...
	if
	:: lb_for188_1 != -1 && ub_for188_2 != -1 -> 
				for(i : lb_for188_1.. ub_for188_2) {
for31:			do
			:: true -> 
				break
			od
