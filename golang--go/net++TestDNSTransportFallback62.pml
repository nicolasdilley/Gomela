...
// git_link=https://github.com/golang/go/blob/138d2c9b88d9e3d5adcebf9cb7c356b43d6a9782/net/dnsclient_unix_test.go#L62
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestDNSTransportFallback620 = [1] of {int};
	run TestDNSTransportFallback62(child_TestDNSTransportFallback620);
	run receiver(child_TestDNSTransportFallback620)
stop_process:skip
}

proctype TestDNSTransportFallback62(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	int var_dnsTransportFallbackTestsdnsTransportFallbackTests = -2; // opt var_dnsTransportFallbackTestsdnsTransportFallbackTests
	run mutexMonitor(r_lookupGroup_mu);
	

	if
	:: var_dnsTransportFallbackTests-1 != -3 -> 
				for(i : 0.. var_dnsTransportFallbackTests-1) {
			for10: skip;
