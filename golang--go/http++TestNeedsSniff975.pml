...
// git_link=https://github.com/golang/go/blob/138d2c9b88d9e3d5adcebf9cb7c356b43d6a9782/net/http/response_test.go#L975
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype TestNeedsSniff975(chan child) {
	...
	run needsSniff553(r_conn_mu,r_writeContinueMu,child_needsSniff5530);
	child_needsSniff5530?0;
	run needsSniff553(r_conn_mu,r_writeContinueMu,child_needsSniff5531);
	child_needsSniff5531?0;
	stop_process: skip;
	child!0
}
proctype needsSniff553(Mutexdef w_conn_mu;Mutexdef w_writeContinueMu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
