...
// https://github.com/valyala/fasthttp/blob/master/fasthttputil/pipeconns_test.go#L272
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_testPipeConnsCloseSerial2720 = [1] of {int};
	run testPipeConnsCloseSerial272(child_testPipeConnsCloseSerial2720)
stop_process:skip
}

proctype testPipeConnsCloseSerial272(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
