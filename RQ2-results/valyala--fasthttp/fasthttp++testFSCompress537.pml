...
// https://github.com/valyala/fasthttp/blob/master/fs_test.go#L537
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_testFSCompress5370 = [1] of {int};
	run testFSCompress537(child_testFSCompress5370)
stop_process:skip
}

proctype testFSCompress537(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
