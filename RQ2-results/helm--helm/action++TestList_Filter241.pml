...
// https://github.com/helm/helm/blob/master/pkg/action/list_test.go#L241
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestList_Filter2410 = [1] of {int};
	run TestList_Filter241(child_TestList_Filter2410)
stop_process:skip
}

proctype TestList_Filter241(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
