...
// https://github.com/helm/helm/blob/master/pkg/action/list_test.go#L163
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestList_StateMask1630 = [1] of {int};
	run TestList_StateMask163(child_TestList_StateMask1630)
stop_process:skip
}

proctype TestList_StateMask163(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_Run1472 = [1] of {int};
	...
	child_Run1471?0;
	run Run147(lister_cfg_RegistryClient_cache_memoryStore_lock,child_Run1472);
	child_Run1472?0;
	stop_process: skip;
	child!0
}
