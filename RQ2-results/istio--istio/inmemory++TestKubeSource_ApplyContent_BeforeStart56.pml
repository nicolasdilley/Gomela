...
// https://github.com/istio/istio/blob/master/galley/pkg/config/source/kube/inmemory/kubesource_test.go#L56
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
proctype Stop107(Mutexdef s_mu;Mutexdef s_source_mu;chan child) {
	...
	int num_msgs;
	chan child_Stop1070 = [1] of {int};
	run Stop107(s_source_mu,child_Stop1070);
	child_Stop1070?0;
	stop_process: skip;
	child!0
}
...
proctype Start102(Mutexdef s_mu;Mutexdef s_source_mu;chan child) {
	...
	int num_msgs;
	chan child_Start1022 = [1] of {int};
	run Start102(s_source_mu,child_Start1022);
	child_Start1022?0;
	stop_process: skip;
	child!0
}
