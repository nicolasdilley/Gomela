...
init { 
	run go_concurrentLoadingVolumes(concurrency136)
stop_process:skip
}

proctype go_concurrentLoadingVolumes(int concurrency) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run go_Anonymous1(task_queue);
	run wgMonitor(wg);
		for(i : 0.. concurrency-1) {
		for20: skip;
		wg.Add!1;
		run go_Anonymous2(task_queue,wg);
		for20_end: skip
	};
	for20_exit: skip;
	wg.Wait?0;
...
proctype go_Anonymous1(Chandef task_queue) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype go_Anonymous2(Chandef task_queue;Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype go_Anonymous3(Chandef task_queue;Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
