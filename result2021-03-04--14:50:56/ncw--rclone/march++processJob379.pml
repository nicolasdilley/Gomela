#define processJob_ci_Checkers  3
#define processJob_srcList  3

// https://github.com/ncw/rclone/blob/80e63af4703730923a97b65fae4d5b712de846c9/fs/march/march.go#L379
typedef Chandef {
	chan sync = [0] of {bool,int};
	chan async_send = [0] of {int};
	chan async_rcv = [0] of {bool,int};
	chan sending = [0] of {int};
	chan closing = [0] of {bool};
	int size = 0;
	int num_msgs = 0;
	bool closed = false;
}
typedef Wgdef {
	chan Add = [0] of {int};
	chan Wait = [0] of {int};
	int Counter = 0;}



init { 
	run go_processJob()
stop_process:skip
}

proctype go_processJob() {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	Chandef limiter;
	Wgdef wg;
	int srcList = processJob_srcList;
	int ci_Checkers = processJob_ci_Checkers;
	run wgMonitor(wg);
	

	if
	:: true -> 
		wg.Add!1;
		run go_Anonymous1(wg)
	:: true;
	fi;
	

	if
	:: true -> 
		wg.Add!1;
		run go_Anonymous2(wg)
	:: true;
	fi;
	wg.Wait?0;
	

	if
	:: true -> 
		goto stop_process
	:: true;
	fi;
	

	if
	:: true -> 
		

		if
		:: true -> 
			goto stop_process
		:: true;
		fi
	fi;
	

	if
	:: ci_Checkers > 0 -> 
		limiter.size = ci_Checkers;
		run AsyncChan(limiter)
	:: else -> 
		run sync_monitor(limiter)
	fi;
	

	if
	:: true -> 
				for(i : 0.. srcList-1) {
			for10: skip;
			wg.Add!1;
			

			if
			:: limiter.async_send!0;
			:: limiter.sync!false,0 -> 
				limiter.sending?state
			fi;
			run go_Anonymous3(limiter,wg);
			for10_end: skip
		};
		for10_exit: skip;
		wg.Wait?0
	:: true;
	fi;
	goto stop_process;
	stop_process: skip
}
proctype go_Anonymous1(Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	stop_process: skip;
		wg.Add!-1
}
proctype go_Anonymous2(Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	stop_process: skip;
		wg.Add!-1
}
proctype go_Anonymous3(Chandef limiter;Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	

	if
	:: limiter.async_rcv?state,num_msgs;
	:: limiter.sync?state,num_msgs;
	fi;
	stop_process: skip;
		wg.Add!-1
}
proctype go_Anonymous4(Chandef limiter;Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	

	if
	:: limiter.async_rcv?state,num_msgs;
	:: limiter.sync?state,num_msgs;
	fi;
	stop_process: skip;
		wg.Add!-1
}

 /* ================================================================================== */
 /* ================================================================================== */
 /* ================================================================================== */ 
proctype AsyncChan(Chandef ch) {
do
:: true ->
if
:: ch.closed -> 
end: if
  :: ch.async_send?0-> // cannot send on closed channel
    assert(false)
  :: ch.closing?true -> // cannot close twice a channel
    assert(false)
  :: ch.sending!true -> // sending state of channel (closed)
    assert(false)
  :: ch.sync!true,ch.num_msgs -> // can always receive on a closed chan
		 ch.num_msgs = ch.num_msgs - 1
  fi;
:: else ->
	if
	:: ch.num_msgs == ch.size ->
		end1: if
		  :: ch.async_rcv!false,ch.num_msgs ->
		    ch.num_msgs = ch.num_msgs - 1
		  :: ch.closing?true -> // closing the channel
		      ch.closed = true
		  :: ch.sending!false;
		fi;
	:: ch.num_msgs == 0 -> 
end2:		if
		:: ch.async_send?0 -> // a message has been received
			ch.num_msgs = ch.num_msgs + 1
		:: ch.closing?true -> // closing the channel
			ch.closed = true
		:: ch.sending!false;
		fi;
		:: else -> 
		end3: if
		  :: ch.async_send?0->
		     ch.num_msgs = ch.num_msgs + 1
		  :: ch.async_rcv!false,ch.num_msgs
		     ch.num_msgs = ch.num_msgs - 1
		  :: ch.closing?true -> // closing the channel
		      ch.closed = true
		  :: ch.sending!false;  // sending channel is open
		fi;
	fi;
fi;
od;
}

proctype sync_monitor(Chandef ch) {
do
:: true
if
:: ch.closed ->
end: if
  :: ch.async_send?0-> // cannot send on closed channel
    assert(false)
  :: ch.closing?true -> // cannot close twice a channel
    assert(false)
  :: ch.sending!true -> // sending state of channel (closed)
    assert(false)
  :: ch.sync!true,0; // can always receive on a closed chan
  fi;
:: else -> 
end1: if
    :: ch.sending!false;
    :: ch.closing?true ->
      ch.closed = true
    fi;
fi;
od
stop_process:
}

proctype wgMonitor(Wgdef wg) {
bool closed;
int i;
bool state;
do
	:: wg.Add?i ->
		wg.Counter = wg.Counter + i;
		assert(wg.Counter >= 0)
	:: wg.Counter == 0 ->
end: if
		:: wg.Add?i ->
			wg.Counter = wg.Counter + i;
			assert(wg.Counter >= 0)
		:: wg.Wait!0;
	fi
od
}

