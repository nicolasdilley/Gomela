...
// https://github.com/dropbox/godropbox/blob/52ad444d35023d078d496d305d75511e772f0295/cinterop/maximally_batched_work.go#L103
typedef Chandef {
...
init { 
	run go_ProcessBatchedData()
stop_process:skip
}

proctype go_ProcessBatchedData() {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	run go_readBatch(readChan);
	run go_writeBatch(writeChan);
	do
	:: true -> 
...
proctype go_readBatch(Chandef copyTo) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype go_writeBatch(Chandef copyFrom) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
