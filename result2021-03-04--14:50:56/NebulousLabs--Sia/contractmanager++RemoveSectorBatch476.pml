...
init { 
	run go_RemoveSectorBatch(sectorRoots476)
stop_process:skip
}

proctype go_RemoveSectorBatch(int sectorRoots) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	:: else -> 
		...
		run go_Anonymous1(semaphore,wg);
		for10_end: skip
	};
	for10_exit: skip;
	wg.Wait?0;
...
proctype go_Anonymous1(Chandef semaphore;Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype go_Anonymous2(Chandef semaphore;Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
