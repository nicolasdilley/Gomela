...
init { 
	run go_evictPods(pods166)
stop_process:skip
}

proctype go_evictPods(int pods) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	:: else -> 
		...
		for(i : 0.. pods-1) {
		for10: skip;
		run go_Anonymous1(doneCh,errCh);
		for10_end: skip
	};
	for10_exit: skip;
	do
...
proctype go_Anonymous1(Chandef doneCh;Chandef errCh) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype go_Anonymous2(Chandef doneCh;Chandef errCh) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
