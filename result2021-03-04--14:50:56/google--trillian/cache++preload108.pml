#define ids108  -2
#define preload_subtrees  1
#define preload_s_populateConcurrency  0

// https://github.com/google/trillian/blob/c92fa63aaa6c133eb8383f2727524421bea420c4/storage/cache/subtree_cache.go#L108
typedef Chandef {
...
init { 
	run go_preload(ids108)
stop_process:skip
}

proctype go_preload(int ids) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	for40_exit: skip;
	run go_Anonymous3(ch,workTokens,wg);
	do
	:: true -> 
...
proctype go_Anonymous2(Chandef ch;Chandef workTokens;Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype go_Anonymous3(Chandef ch;Chandef workTokens;Wgdef wg) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
