...
init { 
	run go_IndexDownload(index51)
stop_process:skip
}

proctype go_IndexDownload(int index) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	for10_exit: skip;
	run go_Anonymous2(inch,errch,index);
...
proctype go_Anonymous2(Chandef inch;Chandef errch;int index) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
