...
// https://github.com/moby/moby/blob/master/daemon/logger/awslogs/cloudwatchlogs_test.go#L340
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype TestLogNonBlockingBufferFull340(chan child) {
	...
	if
	:: stream_messages.enq!0;
	:: stream_messages.sync!false -> 
		stream_messages.sending!false
	fi;
	...
	run AnonymousTestLogNonBlockingBufferFull350342(stream_messages,errorCh,started,stream_lock,child_AnonymousTestLogNonBlockingBufferFull3503420);
...
proctype AnonymousTestLogNonBlockingBufferFull350342(Chandef stream_messages;Chandef errorCh;Chandef started;Mutexdef stream_lock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
