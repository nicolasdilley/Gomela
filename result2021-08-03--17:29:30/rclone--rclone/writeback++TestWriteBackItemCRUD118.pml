...
// git_link=https://github.com/rclone/rclone/blob/a223b78872dc7a8713ca2aa4596ae3e30c173cdd/vfs/vfscache/writeback/writeback_test.go#L118
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_TestWriteBackItemCRUD1180)
stop_process:skip
}
...
proctype checkOnHeap83(Mutexdef wb_mu;chan child) {
	...
	bool state;
	int num_msgs;
	int var_wb_items = -2; // opt var_wb_items
	wb_mu.Lock!false;
		defer1: skip;
	wb_mu.Unlock!false;
	stop_process: skip;
...
proctype string27(Mutexdef wb_mu;chan child) {
	...
	bool state;
	int num_msgs;
	int var_wsCopy = -2; // opt var_wsCopy
	int var_ws = -2; // opt var_ws
	wb_mu.Lock!false;
	goto defer1;
		defer1: skip;
	wb_mu.Unlock!false;
