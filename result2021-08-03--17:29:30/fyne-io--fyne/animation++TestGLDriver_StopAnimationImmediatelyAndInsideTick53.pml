...
// git_link=https://github.com/fyne-io/fyne/blob/9b5a176fe1dc80beae72a3074d0d5cd2b409637c/internal/animation/animation_test.go#L53
#define ub_for68_3  -2
typedef Wgdef {
...
init { 
	...
	run receiver(child_TestGLDriver_StopAnimationImmediatelyAndInsideTick530)
stop_process:skip
}
...
proctype AnonymousrunAnimations670(Mutexdef r_animationMutex;chan child) {
	...
	int var_oldList = -2; // opt var_oldList
...
proctype Stop34(Mutexdef r_animationMutex;chan child) {
	...
	bool state;
	int num_msgs;
	int var_r_pendingAnimations = -2; // opt var_r_pendingAnimations
	int var_r_animations = -2; // opt var_r_animations
	r_animationMutex.Lock!false;
