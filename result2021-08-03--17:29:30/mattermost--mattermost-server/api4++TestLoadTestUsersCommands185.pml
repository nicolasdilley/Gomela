...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/commands_test.go#L185
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestLoadTestUsersCommands1850 = [1] of {int};
	run TestLoadTestUsersCommands185(child_TestLoadTestUsersCommands1850);
	run receiver(child_TestLoadTestUsersCommands1850)
stop_process:skip
}

proctype TestLoadTestUsersCommands185(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
