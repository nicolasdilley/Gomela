...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/saml_test.go#L27
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestSamlCompleteCSRFPass270 = [1] of {int};
	run TestSamlCompleteCSRFPass27(child_TestSamlCompleteCSRFPass270);
	run receiver(child_TestSamlCompleteCSRFPass270)
stop_process:skip
}

proctype TestSamlCompleteCSRFPass27(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
