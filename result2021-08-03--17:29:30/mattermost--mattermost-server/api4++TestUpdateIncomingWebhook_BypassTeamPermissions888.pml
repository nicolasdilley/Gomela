...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/webhook_test.go#L888
#define not_found_897  -2 // opt data line 1132
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestUpdateIncomingWebhook_BypassTeamPermissions8880 = [1] of {int};
	run TestUpdateIncomingWebhook_BypassTeamPermissions888(child_TestUpdateIncomingWebhook_BypassTeamPermissions8880);
	run receiver(child_TestUpdateIncomingWebhook_BypassTeamPermissions8880)
stop_process:skip
}

proctype TestUpdateIncomingWebhook_BypassTeamPermissions888(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	child_CreateChannelWithClientAndTeam5987?0;
		defer2: skip;
	run RestoreDefaultRolePermissions1132(th_BasicPost_propsMu,th_ConfigStore_configLock,th_Server_featureFlagSynchronizerMutex,th_Server_metricsLock,th_Server_PluginsLock,th_Server_pluginCommandsLock,th_Server_uploadLockMapMut,not_found_897,child_RestoreDefaultRolePermissions11322);
