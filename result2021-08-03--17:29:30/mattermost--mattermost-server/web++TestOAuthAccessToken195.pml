...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/web/oauth_test.go#L195
#define not_found_212  -2 // opt data line 693
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestOAuthAccessToken1950 = [1] of {int};
	run TestOAuthAccessToken195(child_TestOAuthAccessToken1950);
	run receiver(child_TestOAuthAccessToken1950)
stop_process:skip
}

proctype TestOAuthAccessToken195(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	child_AddPermissionToRole7515?0;
		defer2: skip;
	run RestoreDefaultRolePermissions693(th_Server_featureFlagSynchronizerMutex,th_Server_metricsLock,th_Server_PluginsLock,th_Server_pluginCommandsLock,th_Server_uploadLockMapMut,not_found_212,child_RestoreDefaultRolePermissions6933);
...
proctype RestoreDefaultRolePermissions693(Mutexdef th_Server_featureFlagSynchronizerMutex;Mutexdef th_Server_metricsLock;Mutexdef th_Server_PluginsLock;Mutexdef th_Server_pluginCommandsLock;Mutexdef th_Server_uploadLockMapMut;int var_data;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
