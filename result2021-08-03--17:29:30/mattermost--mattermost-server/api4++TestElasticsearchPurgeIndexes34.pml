...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/elasticsearch_test.go#L34
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestElasticsearchPurgeIndexes340 = [1] of {int};
	run TestElasticsearchPurgeIndexes34(child_TestElasticsearchPurgeIndexes340);
	run receiver(child_TestElasticsearchPurgeIndexes340)
stop_process:skip
}

proctype TestElasticsearchPurgeIndexes34(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
