...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/app/slashcommands/command_join_test.go#L62
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestJoinCommandWithTilde620 = [1] of {int};
	run TestJoinCommandWithTilde62(child_TestJoinCommandWithTilde620);
	run receiver(child_TestJoinCommandWithTilde620)
stop_process:skip
}

proctype TestJoinCommandWithTilde62(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
