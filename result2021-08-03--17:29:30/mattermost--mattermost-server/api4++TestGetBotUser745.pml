...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/api4/user_test.go#L745
#define not_found_749  -2 // opt data line 1132
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestGetBotUser7450 = [1] of {int};
	run TestGetBotUser745(child_TestGetBotUser7450);
	run receiver(child_TestGetBotUser7450)
stop_process:skip
}

proctype TestGetBotUser745(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
