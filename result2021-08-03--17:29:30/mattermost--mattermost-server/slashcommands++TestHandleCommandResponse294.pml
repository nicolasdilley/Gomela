...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/app/slashcommands/command_test.go#L294
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestHandleCommandResponse2940 = [1] of {int};
	run TestHandleCommandResponse294(child_TestHandleCommandResponse2940);
	run receiver(child_TestHandleCommandResponse2940)
stop_process:skip
}

proctype TestHandleCommandResponse294(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
