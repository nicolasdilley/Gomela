...
// git_link=https://github.com/mattermost/mattermost-server/blob/5c16de58a02099df38e1e8486df148d1b798460a/app/slashcommands/command_leave_test.go#L16
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	chan child_TestLeaveProviderDoCommand160 = [1] of {int};
	run TestLeaveProviderDoCommand16(child_TestLeaveProviderDoCommand160);
	run receiver(child_TestLeaveProviderDoCommand160)
stop_process:skip
}

proctype TestLeaveProviderDoCommand16(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	chan child_tearDown3870 = [1] of {int};
	chan child_createGuest1991 = [1] of {int};
	...
	run createGuest199(th_BasicPost_propsMu,th_Server_featureFlagSynchronizerMutex,th_Server_metricsLock,th_Server_PluginsLock,th_Server_pluginCommandsLock,th_Server_uploadLockMapMut,child_createGuest1991);
	child_createGuest1991?0;
		defer1: skip;
