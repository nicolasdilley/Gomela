...
// git_link=https://github.com/cli/cli/blob/25d79c4e16d54d6f3621517269f07048748a6b97/pkg/cmd/pr/checkout/checkout_test.go#L255
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestPRCheckout_differentRepo_remoteExists2550 = [1] of {int};
	run TestPRCheckout_differentRepo_remoteExists255(child_TestPRCheckout_differentRepo_remoteExists2550);
	run receiver(child_TestPRCheckout_differentRepo_remoteExists2550)
stop_process:skip
}

proctype TestPRCheckout_differentRepo_remoteExists255(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
