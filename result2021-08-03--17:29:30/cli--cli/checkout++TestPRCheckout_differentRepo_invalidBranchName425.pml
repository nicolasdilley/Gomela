...
// git_link=https://github.com/cli/cli/blob/25d79c4e16d54d6f3621517269f07048748a6b97/pkg/cmd/pr/checkout/checkout_test.go#L425
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestPRCheckout_differentRepo_invalidBranchName4250 = [1] of {int};
	run TestPRCheckout_differentRepo_invalidBranchName425(child_TestPRCheckout_differentRepo_invalidBranchName4250);
	run receiver(child_TestPRCheckout_differentRepo_invalidBranchName4250)
stop_process:skip
}

proctype TestPRCheckout_differentRepo_invalidBranchName425(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
