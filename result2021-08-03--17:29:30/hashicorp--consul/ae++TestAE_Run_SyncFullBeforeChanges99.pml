...
// git_link=https://github.com/hashicorp/consul/blob/8d6cbe72813779f60ac24d119cc311c21003f4ce/agent/ae/ae_test.go#L99
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	...
	run receiver(child_TestAE_Run_SyncFullBeforeChanges990)
stop_process:skip
}
...
proctype TestAE_Run_SyncFullBeforeChanges99(chan child) {
	...
	Chandef shutdownCh_ch;
	run sync_monitor(shutdownCh_ch);
	run mutexMonitor(l_pauseLock);
	run wgMonitor(wg);
	wg.update!1;
	run AnonymousTestAE_Run_SyncFullBeforeChanges116100(shutdownCh_ch,wg,l_pauseLock,child_AnonymousTestAE_Run_SyncFullBeforeChanges1161000);
	run receiver(child_AnonymousTestAE_Run_SyncFullBeforeChanges1161000);
	wg.wait?0;
	stop_process: skip;
	child!0
}
proctype AnonymousTestAE_Run_SyncFullBeforeChanges116100(Chandef shutdownCh_ch;Wgdef wg;Mutexdef l_pauseLock;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
