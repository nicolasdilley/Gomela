...
#define def_var_maxServers  -2 // opt maxServers line 167
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run test_reconcileServerList167(def_var_maxServers,child_test_reconcileServerList1670);
	run receiver(child_test_reconcileServerList1670)
stop_process:skip
}

proctype test_reconcileServerList167(int var_maxServers;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
	...
	Mutexdef m_listLock;
	int var_healthyServers = -2; // opt var_healthyServers
	int var_m_getServerList_servers = -2; // opt var_m_getServerList_servers
	int var_failedServers = -2; // opt var_failedServers
...
proctype reconcileServerList383(Mutexdef m_listLock;chan child) {
	...
	int var_mergedList = -2; // opt var_mergedList
	int var_newServerCfg_servers = -2; // opt var_newServerCfg_servers
	int var_l_servers = -2; // opt var_l_servers
	m_listLock.Lock!false;
