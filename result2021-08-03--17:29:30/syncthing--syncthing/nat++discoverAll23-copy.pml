...
#define def_var_providers25  3 // mand providers line 25
#define not_found_31  -2 // opt f line 31
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
proctype discoverAll23(chan child) {
	...
	bool state;
	int num_msgs;
	chan child_AnonymousdiscoverAll45282 = [1] of {int};
	...
	run sync_monitor(done_ch);
		...
		for10: skip;
		run AnonymousdiscoverAll3131(c_ch,done_ch,wg,not_found_31,child_AnonymousdiscoverAll31310);
		run receiver(child_AnonymousdiscoverAll31310);
		for10_end: skip
	};
	for10_exit: skip;
	run AnonymousdiscoverAll4528(done_ch,c_ch,wg,child_AnonymousdiscoverAll45282);
	run receiver(child_AnonymousdiscoverAll45282);
	wg.wait?0;
	c_ch.closing!true;
...
proctype AnonymousdiscoverAll4528(Chandef done_ch;Chandef c_ch;Wgdef wg;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
