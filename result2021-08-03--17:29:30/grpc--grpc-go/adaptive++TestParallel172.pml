...
// git_link=https://github.com/grpc/grpc-go/blob/ebd6aba6754d073a696e5727158cd0c917ce1019/balancer/rls/internal/adaptive/adaptive_test.go#L172
#define def_var_numRoutines181  ?? // mand numRoutines line 181
#define ub_for189_4  -2
typedef Wgdef {
...
init { 
	...
	run receiver(child_TestParallel1720)
stop_process:skip
}
...
proctype TestParallel172(chan child) {
	...
	Wgdef wg;
	Mutexdef th_mu;
	int var_numRoutines = def_var_numRoutines181; // mand var_numRoutines
	run mutexMonitor(th_mu);
	run wgMonitor(wg);
		for(i : 0.. var_numRoutines-1) {
		for10: skip;
		wg.update!1;
		run AnonymousTestParallel183183(wg,th_mu,child_AnonymousTestParallel1831830);
		run receiver(child_AnonymousTestParallel1831830);
		for10_end: skip
	};
	for10_exit: skip;
	wg.wait?0;
