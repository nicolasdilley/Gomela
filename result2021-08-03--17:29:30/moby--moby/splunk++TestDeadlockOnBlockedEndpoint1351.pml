...
// git_link=https://github.com/moby/moby/blob/5a82cee6d499e51bc903a743e4f0f205adb161a9/daemon/logger/splunk/splunk_test.go#L1351
typedef Chandef {
	chan sync = [0] of {bool};
	chan enq = [0] of {int};
...
init { 
	...
	run receiver(child_TestDeadlockOnBlockedEndpoint13510)
stop_process:skip
}
...
proctype TestDeadlockOnBlockedEndpoint1351(chan child) {
	...
	run withBlock68(hec_mu,child_withBlock681);
	child_withBlock681?0;
	run sync_monitor(done_ch);
	run AnonymousTestDeadlockOnBlockedEndpoint13811380(done_ch,hec_mu,child_AnonymousTestDeadlockOnBlockedEndpoint138113802);
	run receiver(child_AnonymousTestDeadlockOnBlockedEndpoint138113802);
	do
	:: true -> 
		break
...
proctype withBlock68(Mutexdef hec_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
...
proctype AnonymousTestDeadlockOnBlockedEndpoint13811380(Chandef done_ch;Mutexdef hec_mu;chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
