...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/pkg/kubelet/dockershim/docker_sandbox_test.go#L175
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	...
	run receiver(child_TestSandboxStatusAfterRestart1750)
stop_process:skip
}
...
proctype getIPsFromPlugin335(Mutexdef ds_cleanupInfosLock;Mutexdef ds_network_podsLock;Mutexdef ds_networkReadyLock;chan child) {
	...
	bool state;
	int num_msgs;
	int var_networkStatus_IPs = -2; // opt var_networkStatus_IPs
