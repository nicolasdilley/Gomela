...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/pkg/kubelet/images/image_gc_manager_test.go#L300
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestFreeSpaceRemoveByLeastRecentlyUsed3000 = [1] of {int};
	run TestFreeSpaceRemoveByLeastRecentlyUsed300(child_TestFreeSpaceRemoveByLeastRecentlyUsed3000);
	run receiver(child_TestFreeSpaceRemoveByLeastRecentlyUsed3000)
stop_process:skip
}

proctype TestFreeSpaceRemoveByLeastRecentlyUsed300(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
