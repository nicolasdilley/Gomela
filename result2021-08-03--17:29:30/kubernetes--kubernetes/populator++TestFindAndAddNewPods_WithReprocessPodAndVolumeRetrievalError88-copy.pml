...
proctype findAndAddNewPods190(Mutexdef dswp_hasAddedPodsLock;Mutexdef dswp_volumePluginMgr_mutex;chan child) {
	...
	int var_dswp_podManager_GetPods = -2; // opt var_dswp_podManager_GetPods
	int var_dswp_actualStateOfWorld_GetMountedVolumes = -2; // opt var_dswp_actualStateOfWorld_GetMountedVolumes
	

	if
	:: var_dswp_podManager_GetPods-1 != -3 -> 
				for(i : 0.. var_dswp_podManager_GetPods-1) {
			for20: skip;
...
proctype processPodVolumes307(Mutexdef dswp_hasAddedPodsLock;Mutexdef dswp_volumePluginMgr_mutex;chan child) {
	...
	chan child_podPreviouslyProcessed4451 = [1] of {int};
	int var_pod_Spec_Volumes = -2; // opt var_pod_Spec_Volumes
