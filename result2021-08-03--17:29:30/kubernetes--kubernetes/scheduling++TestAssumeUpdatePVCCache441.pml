...
// git_link=https://github.com/kubernetes/kubernetes/blob/0277cff2682d5a0d3c6fa2109f7f82e8db3f2ed8/pkg/controller/volume/scheduling/scheduler_assume_cache_test.go#L441
typedef Mutexdef {
	chan Lock = [0] of {bool};
...
init { 
	chan child_TestAssumeUpdatePVCCache4410 = [1] of {int};
	run TestAssumeUpdatePVCCache441(child_TestAssumeUpdatePVCCache4410);
	run receiver(child_TestAssumeUpdatePVCCache4410)
stop_process:skip
}

proctype TestAssumeUpdatePVCCache441(chan child) {
	bool closed; 
	int i;
	bool state;
	int num_msgs;
